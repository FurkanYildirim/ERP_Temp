...
sap.ui.define([
	...
	'sap/apf/utils/trace',
	'sap/ui/Device'
],function(BaseController, trace, Device) {
	"use strict";
	...
	return BaseController.extend("sap.apf.ui.reuse.controller.analysisPath", {
		...
		refreshAnalysisPath : function(startIndex) {
			if(this.getView().getCarouselView().getController().oCoreApi.getSteps().length < 2){
				this.getView().getCarouselView().getController().getView().moveDownButton.setEnabled(false);
				this.getView().getCarouselView().getController().getView().moveUpButton.setEnabled(false);
			}
