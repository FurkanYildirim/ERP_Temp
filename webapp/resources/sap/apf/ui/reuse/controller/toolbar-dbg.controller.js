...
sap.ui.define([
...
], function(Print, utils, Button, Dialog, Input, mobileLibrary, Text, Device, Item, coreLibrary, BaseController, ViewType, JSONModel) {
	"use strict";
	...
	return BaseController.extend("sap.apf.ui.reuse.controller.toolbar", {
		...
		getSaveDialog : function(bSaveAs, reset, aPath) {
			...
			this.oInput.attachLiveChange(function(data) {
				...
				if (sSavePathName.trim() === "") {
					_setStateInSaveDialog(ValueState.Error, true, false);
				} else {
					_setStateInSaveDialog(ValueState.None, false, true);
				}
				if ((sSavePathName.match(regEx) !== null)) {
					var sValueStateMessage = self.oCoreApi.getTextNotHtmlEncoded('invalidPathName');
					_setStateInSaveDialog(ValueState.Error, true, false, sValueStateMessage);
				}
		...
		*/
		 ...
		 */
		apfDestroy : function() {
			utils.checkAndCloseDialog(this.saveDialog);
			utils.checkAndCloseDialog(this.newOpenDialog);
			utils.checkAndCloseDialog(this.newDialog);
			utils.checkAndCloseDialog(this.confirmDialog);
			utils.checkAndCloseDialog(this.errorMsgDialog);
			utils.checkAndCloseDialog(this.noPathAddedDialog);
			//Selection Dialogs
			if (this.deleteAnalysisPath !== undefined) {
				utils.checkAndCloseDialog(this.deleteAnalysisPath.getController().oDialog);
			}
			if (this.pathGallery !== undefined) {
				utils.checkAndCloseDialog(this.pathGallery.getController().oDialog);
			}
		}
	});
});