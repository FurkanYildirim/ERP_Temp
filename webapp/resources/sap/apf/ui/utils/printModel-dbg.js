...
sap.ui.define(
	...
	function(formatter, exportToGlobal, Common) {
		"use strict";
		/**
		 * @alias sap.apf.ui.utils.PrintModel
		 */
		function PrintModel(oInject) {
			this.oUiApi = oInject.uiApi;
			this.oCoreApi = oInject.oCoreApi;
			this.oFilterIdHandler = oInject.oFilterIdHandler;
		}
		PrintModel.prototype.constructor = PrintModel;
	...
	otherwise get formatted filter values for application specific filters from APF****/
		...
		PrintModel.prototype.getFiltersToPrint = function() {
		 ...
		 */
		PrintModel.prototype.getHeaderForFirstPage = function() {
			return this.oUiApi.getLayoutView().getController().oSavedPathName.getText();
		};
		PrintModel.prototype.getApplicationName = function() {
			var oMessageObject;
		...
		PrintModel.prototype.setApplicationConfig = function(conf) {
		 ...
		 */
		PrintModel.prototype.getRepresentationForPrint = function(oStep) {
			var oPrintContent;
			var i;
			var oItems;
			var oItem;
		...
		exportToGlobal("sap.apf.ui.utils.PrintModel", PrintModel);
