...
sap.ui.define([
	 ...
	 * @alias sap.apf.core.ConfigurationFactory
	 */
	function ConfigurationFactory(oInject) {
		...
		var idRegistry = new Hashtable(oInject.instances.messageHandler);
		var setItem = function(oItem) {
			...
			if (!idRegistry) {
				idRegistry = new Hashtable(oInject.instances.messageHandler);
			}
		...
		function loadBinding(oBinding) {
			function checkRepresentationForID() {
				var representationRegistry = new Hashtable(oInject.instances.messageHandler);
		...
		function loadRepresentationType(oRepresentationTypeConfig) {
			...
			if (!isPredefinedRepresentation(oRepresentationTypeConfig.id)) {
				representationConstructorCandidate = utils.extractFunctionFromModulePathString(oRepresentationTypeConfig.constructor);
		...
		function loadFacetFiltersAndConvert(aFacetFilters, currentCounter, deferred, allProperties) {
			...
			function convert(facetFilter, metadataProperty){
				if(utils.isPropertyTypeWithDateSemantics(metadataProperty)){
					facetFilter.preselectionDefaults = utils.convertDateListToInternalFormat(facetFilter.preselectionDefaults, metadataProperty);
					facetFilter.valueList = utils.convertDateListToInternalFormat(facetFilter.valueList, metadataProperty);
				}
			}
		...
		this.loadConfig = function(oConfiguration, bInDesignTime) {
			...
			utils.migrateConfigToCategoryStepAssignment(oConfiguration, inject);
			var aRepresentationTypes = sap.apf.core.representationTypes();
		...
		this.getServiceDocuments = function() {
			...
			return utils.eliminateDuplicatesInArray(oInject.instances.messageHandler, aServiceDocuments);
		};
		/**
		 * @private
		...
		this.getFacetFilterConfigurations = function() {
			...
			for (i = 0; i < facetFilters.length; i++){
				...
				if (facetFilter.preselectionFunction) {
					resolvedFunction = utils.extractFunctionFromModulePathString(facetFilter.preselectionFunction);
	...
	exportToGlobal("sap.apf.core.ConfigurationFactory", ConfigurationFactory);
