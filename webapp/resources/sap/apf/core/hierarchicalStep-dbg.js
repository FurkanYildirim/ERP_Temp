...
sap.ui.define([
...
], function(step, uriGenerator, exportToGlobal, ODataUtils, OperationMode, ODataModel, Sorter, jQuery) {
	"use strict";
	/**
	 * @private
	 * @class A step is a runtime container for binding and request.
	 * @alias sap.apf.core.HierarchicalStep
	 ...
	 */
	function HierarchicalStep(messageHandler, stepConfig, factory, representationId, coreApi) {
		...
		if (sapSystem) {
			service = ODataUtils.setOrigin(service, { force : true, alias : sapSystem});
		}
		...
		this.getSorter = function(){
			...
			if(requestOptions && requestOptions.orderby && requestOptions.orderby.length > 0){
				requestOptions.orderby.forEach(function(orderby){
					sorter.push(new Sorter(orderby.property, !orderby.ascending));
				});
			}
			if(sorter.length === 0){
				return;
	...
	exportToGlobal("sap.apf.core.HierarchicalStep", HierarchicalStep);
