...
sap.ui.define([
          ...
          ],	function(utilsUtils, Hashtable, UI5ODataModel, ODataUtils) {
	'use strict';
	...
	var Metadata = function(oInject, sAbsolutePathToServiceDocument) {
		...
		var ODataModel = oInject.constructors.ODataModel || UI5ODataModel;
		...
		function determineServiceRootWithOrigin(pathToServiceDocument) {
			...
			if (sapSystem) {
				return ODataUtils.setOrigin(pathToServiceDocument, { force : true, alias : sapSystem});
			}
		...
		function defineApiResult(input) {
			...
			function convertAnnotationName(sAlternativeName, property) {
				...
				if (Array.isArray(object[property]) === true) {
