...
sap.ui.define([
...
], function(ajax, exportToGlobal, filter, Hashtable, constants, ODataUtils, jQuery) {
	"use strict";
	/**
	 * @class Handles the session state of an APF based application, e.g. the XSRF token handling
	 * @alias sap.apf.core.SessionHandler
	 */
	function SessionHandler(inject) {
		var apfState = null;
		...
		var pathName = '';
		var sXsrfToken = "";
		var oHashTableXsrfToken = new Hashtable(inject.instances.messageHandler);
		...
		function includeOriginInServiceRoot(serviceRootPath) {
			...
			if (sapSystem) {
				return  ODataUtils.setOrigin(serviceRootPath, { force : true, alias : sapSystem});
			}
	...
	exportToGlobal("sap.apf.core.SessionHandler", SessionHandler);
