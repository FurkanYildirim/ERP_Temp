...
sap.ui.define([
...
], function(datajs, exportToGlobal, utils, constants, jQuery) {
	"use strict";
	/**
	 ...
	 * @alias sap.apf.core.OdataProxy
	 */
	function OdataProxy(serviceConfiguration, inject) {
		...
		function formatValue(sProperty, value, entityMetadata) {
			...
			if (entityMetadata && entityMetadata.dataType) {
				return utils.formatValue(value, entityMetadata.dataType.type);
			}			
	...
	exportToGlobal("sap.apf.core.OdataProxy", OdataProxy);
