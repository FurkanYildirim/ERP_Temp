...
sap.ui.define(
	...
	function(exportToGlobal, utils, jQuery) {
		"use strict";
		/**
		 * @private
		 * @alias sap.apf.modeler.core.ConfigurationEditor
		 ...
		 * @constructor
		 */
		function ConfigurationEditor(configuration, inject, callbackAfterLoad, dataFromCopy) {
			var that = this;
			var filterOption;
				...
				serviceList.forEach(function(serviceRoots) {
					metadataFactory.getMetadata(serviceRoots).done(function(metadata) {
						...
						if (numberOfServices === 0) {
							allKnownProperties = utils.eliminateDuplicatesInArray(messageHandler, allKnownProperties);
				...
				serviceList.forEach(function(serviceRoot) {
					metadataFactory.getMetadata(serviceRoot).done(function(metadata) {
						...
						if (--numberOfServiceLists === 0) {
							deferred.resolve(utils.eliminateDuplicatesInArray(messageHandler, properties));
						}
					});
				});
				if (numberOfServiceLists === 0) {
		...
		exportToGlobal("sap.apf.modeler.core.ConfigurationEditor", ConfigurationEditor);
