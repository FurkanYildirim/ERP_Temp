...
sap.ui.define([
	...
	/**
	 * @private
	 * @alias sap.apf.modeler.core.Step
	 ...
	 * @constructor
	 */
	function Step(stepId, inject, dataFromCopy) {
		...
		this.getTopN = function() {
			if (topNSettings && topNSettings.top > 0) {
				if(Array.isArray(topNSettings.orderby)){
					var selectProperties = this.getSelectProperties();
					var i;
					for(i = topNSettings.orderby.length - 1; i >= 0; i--){
						if(!selectProperties.includes(topNSettings.orderby[i].property)){
	...
	exportToGlobal("sap.apf.modeler.core.Step", Step);
