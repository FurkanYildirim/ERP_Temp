...
sap.ui.define(
	...
	function(Step, exportToGlobal, jQuery) {
		"use strict";
		/**
		 * @private
		 * @alias sap.apf.modeler.core.HierarchicalStep
		 * @class A hierarchical step proxy object providing editor methods on configuration objects.
		 * @extends sap.apf.modeler.core.Step
		 ...
		 * @constructor
		 */
		function HierarchicalStep(stepId, inject, dataFromCopy) {
			Step.call(this, stepId, inject, dataFromCopy);
			...
			this.copy = function(newStepIdForCopy) {
				...
				return new HierarchicalStep((newStepIdForCopy || this.getId()), inject, dataFromCopy);
			};
		}

		// compatiblity export as the global name and the module name differ
		exportToGlobal("sap.apf.modeler.core.HierarchicalStep", HierarchicalStep);
