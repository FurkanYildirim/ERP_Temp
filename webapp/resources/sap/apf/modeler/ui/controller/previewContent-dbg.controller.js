...
sap.ui.define([
	...
	], function(utils, uiUtilsConstants, WrappedChartWithCornerTexts, Controller, jQuery) {
	'use strict';
			...
			for(i = 0; i < len; i++) {
				...
				if (sChartType === oTableRepresentation) {
					aProperties.forEach(function(sProperty) {
						var sPropertyValue = sProperty + " - " + utils.createRandomNumberString(4);
					...
					aMeasures.forEach(function(sMeasure) {
						var sMeasureValue = utils.createRandomNumberString(4);
	...
	function _prepareRepresentationInstance() {
		...
		var oRepresentationConstructor = sap.ui.requireSync(sRepresentationPath) 
			|| utils.extractFunctionFromModulePathString(sRepresentationConstructor);
		var oPropertyRows = {};
	...
	var previewContent = Controller.extend("sap.apf.modeler.ui.controller.previewContent", {
		onInit : function() {
