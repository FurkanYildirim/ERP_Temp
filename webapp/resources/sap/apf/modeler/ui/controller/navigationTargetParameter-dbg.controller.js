...
sap.ui.define([
	...
	function setParameterValuePair(oController){
		...
		if(!oController.navigationTarget.getNavigationParameter(key)){
			...
			oController.byId("idNavigationParametersKey").setValueState(ValueState.None);
			//save old key
			oController.oldKey = key;
		} else {
			oController.byId("idNavigationParametersKey").setValueState(ValueState.Error);
	...
	sap.ui.controller("sap.apf.modeler.ui.controller.navigationTargetParameter", {
		...
		onKeyEntered : function () {
			this.byId("idNavigationParametersKey").setValueState(ValueState.None);
			setParameterValuePair(this);
		},
		onValueEntered : function () {
			this.byId("idNavigationParametersValue").setValueState(ValueState.None);
			setParameterValuePair(this);
		},
		validate : function(){
			if(getValue(this) && !getKey(this)){
				this.byId("idNavigationParametersKey").setValueState(ValueState.Error);
				return false;
			}
			if(!getValue(this) && getKey(this)){
				this.byId("idNavigationParametersValue").setValueState(ValueState.Error);
				return false;
			}
			if(this.byId("idNavigationParametersKey").getValueState() === ValueState.Error){
				return false;
			}
			return true;
		}
