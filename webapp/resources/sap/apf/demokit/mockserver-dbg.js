...
], function(UI5MockServer, parseTextPropertyFile, utils, Log, Storage, jQuery) {
	'use strict';
		...
		var self = {
			...
			importTextsFromFile : function(filePath, modelerService, optionalCallback) {
				...
				jQuery.ajax({
					...
					var textFileInformation = parseTextPropertyFile(data, {
							instances : {
			...
			mockDemokitService: function () {
				//Demokit Service
				var serviceMockServer = new UI5MockServer({
					rootUri: self.constants.DEMOKIT_SERVICE_URL
				});
				serviceMockServer.attachAfter(UI5MockServer.HTTPMETHOD.GET, self._postProcessDemokitData);
			...
			savePath: function(requestObject) {
				...
				if (!oBody.AnalysisPath) {
					oBody.AnalysisPath = utils.createPseudoGuid();
				}
			...
			mockRuntimeService: function (configurationList) {
				...
				mockRuntimeServer.attachBefore(UI5MockServer.HTTPMETHOD.PUT, self.savePath, self.constants.PATH_ENTITYSET);
				mockRuntimeServer.attachBefore(UI5MockServer.HTTPMETHOD.POST, self.savePath, self.constants.PATH_ENTITYSET);
				mockRuntimeServer.attachAfter(UI5MockServer.HTTPMETHOD.GET, self._loadAnalyticalConfiguration, self.constants.CONFIGURATION_ENTITYSET);
				mockRuntimeServer.attachAfter(UI5MockServer.HTTPMETHOD.GET, self.loadTexts, self.constants.TEXTS_ENTITYSET);
			...
			mockConfigurationService: function (configurationList) {
				...
				configurationMockServer.attachAfter(UI5MockServer.HTTPMETHOD.GET, self._loadAnalyticalConfiguration, self.constants.CONFIGURATION_ENTITYSET);
				configurationMockServer.attachAfter(UI5MockServer.HTTPMETHOD.PUT, self.saveAnalyticalConfiguration.bind(null, "PUT"), self.constants.CONFIGURATION_ENTITYSET); //new configuration
				configurationMockServer.attachAfter(UI5MockServer.HTTPMETHOD.POST, self.saveAnalyticalConfiguration.bind(null, "POST"), self.constants.CONFIGURATION_ENTITYSET); // Update configuration
				configurationMockServer.attachAfter(UI5MockServer.HTTPMETHOD.POST, self.saveText, self.constants.TEXTS_ENTITYSET); // New text in modeler
				configurationMockServer.attachAfter(UI5MockServer.HTTPMETHOD.PUT, self.saveText, self.constants.TEXTS_ENTITYSET); // New text while import
				configurationMockServer.attachAfter(UI5MockServer.HTTPMETHOD.GET, self.loadTexts, self.constants.TEXTS_ENTITYSET);
			...
			saveAnalyticalConfiguration : function(method, requestObject) {
				...
				if (method === "POST") {
					configId = utils.createPseudoGuid();
			...
			saveText: function(requestObject) {
				...
				if (!utils.isValidGuid(requestObject.mParameters.oEntity.TextElement)){
					requestObject.mParameters.oEntity.TextElement = utils.createPseudoGuid();
				}
			...
			mockHierarchyService: function () {
				...
				var hierarchyServiceMockServer = new UI5MockServer({
					rootUri: self.constants.DEMOKIT_HIERARCHY_SERVICE_URL
				});
				hierarchyServiceMockServer.attachAfter(UI5MockServer.HTTPMETHOD.GET, responseProcessing);
			...
			_postProcessDemokitData: function(oEvent) {
				...
				if (oXhr && oFilteredData) {
					...
					if (containsPagination) {
						...
						var responseWithoutPagination;
						jQuery.ajax({
							url: urlWithoutPagination,
							dataType: "json",
							async: false,
							success: function(data) {
								responseWithoutPagination = data.d;
							}
						});
			...
			_attachGUIDs: function(results) {
				return results.map(function(dataPoint) {
					if (dataPoint.__metadata) {
						var guid = "('" + utils.createPseudoGuid() + "')";
