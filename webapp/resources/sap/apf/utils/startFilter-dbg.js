...
sap.ui.define([
	...
	var StartFilter = function(inject, config, context) {
		...
		if(config.valueHelpRequest || (Array.isArray(config.valueList) && config.valueList.length > 0)){
		...
		this.deserialize = function(serializedStartFilter) {
			...
			if(Array.isArray(serializedStartFilter.selectedValues)){
		...
		function determineValues(currentCount, concurrenCount) {
			...
			if (valueHelpConfigured && Array.isArray(context)) {
			...
			} else if (Array.isArray(context) && config.filterResolutionRequest && !valueHelpConfigured) {
				sendFilterResolutionRequest(currentCount, concurrenCount).then(function(response) {
					deferredDetermination.resolve(response.data);
				});
		...
		function determineSelected(currentCount) {
			...
			} else if (valueHelpConfigured && !Array.isArray(context) && !(config.preselectionDefaults && config.preselectionDefaults.length > 0) && !config.preselectionFunction) {
