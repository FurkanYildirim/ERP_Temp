...
sap.ui.define([
    ...
    */
    ...
    ShellNavigationMenu.prototype.onAfterRendering = function () {
        ...
        // Only if the App Title is truncated add a tooltip
        var oTitleDomRef = this.oTitle.getDomRef();
        if (oTitleDomRef) {
            var oTitleInnerSpanDomRef = oTitleDomRef.firstChild;
            if (oTitleInnerSpanDomRef) {
                var bTruncated = oTitleDomRef.getBoundingClientRect().right < oTitleInnerSpanDomRef.getBoundingClientRect().right;
                var sNewTooltip = bTruncated ? this.getTitle() : null;
                if (this.oTitle.getTooltip() !== sNewTooltip) {
                    this.oTitle.setTooltip(sNewTooltip);
                    this.oTitle.rerender();
                }
            }
        }
    };
