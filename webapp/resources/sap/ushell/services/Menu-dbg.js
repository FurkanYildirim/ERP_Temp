...
sap.ui.define([
    ...
    Menu.prototype.getDefaultSpace = function () {
        if (Config.last("/core/menu/personalization/enabled")) {
            return Promise.all([
                this.getMenuModel(),
                this.getContentNodes([ContentNodeType.Space, ContentNodeType.Page])
            ])
                .then(function (aResult) {
                    var oModel = aResult[0];
                    var aContentNodes = aResult[1];

                    //use of pinnedSortOrder property to find default space
                    // Pinned sort order is always defined.
                    var oFirstPersonalizedSpace =
                        oModel.getData()
                    ...
                    var oNonEmptySpace = aContentNodes.find(function findContentNodeForMenuEntry (oContentNode) {
