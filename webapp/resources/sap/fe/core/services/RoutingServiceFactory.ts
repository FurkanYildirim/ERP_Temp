...
export type RoutingNavigationInfo = {
	oAsyncContext?: Promise<ODataV4Context>;
	...
	useContext?: Context;
	listBindingForCreate?: ODataListBinding;
...
export class RoutingService extends Service<RoutingServiceSettings> {
	 ...
	 */
	navigateToContext(
		oContext: Context | ODataListBinding,
		mParameters:
			| {
		...
		if (mParameters && mParameters.targetPath && oViewData && oViewData.navigation) {
			...
			if (oRouteDetail.parameters && oContext.isA<Context>("sap.ui.model.odata.v4.Context")) {
		...
		if (mParameters?.updateFCLLevel !== -1 && mParameters?.bRecreateContext !== true) {
			if (oContext.isA<Context>("sap.ui.model.odata.v4.Context")) {
