...
import { constant, equal, getExpressionFromAnnotation, isConstant, or } from "sap/fe/core/helpers/BindingToolkit";
...
export const isRequiredExpression = function (
	oTarget: Property | ActionParameter,
	relativePath?: string[]
): BindingToolkitExpression<boolean> {
	const oFieldControlValue = oTarget.annotations?.Common?.FieldControl?.valueOf();
	const fieldControlValue = getExpressionFromAnnotation(oFieldControlValue, relativePath);
	return or(
		isConstant(fieldControlValue) && equal(fieldControlValue, "Common.FieldControlType/Mandatory"),
		equal(fieldControlValue, 7),
		equal(fieldControlValue, "7")
	);
};
