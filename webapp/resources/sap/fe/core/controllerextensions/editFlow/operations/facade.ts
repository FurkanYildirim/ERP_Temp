...
function update412TransistionMessages(
	...
	let { processedMessageIds, delaySuccessMessages, strictHandlingTransitionFails } = strictHandlingUtilities as StrictHandlingUtilities;
...
function checkforOtherMessages(isChangeSet: boolean) {
	if (isChangeSet) {
		const aMessages: Message[] = messageHandling.getMessages();
