...
const _mControlStateHandlerMap: Record<string, any> = {
	...
	"sap.ui.mdc.FilterBar": {
		...
		apply: async function (filterBar: FilterBar, controlState: ControlState, navParameter: NavigationParameter) {
			try {
				if (controlState) {
					const isInitialStateApplicable = this._isInitialStatesApplicable(controlState?.initialState, filterBar);
					...
					if (navigationType === NavType.hybrid && controlState.fullState !== undefined) {
						const filterBarPropertyInfos = await SelectionVariantToStateFilters.getFilterBarSupportedFields(filterBar),
							...
							xAppStateFilters = SelectionVariantToStateFilters.getConditionsFromSV(
	...
	"sap.ui.mdc.Table": {
		...
		apply: async function (table: Table, controlState: ControlState, navParameters: NavigationParameter) {
			try {
				if (controlState) {
...
class ViewState extends ControllerExtension {
	...
	_isInitialStatesApplicable(initialState: object, control: FilterBar | Table, isNavHybrid?: boolean): boolean {
		return (
			initialState &&
			this.invalidateInitialStateForApply.indexOf(control.getId()) === -1 &&
			this.controlsVariantIdUnavailable.indexOf(control.getId()) === -1 &&
			(isNavHybrid ?? true)
		);
	}
}
