...
import type { RoutingNavigationInfo, RoutingService } from "sap/fe/core/services/RoutingServiceFactory";
...
class InternalRouting extends ControllerExtension {
	...
	_onRouteMatched(oEvent: Event) {
		...
		const oNavigationParameters = oEvent.getParameter("navigationInfo") as RoutingNavigationInfo;
	 ...
	 * @ui5-restricted
	 */
	async _bindDeferred(sTargetPath: string, oNavigationParameters: RoutingNavigationInfo) {
		...
		if (oNavigationParameters.bDeferredContext || !oNavigationParameters.oAsyncContext) {
			...
			if (this._oPageComponent && this._oPageComponent.createDeferredContext) {
				this._oPageComponent.createDeferredContext(
					sTargetPath,
					oNavigationParameters.listBindingForCreate as ODataListBinding,
	...
	async resolvePath(pathToResolve: string, model: ODataModel, navigationParameter: RoutingNavigationInfo): Promise<string> {
		...
		let rootEntityName = currentHashNoParams?.substr(0, currentHashNoParams.indexOf("("));
