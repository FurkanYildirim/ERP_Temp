	...
	switch (applyExpression.Function as ODataFunction) {
		case "odata.concat":
			return concat(
				...applyExpression.Apply.map((applyParam) => {
					return getExpressionFromAnnotation(
						convertSubApplyParameters(applyParam as unknown as SubApplyExpressionFromMetamodel),
						visitedNavigationPaths,
						undefined,
						pathVisitor
		...
		case "odata.uriEncode":
			const parameter = getExpressionFromAnnotation(
				convertSubApplyParameters(applyExpression.Apply[0] as unknown as SubApplyExpressionFromMetamodel),
				visitedNavigationPaths,
				undefined,
				pathVisitor
			);
		...
		case "odata.fillUriTemplate":
			...
			templateParams.forEach((applyParam) => {
				targetObject[applyParam.$Name as string] = getExpressionFromAnnotation(
					convertSubApplyParameters(applyParam.$LabeledElement as SubApplyExpressionFromMetamodel),
					visitedNavigationPaths,
					undefined,
					pathVisitor
				);
