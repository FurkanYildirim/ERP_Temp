...
sap.ui.define([
...
], function (MetadataAnalyser, CommonUtils, SelectionVariant, i18nHelper, CardsNavigationHelper, DynamicDateUtil) {
    "use strict";
    ...
    */
    ...
    /**
     * Adds Custom navigation parameters to configuration parameters of Integration Card.
     * 
     * @param {object} oCardDefinition
     * @param {object} oFinal The configuration parameters
     * @returns {array} aCustomFilters An array consisiting property names of custom filters.
    */
    function addCustomNavigationParametersToManifest(oCardDefinition, oFinal) {
        var oCustomFilterSV = new SelectionVariant(),
            oView = oCardDefinition.view,
            oContext = oView && oView.getBindingContext(),
            oController = oView && oView.getController();

        CardsNavigationHelper.processCustomParameters(oController.oCardComponentData.mainComponent,
            oController.getCardPropertiesModel(),
            oContext && oContext.getAllData(),
            oCustomFilterSV,
            {});

        var aCustomFilters = oCustomFilterSV && oCustomFilterSV.getSelectOptionsPropertyNames() || [];
        var oSelectionOptions = oCustomFilterSV && oCustomFilterSV.selectOptions || {};

        aCustomFilters.forEach(function (sPropertyName) {
            var oPropertyValue = { "parameters": [], "SelectOptions": [{ "PropertyName": sPropertyName, "Ranges": oSelectionOptions[sPropertyName] }] };

            if (oFinal[sPropertyName]) {
                oFinal[sPropertyName].value = JSON.stringify(oPropertyValue);
            } else {
                oFinal[sPropertyName] = {
                    value: JSON.stringify(oPropertyValue)
                };
            }
        });
    ...
    */
    ...
    return {
        ...
        addCustomNavigationParametersToManifest: addCustomNavigationParametersToManifest,
