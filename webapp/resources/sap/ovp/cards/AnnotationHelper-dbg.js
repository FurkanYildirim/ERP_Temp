...
sap.ui.define([
    ...
    function getCardSelections(ovpCardProperties, bODataV4) {
        ...
        return {
            filters: getFilters(ovpCardProperties, oSelectionVariant, bODataV4),
    ...
    function getParameters(oSelectionVariant, bODataV4) {
        ...
        for (var i = 0; i < iLength; i++) {
            ...
            aParameters[aParameters.length] = {
                ...
                value: getPrimitiveValue(oParameter.PropertyValue, bODataV4)
            };
        }
        return aParameters;
    }
    ...
    /**
     * Combines multiple filters having Negation operator 
     * i.e. NE / NotContains / NotStartsWith / NotEndsWith and Sign value 'I' with and operator.
     * 
     * In case if atleast one Range of filter out of multiple have Negation operator and 
     * sign as 'I' ( include ) then use and operator between all range values of filter.
     *
     * @param aFilters - filters array
     * @returns {Array} of model filters
     */
    function addFiltersWithNegation(aFilters) {
        var oPathFrequencyMap = {},
            aRelatedFilters = [],
            bNegationAndIncludeExists = false,
            aNegationOperators = ["NE", "NotContains", "NotStartsWith", "NotEndsWith"],
            oMultiFilter = {},
            aModelFilters = [],
            oMultiFilterPropertyMap = {};

        aFilters.forEach(function(oFilter) {
            oPathFrequencyMap[oFilter.path] = oPathFrequencyMap[oFilter.path] + 1 || 1;
        });

        aFilters.forEach(function(oFilter) {
            if (oPathFrequencyMap[oFilter.path] > 1) {
                aRelatedFilters = aFilters.filter(function(oFilterItem) {
                    return oFilterItem && oFilterItem.path === oFilter.path;
                });
                bNegationAndIncludeExists = aRelatedFilters.some(function(oFilterItem) {
                    return aNegationOperators.indexOf(oFilterItem.operator) > -1 && oFilterItem.sign === "I";
                });

                if (bNegationAndIncludeExists && !oMultiFilterPropertyMap[oFilter.path]) {
                    oMultiFilter = { filters: aRelatedFilters, and: true };
                    aModelFilters.push(oMultiFilter);
                    oMultiFilterPropertyMap[oFilter.path] = true;
                } else if (!oMultiFilterPropertyMap[oFilter.path]) {
     ...
     * @param oSelectionVariant - optional selection variant annotation with SelectOptions configuration
     * @param bODataV4 - identifies an odata v4 model
     * @returns {Array} of model filters
     */
    function getFilters(ovpCardProperties, oSelectionVariant, bODataV4) {
        var aFilters = [];
                ...
                for (var j = 0; j < oSelectOption.Ranges.length; j++) {
                    ...
                    if (sRangeSignEnumMember) {
                        ...
                        var oFilter = {
                            ...
                            value1: getPrimitiveValue(oRange.Low, bODataV4),
                            value2: getPrimitiveValue(oRange.High, bODataV4),
        ...
        aFilters = addFiltersWithNegation(aFilters);
        return aFilters;
    }
    ...
    function getPrimitiveValue(oValue, bODataV4) {
        var value;
    ...
    function getPathOrPrimitiveValue(oItem, bODataV4, bKPIHeader) {
        if (oItem) {
            ...
            if (sPath) {
                ...
                return getPrimitiveValue(oItem, bODataV4);
            }
        } else {
            return "";
        }
    ...
    function formTheFilterByString(iContext, oSelectionVariant, aAllSelVar) {
        ...
        var aFilters = getFilters(oCardPropsModel, oSelectionVariant, bODataV4);
        if (aFilters.length == 0) {
            return "";
        }
        var sProp;
    ...
    function getKPIHeaderAggregateNumber(iContext, dataPoint) {
        ...
        var sFunctionName = "sap.ovp.cards.AnnotationHelper.KpiValueFormatter";
        var sPath = bODataV4 ? dataPoint.Value.$Path : dataPoint.Value.Path;
        var aParts = [sPath];
