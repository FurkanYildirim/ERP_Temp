...
sap.ui.define([
	...
	ThreeUtils.computeClusterBoundingBoxes = function(clusters, data, nativeScene, textureUpdateResults) {
		...
		var cluster, instances, repeats, j, jCount, instanceIndex, k, offset, t, w, additionals, node, rg;
		var maxRadiusSq;
	...
	ThreeUtils.initClustersBeforeProcessingHierarchy = function(clusters) {
		for (var i = 0, iCount = clusters.length; i < iCount; ++i) {
			clusters[i].additionalRenderables = [];
		}
	};
	...
	ThreeUtils.updateNodeClusterRenderingInfo = function(node, geometryClusters, scratchBuf, textureDataArray, textureUpdateResults) {
		...
		if (rg) {
			if (rg.instanceIndex === null) {
				...
				geometryClusters[rg.clusterIndex].additionalRenderables.push(node);
	...
	ThreeUtils.buildEmptyMesh = function(boundingBox, flags, pointCount, elementCount) {
		var ret = {
			normals : (flags & 3) > 0 ? new Float32Array(pointCount * 3) : undefined,
			uvs : (flags & 4) > 0 ? new Float32Array(pointCount * 2) : undefined,
			indices : new Uint16Array(elementCount * (3 - (flags & 1)))
		};
	...
	ThreeUtils.partitionCluster = function(geometries, clusterDefinitions, geometryToClusterMap) {
		var geo;
		var current = null;
		...
		for (var i = 0, len = geometries.length; i < len; ++i) {
			...
			geometryToClusterMap.set(geo.submesh, currentIndex);
