...
sap.ui.define([
	...
	SceneBuilder.prototype._addGeometryToCluster = function(bbox, points, normals, uvs, indices, isPolyline, materialId, submeshId, instanceCount) {
		var nativeSceneUD = this._vkScene.getSceneRef().userData;
		...
		var clusterType = materialId + ":" + (isPolyline ? "l" : "t") + (hasUVs ? "u" : "n") + instanceCount;
	...
	SceneBuilder.prototype.preprocessMeshes = function(meshes) {
		...
		var totalPreprocessedVertices = 0, totalPreprocessedTriangles = 0, totalPreprocessedLines = 0;

		var mesh, meshId, submeshes, si, slen, submesh, materialId, lod, geometryId;
		var pointCount, elementCount, flags;
		var isPolyline, hasUVs;
		...
		for (var mi = 0, mlen = meshes.length; mi < mlen; ++mi) {
			...
			for (si = 0, slen = submeshes.length; si < slen; ++si) {
				...
				elementCount = lod.elementCount;
				flags = lod.flags;
				if (pointCount > 0 && elementCount > 0 && flags != null) {
					...
					TotaraUtils.pushElementIntoMapArray(preprocessedClusters,
						materialId + ":" + (isPolyline ? "l" : "t") + (hasUVs ? "u" : "n") + instanceCount,
						{
							submesh: submesh.id,
							verts: pointCount,
							elems: elementCount
		...
		preprocessedClusters.forEach(function(value) {
			...
			ThreeUtils.partitionCluster(value, this._preprocessedClusterDefinitions, this._preprocessedSubmeshToClusterMap);
		}, this);

		this._isSmallScene = totalPreprocessedLods < 2000 && totalPreprocessedVertices < 1000000;
		Log.info(preprocessedClusters.size + " unique cluster types detected in " + (performance.now() - timerPreprocessBegin).toFixed(1) + "ms.  "
			+ "Scene classification: " + (this._isSmallScene ? "small.  " : "large.  ")
			...
			+ totalPreprocessedVertices.toLocaleString() + " vertices.  "
			+ totalPreprocessedTriangles.toLocaleString() + " triangles.  "
			+ totalPreprocessedLines.toLocaleString() + " lines.");
	};
	...
	SceneBuilder.prototype.insertSubmesh = function(submeshInfo, sceneBoundingbox) {
		...
		if (!boundingBox) {
			...
			var el = ThreeUtils.buildEmptyMesh(boundingBox, flags, pointCount, elementCount);
			geometryDescriptor = this._addGeometryToCluster(boundingBox, el.points, undefined, undefined, el.indices, true, submeshInfo.materialId, submeshId, instancesCount);
			...
			var r = tooSmall ? ThreeUtils.buildEmptyMesh(boundingBox, flags, pointCount, elementCount)
				: ThreeUtils.buildTemporaryMesh(boundingBox, flags, pointCount, elementCount, innerBoxesData, forceSingleCube, limitPointElementCount);
			...
			if (forceNonCluster || r.points.length > pointCount * 3 || r.indices.length > elementCount * 3) {
				...
				geometryDescriptor = this._addGeometryToCluster(boundingBox, r.points, r.normals, r.uvs, r.indices, false, submeshInfo.materialId, submeshId, instancesCount);
	...
	SceneBuilder.prototype.setGeometry = function(geoInfo) {
		...
		if (proxies) {
			...
			var nativeSceneUD = this._vkScene.getSceneRef().userData;
			proxies.forEach(function(submeshInfo) {
				var geometryDescriptor = this._addGeometryToCluster(null, data.points, data.normals, data.uvs, data.indices, geoInfo.isPolyline, submeshInfo.materialId, instanceCount);
	...
	SceneBuilder.prototype.computeMeshPriority = function(meshId, submeshBBoxes) {
