...
sap.ui.define([
	...
	var vertexInterp = function(index3, p1x, p1y, p1z, p2x, p2y, p2z, valp1, valp2, bboxMin, bboxSize) {
		if (Math.abs(isolevel - valp1) < 0.00001) {
			vertlist[index3] = bboxMin[0] + p1x * bboxSize[0];
			vertlist[index3 + 1] = bboxMin[1] + p1y * bboxSize[1];
			vertlist[index3 + 2] = bboxMin[2] + p1z * bboxSize[2];
		} else if (Math.abs(isolevel - valp2) < 0.00001) {
			vertlist[index3] = bboxMin[0] + p2x * bboxSize[0];
			vertlist[index3 + 1] = bboxMin[1] + p2y * bboxSize[1];
			vertlist[index3 + 2] = bboxMin[2] + p2z * bboxSize[2];
			...
			vertlist[index3] = bboxMin[0] + (p1x + tmpval * (p2x - p1x)) * bboxSize[0];
			vertlist[index3 + 1] = bboxMin[1] + (p1y + tmpval * (p2y - p1y)) * bboxSize[1];
			vertlist[index3 + 2] = bboxMin[2] + (p1z + tmpval * (p2z - p1z)) * bboxSize[2];
		}
	};
			...
			for (y = -1; y <= dim1; ++y) {
				for (x = -1; x <= dim1; ++x) {
					...
					if (edgeTable[cubeIndex] & 1) {
						vertexInterp(0, modX, modY, modZ, modX1, modY, modZ, c000, c100, bbox, bboxSize);
					}
					if (edgeTable[cubeIndex] & 2) {
						vertexInterp(3, modX1, modY, modZ, modX1, modY, modZ1, c100, c101, bbox, bboxSize);
					}
					if (edgeTable[cubeIndex] & 4) {
						vertexInterp(6, modX1, modY, modZ1, modX, modY, modZ1, c101, c001, bbox, bboxSize);
					}
					if (edgeTable[cubeIndex] & 8) {
						vertexInterp(9, modX, modY, modZ1, modX, modY, modZ, c001, c000, bbox, bboxSize);
					}
					if (edgeTable[cubeIndex] & 16) {
						vertexInterp(12, modX, modY1, modZ, modX1, modY1, modZ, c010, c110, bbox, bboxSize);
					}
					if (edgeTable[cubeIndex] & 32) {
						vertexInterp(15, modX1, modY1, modZ, modX1, modY1, modZ1, c110, c111, bbox, bboxSize);
					}
					if (edgeTable[cubeIndex] & 64) {
						vertexInterp(18, modX1, modY1, modZ1, modX, modY1, modZ1, c111, c011, bbox, bboxSize);
					}
					if (edgeTable[cubeIndex] & 128) {
						vertexInterp(21, modX, modY1, modZ1, modX, modY1, modZ, c011, c010, bbox, bboxSize);
					}
					if (edgeTable[cubeIndex] & 256) {
						vertexInterp(24, modX, modY, modZ, modX, modY1, modZ, c000, c010, bbox, bboxSize);
					}
					if (edgeTable[cubeIndex] & 512) {
						vertexInterp(27, modX1, modY, modZ, modX1, modY1, modZ, c100, c110, bbox, bboxSize);
					}
					if (edgeTable[cubeIndex] & 1024) {
						vertexInterp(30, modX1, modY, modZ1, modX1, modY1, modZ1, c101, c111, bbox, bboxSize);
					}
					if (edgeTable[cubeIndex] & 2048) {
						vertexInterp(33, modX, modY, modZ1, modX, modY1, modZ1, c001, c011, bbox, bboxSize);
					}
