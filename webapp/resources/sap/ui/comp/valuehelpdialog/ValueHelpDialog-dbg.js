...
sap.ui.define([
	...
	ValueHelpDialog.prototype._initializeTable = function() {
		...
		var oTableSelectionInstance, bIsSelectionPluginAvailable = false, oTableTitle, oTableToolbar;
		if (this._oTable) {
			oTableSelectionInstance = this._oTable;
			if (this._isMultiSelectionPluginAvailable()) {
				oTableSelectionInstance = this._oTable._getSelectionPlugin();
		...
		if (this._oTable && this._oTable.isA("sap.ui.table.Table")) {
			...
			oTableToolbar = new Toolbar();
			oTableToolbar.addContent(oTableTitle);
						...
						if (Array.isArray(vTemplate)) {
							...
							oColumnMenu = new ColumnMenu({
								...
								beforeOpen: function (oEvent) {
									...
									aQuickActions = this._getAllEffectiveQuickActions();

									if (aQuickActions.length){
										aQuickActions[0].setVisible(false);
									}
	...
	ValueHelpDialog.prototype.onAfterRendering = function() {
		var iButtonOkWidth = this._oButtonOk.getDomRef() && this._oButtonOk.getDomRef().offsetWidth,
			iButtonCancelWidth = this._oButtonCancel.getDomRef() && this._oButtonCancel.getDomRef().offsetWidth;
		...
		if (iButtonOkWidth && iButtonCancelWidth) {
			if (iButtonCancelWidth > iButtonOkWidth) {
				this._oButtonOk.setWidth(iButtonCancelWidth + "px");
			} else {
				this._oButtonCancel.setWidth(iButtonOkWidth + "px");
			}
		}
