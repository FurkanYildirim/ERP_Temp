...
sap.ui.define([
	...
	/**
	 * Determines the logical owner of the CopyProvider plugin.
	 * @private
	 */
	SmartTable.prototype.getCopyProviderPluginOwner = function() {
		return this._oTable || this._oTableReady.promise;
	};

	SmartTable.prototype.setCopyProvider = function(oCopyProvider) {
		this.setAggregation("copyProvider", oCopyProvider, true);
		oCopyProvider && this._bToolbarInsertedIntoItems && this._createToolbarContent();
		return this;
	};
	...
	SmartTable.prototype._createExportColumnConfiguration = function() {
		var aColumns, i, iLen, oColumn, aSheetColumns = [], bUtcDefault, vUseUTCDateTime;
		...
		for (i = 0; i < iLen; i++) {
			...
			if (oColumn.getVisible()) {
				var oExportColumn = this._getExportColumnConfiguration(oColumn, bUtcDefault);
	...
	SmartTable.prototype._getExportColumnConfiguration = function(oColumn, bUtcDefault) {
		var nWidth, oColumnDataType;
		var oColumnP13nData = oColumn.data("p13nData");
		var oExportColumn = {};
		oExportColumn.bUtc = bUtcDefault;

		var fnReplace = function(match, group) {
			return '{' + (parseInt(group) - 1) + '}';
		};
		oExportColumn.sPath = this._getColumnLeadingProperty(oColumn, oColumnP13nData);

		if (Array.isArray(oExportColumn.sPath)) {
			oExportColumn.sPath = oExportColumn.sPath[0];
		}

		if (oExportColumn.sPath) {
			oExportColumn.sLabel = this._getColumnLabel(oColumn);
			nWidth = oColumn.getWidth().toLowerCase() || oColumnP13nData.width || "";
			oExportColumn.nWidth = this._getColumnWidthNumber(nWidth);
			oExportColumn.sType = oColumnP13nData.type === "numeric" ? "Number" : oColumnP13nData.type;
			oColumnDataType = oColumnP13nData.typeInstance;

			if (oColumnP13nData.unit) {
				oExportColumn.sType = oColumnP13nData.isCurrency ? "Currency" : "Number";
				...
				oExportColumn.sTemplate = oFormatter.getTemplate().replace(/\$(\d)/g, fnReplace);
				oExportColumn.inputFormat = oFormatter.getRegExPattern().toString().slice(1, -1);
				oExportColumn.sType = "String";

			} else if (oColumnP13nData.sType != "date" && ODataType.isDateOrTime(oColumnP13nData.edmType)) {

				// set type as expected by excel for OData specific Date/Time fields
				oExportColumn.sType = ODataType.getTypeName(oColumnP13nData.edmType);
				if (oExportColumn.sType === "DateTimeOffset") {
					oExportColumn.sType = "DateTime";
					oExportColumn.bUtc = false;
					if (oColumnP13nData.timezone) {
						oExportColumn.timezone = oColumnP13nData.timezone;
					} else if (oColumnP13nData.timezone === undefined) {
						oExportColumn.timezone = oColumnP13nData.additionalProperty;
					}
				}

			} else if (oExportColumn.sType === "stringdate") {
				oExportColumn.sType = "Date";
				oExportColumn.inputFormat = "YYYYMMDD";
			} else if (oExportColumn.sType === "boolean") {
				oExportColumn.sType = "Boolean";
				oExportColumn.oType = ODataType.getType(oColumnP13nData.edmType);
			} else if (oColumnP13nData.description) {
				...
				oExportColumn.sType = "String";
				oExportColumn.sTemplate = FormatUtil.getFormattedExpressionFromDisplayBehaviour(oColumnP13nData.displayBehaviour, "{0}", "{1}");

			} else if (oColumnP13nData.isDigitSequence) {
				oExportColumn.sType = "Number";
			} else if (oExportColumn.sType === "string") {
				oExportColumn.sType = "String";
			} else if (oExportColumn.sType === "date") {
				oExportColumn.sType = "Date";
			} else if (oExportColumn.sType === "time") {
				oExportColumn.sType = "Time";
			}

			if (oColumnP13nData.description) {
				if (oColumnP13nData.displayBehaviour === "descriptionOnly") {
					oExportColumn.property = oColumnP13nData.description;
				} else if (oColumnP13nData.displayBehaviour === "idOnly") {
					oExportColumn.property = oExportColumn.sPath;
				} else {
					oExportColumn.property = [oExportColumn.sPath, oColumnP13nData.description];
				}
			} else {
				oExportColumn.property = oExportColumn.sPath;
			}

			var oColumnConfiguration = {
				columnId: oColumn.getId(),
				property: oExportColumn.property,
				type: oExportColumn.sType,
				inputFormat: oExportColumn.inputFormat,
				label: oExportColumn.sLabel ? oExportColumn.sLabel : oExportColumn.sPath,
				width: oExportColumn.nWidth,
				textAlign: oColumn.getHAlign(),
				template: oExportColumn.sTemplate,
				trueValue: (oExportColumn.sType === "Boolean" && oExportColumn.oType) ? oExportColumn.oType.formatValue(true, "string") : undefined,
				falseValue: (oExportColumn.sType === "Boolean" && oExportColumn.oType) ? oExportColumn.oType.formatValue(false, "string") : undefined,
				unitProperty: (oExportColumn.sType === "Currency" || oExportColumn.sType === "Number") ? oColumnP13nData.unit : undefined,
				displayUnit: oExportColumn.sType === "Currency" ? true : undefined,
				precision: oColumnP13nData.precision,
				scale: oColumnP13nData.scale,
				autoScale: oExportColumn.sType === "Number",
				utc: (oExportColumn.sType === "DateTime" && oExportColumn.bUtc != null)  ? oExportColumn.bUtc : undefined,
				timezoneProperty: oExportColumn.timezone ? oExportColumn.timezone : undefined
			};

			return oColumnConfiguration;
		}
	...
	SmartTable.prototype._updateQuickSort = function(oColumn) {
		...
		if (oColumnInfo.unit) {
			...
			if (oAdditionalPropertyColumn && this._getColumnSortProperty(oColumn)) {
				this._oQuickSort.addItem(new QuickSortItem({
	...
	SmartTable.prototype._processDateFiltersForVariant = function (oCurrentVariant) {
		...
		var aDateFields = PersonalizationUtil.getColumnKeysOfType("date", this._oPersController._oColumnHelper.getColumnMap()),
			aTimeFields = PersonalizationUtil.getColumnKeysOfType("time", this._oPersController._oColumnHelper.getColumnMap());
