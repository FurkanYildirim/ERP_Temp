...
sap.ui.define([
	...
	ODataControlFactory.prototype._createEdmString = function(oSettings) {
		...
		if (MetadataAnalyser.getIsTimezoneProperty(oEdmProperty)) {
			mAttributes.value = {
                parts :  [{value : null}, {path : this._oMetaData.path}],
                type : this._oTypes.getType(this._oMetaData.property, {showDate: false, showTime: false})
			};
	...
	ODataControlFactory.prototype.fetchIDAndDescriptionCollectionIfRequired = function(oSettings){
		var oValueListProvider = this.getValueListProvider(),
			bIsFixedValues = this._isFixedList();

		if (oValueListProvider && bIsFixedValues && oSettings.mode === "edit") {
				oValueListProvider._updateDescriptionIfRequired(oSettings);
