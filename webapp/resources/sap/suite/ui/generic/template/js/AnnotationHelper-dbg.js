...
], function (AnnotationHelperModel, AnnotationHelperModelBasics, SmartField, FeLogger, formatMessage, deepExtend, RuntimeFormatters, StableIdHelper, testableHelper, Device,
...
*/
		...
		_addFullPathToImageUrlIfNeeded: function (sImageUrl, sAppComponentName, bSuppressIcons) {
			if (!sImageUrl) {
				return "";
			}
			if (bSuppressIcons && (sImageUrl.substring(0, 11) === "sap-icon://")) {
				return "";
			}
		...
		// is called from LightBoxItem Control, which actually supress the visibility of the
		// LightBoxItem if the path resolves to a icon.
		...
		formatImageUrl: function (oInterface, oImageUrl, sAppComponentName, bExpand, bSuppressIcons) {
			...
			if (oImageUrl && oImageUrl.Path) {
				if (bSuppressIcons) {
					return "{parts: [{path: '" + oImageUrl.Path + "'}, {path: '_templPrivGlobal>/generic/appComponentName'}, {value:" + bSuppressIcons + "}], formatter: 'sap.suite.ui.generic.template.js.AnnotationHelper.formatImageUrlRuntime'}";
				} else {
			...
			} else if (bSuppressIcons && oImageUrl && oImageUrl.String.indexOf("sap-icon://") > -1) {
				return "";
		...
		formatImageUrlRuntime: function (sImageUrl, sAppComponentName, bSuppressIcons) {
			return oAnnotationHelper._addFullPathToImageUrlIfNeeded(sImageUrl, sAppComponentName, bSuppressIcons);
		},
		...
		getExtensionPointFacetTitle: function (oInterface, sManifestKey, oManifestExtend, sLabel) {
			...
			return (oExtension && oExtension['sap.ui.generic.app'] && oExtension['sap.ui.generic.app'].title) || sap.ui.model.odata.AnnotationHelper.format(oInterface, sLabel);
		},
		...
		getStringifiedCreatablePath: function (oInterface, oSourceEntitySet, oRelatedEntitySet) {
			return JSON.stringify(oAnnotationHelper.getCreatablePath(oInterface, oSourceEntitySet, oRelatedEntitySet));
		},
	...
	oAnnotationHelper.getStringifiedCreatablePath.requiresIContext = true;
