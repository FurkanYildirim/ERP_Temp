sap.ui.define(["sap/ui/base/Object",
	...
	"sap/ui/model/odata/ODataUtils"
			...
			testableHelper, FeLogger, odata4analytics, extend, deepExtend, FeError, StableIdHelper, ControlStateWrapperFactory, oDataModelHelper, IconPool, ExportFilter, ODataUtils) {
	"use strict";
	...
	function getMethods(oController, oServices, oComponentUtils) {
		...
		*/
		...
		function fnShouldDeleteButtonGetEnabled (oModel, aContexts, oControl) {
			...
			return aContexts.some(function(oContext) {
				 ...
				 *	3. we do not have a deletable path that disallows the deletion of that object
				 */
		...
		*/
					...
					switch (oDatafield.type) {
						case "Edm.Guid":
							if (oResponse.DraftAdministrativeData && oResponse.DraftAdministrativeData.DraftIsCreatedByMe) {
								...
								sRoute += sKeySeparator + oDatafield.name + "=" + "guid'" + oResponse[oDatafield.name] + "'";
							}
							break;
						case "Edm.Boolean":
							if (oResponse.DraftAdministrativeData && oResponse.DraftAdministrativeData.DraftIsCreatedByMe) {
								...
								sRoute += sKeySeparator + oDatafield.name + "=" + oResponse[oDatafield.name];
							}
							break;
						case "Edm.Decimal":
						case "Edm.Int16":
						...
						case "Edm.SByte":
							var sFormattedvalue = ODataUtils.formatValue(oResponse[oDatafield.name], oDatafield.type);
							sRoute += sKeySeparator + oDatafield.name + "=" + sFormattedvalue;
							break;
						default:
							if (typeof oResponse[oDatafield.name] === "string") {
								sRoute += sKeySeparator + oDatafield.name + "=" + "'" + oResponse[oDatafield.name] + "'";
							} else {
								sRoute += sKeySeparator + oDatafield.name + "=" + oResponse[oDatafield.name];
							}
		...
		function fnTransformTechnicalPropsOnExportedFile (aFilterSettings) {
			...
			var oEditStateControl = oController.getView().byId("editStateFilter");
			...
			aTechnicalProps.forEach(function (oTechnialProp){
				...
				var iIndexOfFilterSettingsToBeRemoved = aFilterSettings.findIndex(function (oFilterSettings){
					return oTechnialProp.property === oFilterSettings.property &&
						oTechnialProp.operator === oFilterSettings.rawValues[0].operator &&
						oTechnialProp.value === oFilterSettings.rawValues[0].value &&
			...
			//Create a new filter with the value retrevied from "editStateFilter" select control
		...
		function fnIncludeEntitySetParametersToExportedFile (oSmartTable, aFilterSettings) {
			...
			var oSmartFilterBar = oController.getView().byId(sSmartFilterId);
				...
				oServices.oDataLossHandler.performIfNoDataLoss(function() {
					oServices.oCRUDManager.callAction({
						...
						if (aResponses && aResponses.length > 0) {
							...
							var bSetToDirty = oResponseContext && (!oResponse.actionContext || oResponseContext.getPath() !== oResponse.actionContext.getPath());
							if (bSetToDirty) {
