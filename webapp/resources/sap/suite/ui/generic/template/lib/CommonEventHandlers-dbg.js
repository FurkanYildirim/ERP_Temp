sap.ui.define(["sap/ui/base/Object",
	...
	Controller, FeLogger, MessageUtils, extend, isEmptyObject, deepExtend, CRUDHelper, XMLView, FormatUtil, FeError, FileUploaderParameter, IconPool, ServiceContainer) {
	"use strict";
	...
	function getMethods(oController, oComponentUtils, oServices, oCommonUtils) {
		...
		function getPaginatorInfo(oTable, oRow, oState){
			...
			var fnPaginate = function(oContext, oViewProxy, bIsEditItem){
				var fnPaginateImpl = function(oTargetContext){
					...
					oCurrentPaginatorInfo = oRet; // make sure that the navigation below does not create a new paginator info but resuses the existing one
		...
		*/
		...
		function fnOnSemanticObjectLinkNavigationTargetObtained(sEntitySet, mFieldSemanticObjects, oEventParameters, oState, sTitle) {
			...
			renderQuickViewFragment.call(this); // To be removed when we consume the Teams Contacts Collaboration Helper
		...
		function onListNavigate(oEventOrSource, oState, oBindingContext, bIsProgrammatic, oViewProxy) {
			...
			/* When this method is called from LR's onListNavigate event handler, a copy of the original event is passed here, which
			   would not be of Event object type and thus, we need to have an additional check by checking its metadata's name to make
			   sure whether the passed object is of Event type. */
