		...
		function getMethods(oController, oComponentUtils, oServices, oCommonUtils, oBusyHelper) {
			...
			function callAction(mParameters, oState) {
				...
				if (sFunctionImportReturnType === sEntityType) {
					var oAppComponent = oComponent.getAppComponent();
					var oInfoForContentIdPromise = CacheHelper.getInfoForContentIdPromise(sEntitySet, oModel, oAppComponent.getId(), oComponentUtils.getRootExpand);
					return oInfoForContentIdPromise.then(function(oInfoObject) {
			...
			function addEntry(oTable, oPredefinedValues) {
				...
				return new Promise(function(fnResolve, fnReject) {
					...
					var getRootExpand = oTargetComponentUtils && oTargetComponentUtils.getRootExpand;
					...
					oInfoForContentIdPromise.then(function(oInfoObject) {
						...
						if (oComponentUtils.getViewLevel() === 0){           // Load main object page (if not already done), as we will navigate there
