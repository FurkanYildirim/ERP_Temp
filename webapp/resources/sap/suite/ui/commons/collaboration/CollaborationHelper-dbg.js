		...
		CollaborationHelper.processAndExpandHash = function () {
			...
			return this._getCurrentUrl().then(function (sCurrentUrl) {
				...
				if (sCurrentUrl.indexOf(URL_KEY) > -1) {
					var sHash;
					if (sCurrentUrl.indexOf(PARAM_SAP_CARD_TITLE) > 0){
						sHash = sCurrentUrl.split(URL_KEY)[1].split('=')[1].split(PARAM_SAP_CARD_TITLE)[0];
					} else {
						sHash = sCurrentUrl.split(URL_KEY)[1].split('=')[1];
					}
					return this._retrieveURL(sHash).then(function (oAppState) {
						if (oAppState.getData()) {
							...
							var sUrl = sCurrentUrl.split(URL_KEY)[0];
							window.location.replace(sUrl);
						}
						return Promise.resolve();
					});
				} else if (sCurrentUrl.indexOf(PARAM_SAP_CARD_TITLE) > -1) {
					var sUrl = sCurrentUrl.split(PARAM_SAP_CARD_TITLE)[0];
					window.location.replace(sUrl);
				}
				return Promise.resolve();
			}.bind(this));
