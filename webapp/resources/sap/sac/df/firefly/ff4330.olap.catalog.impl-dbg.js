...
{
	var listener = new oFF.OlapFileRequestListener2();
	listener.callback = callback;
	return listener;
};
		...
		while (iterator.hasNext())
		{
			...
			if (oFF.OlapFileUtils.isValidModelName(item.getName()))
			{
				...
				if (isDWCSystem)
				{
					...
					{
						objectAttribute.put(oFF.XFileAttribute.ICON.getName(), olapFs.getIconByName("CUBE"));
					}
					else if (oFF.XString.isEqual(item.getType().getName(), "inamodel"))
					{
						objectAttribute.put(oFF.XFileAttribute.ICON.getName(), olapFs.getIconByName("DATASET"));
					}
					else if (item.isComponentNode())
					{
						objectAttribute.put(oFF.XFileAttribute.ICON.getName(), olapFs.getIconByName("SPACE"));
					}
				}
...
{
	var syncAction = oFF.OlapFileFetchAction.createAndRun(syncType, olapFile, this, oFF.OlapFileRequestListener.create( function(fileRequestAction){
		var data = fileRequestAction.getData();
		if (oFF.notNull(listener))
		{
			var proxy2 = oFF.SyncActionProxy.create(fileRequestAction);
			listener.onFileFetchMetadata(proxy2, data, data.getCachedMetadata(), customIdentifier);
		}
...
{
	this.m_iconMap = oFF.XLinkedHashMapOfStringByString.create();
	var prefix = "sap-icon://fpa/";
	var prefixNoFpa = "sap-icon://";
...
oFF.OlapFileFetchAction.prototype.processTargetUri = function(syncType, olapDriver)
{
	...
	try
	{
		if (this.m_metaDataOnly)
		{
			...
			{
				var metadata = this.getFolderMetaData(fileName, false);
		...
		else
		{
			...
			{
				var allSchemaObjects = olapDriver.getAllSchemas(this.m_FileQueryConfig);
...
oFF.OlapFileFetchAction.prototype.setChildFiles = function(olapDriver, allObjects, enableSearch)
{
	...
	while (objectTypeIterator.hasNext())
	{
		...
		{
			olapAttr.put(oFF.XFileAttribute.ICON.getName(), this.m_ofs.getIconByName("SPACE"));
		}
		var nextObject = objectTypeIterator.next();
		if (oFF.notNull(nextObject) && oFF.XString.size(nextObject.getName()) > 0)
		{
			var objectName = oFF.XString.toLowerCase(nextObject.getName());
			if (oFF.OlapFileUtils.isValidSchemaName(objectName))
			{
				var childFolderUri = oFF.XUri.createChildFile(this.m_file.getTargetUri(), objectName);
				var childFolder = oFF.OlapFile.createFile(this.getProcess(), this.m_ofs, childFolderUri, olapAttr);
				var metadata = this.getFolderMetaData(objectName, enableSearch);
...
oFF.OlapFileFetchAction.prototype.getAndSetObjectsFolders = function(olapDriver)
{
	...
	var schemaFolderMetadata = this.getFolderMetaData("Schema", false);
	...
	var typeFolderMetadata = this.getFolderMetaData("Type", false);
...
oFF.OlapFileFetchAction.prototype.getFolderMetaData = function(displayName, enableSearch)
{
	...
	typeFolderMetadata.putBoolean(oFF.XFileAttribute.SUPPORTS_SEARCH.getName(), enableSearch);
...
{
	var olapDriver = this.getOlapCatalogFileSystemDriverByHost();
	var childFiles = oFF.OlapFileUtils.processCatalogResult(result, this.getProcess(), this.m_ofs, this.m_file, olapDriver.isDWCSystem());
	var targets = oFF.XList.create();
	if (oFF.notNull(childFiles))
	{
		for (var i = 0; i < childFiles.size(); i++)
		{
...
oFF.OlapFile.prototype = new oFF.DfXFileProviderNonBlocking();
...
oFF.OlapFile.prototype.processProviderDelete = oFF.noSupport;
...
{
	var sync = this.m_fs.processFetchChildren(this, syncType, listener, customIdentifier, config);
