...
oFF.QInAValueUtils = {
	...
	_importValueInternal:function(importer, valueBag, inaElement, parameterName, valueType, field, returnValue)
	{
		...
		var applyFieldValue = !returnValue && oFF.notNull(field) && useFieldLiteralValue;
		var valueType2 = applyFieldValue ? field.getValueType() : valueType;
...
oFF.QInARepoOptions = {
	...
	importUniqueAxisProperties:function(inaResultStructureFeature, provider)
	{
		...
		{
			var inaUniqueAxisProperties = inaResultStructureFeature.getStructureByKey("UniqueAxisProperties");
			if (oFF.notNull(inaUniqueAxisProperties) && inaUniqueAxisProperties.hasElements())
			{
				...
				var uniqueAxisProperties = provider.getQueryModel().getReturnedUniqueAxisProperties();
				var inaElementNames = inaUniqueAxisProperties.getKeysAsReadOnlyListOfString();
				var size = inaElementNames.size();
				for (var i = 0; i < size; i++)
				{
					var inaKey = inaElementNames.get(i);
	...
	{
			oFF.XCollectionUtils.forEachString(structure.getKeysAsReadOnlyListOfString(),  function(inaKey){
			var dataSelection = oFF.QInAConverter.lookupReturnedDataSelection(inaKey);
			if (oFF.notNull(dataSelection))
			{
				uniqueAxisProperties.toggleReturnedDataSelection(axisType, dataSelection, structure.getBooleanByKeyExt(dataSelection.getName(), false));
			}
		}.bind(this));
	},
	...
	{
			var queryModel = provider.getQueryModel();
		var returnedAxisProperties = oFF.notNull(queryModel) && queryModel.getModelCapabilities().supportsUniqueAxisProperties() ? queryModel.getReturnedUniqueAxisProperties() : null;
		var rowProperties = oFF.isNull(returnedAxisProperties) ? null : returnedAxisProperties.getRowProperties();
		var columnProperties = oFF.isNull(returnedAxisProperties) ? null : returnedAxisProperties.getColumnProperties();
		var hasRowProperties = oFF.XCollectionUtils.hasElements(rowProperties);
		var hasColumnProperties = oFF.XCollectionUtils.hasElements(columnProperties);
		if (hasRowProperties || hasColumnProperties)
		{
			var inaUniqueAxisProperties = inaOptions.putNewStructure("UniqueAxisProperties");
			if (hasRowProperties)
			{
				var rowStructure = inaUniqueAxisProperties.putNewStructure("Rows");
				oFF.XCollectionUtils.forEach(rowProperties.getKeysAsReadOnlyList(),  function(rp){
					rowStructure.putBoolean(oFF.QInAConverter.lookupReturnedDataSelectionInA(rp), rowProperties.getByKey(rp).getBoolean());
				}.bind(this));
			}
			if (hasColumnProperties)
			{
				var columnStructure = inaUniqueAxisProperties.putNewStructure("Columns");
				oFF.XCollectionUtils.forEach(columnProperties.getKeysAsReadOnlyList(),  function(cp){
					columnStructure.putBoolean(oFF.QInAConverter.lookupReturnedDataSelectionInA(cp), columnProperties.getByKey(cp).getBoolean());
				}.bind(this));
			}
		}
	},
...
oFF.QInARepoQueryLegacy.prototype.importComponentWithStructure = function(importer, inaStructure, modelComponent, parentComponent, context)
{
	...
	if (oFF.notNull(inaDefinition))
	{
		...
		var isInaRepoMode = importer.mode.isTypeOf(oFF.QModelFormat.INA_REPOSITORY);
...
oFF.QInARepoQueryLegacy.prototype.getInactiveCapabilities = function(queryModel)
{
	...
	var allCapabilities = oFF.FeatureToggle.getAllFeatureToggles();
...
oFF.QInARepoBlendableQueryManager.importQueryModelLinkSettingStructure = function(inaQueryModelLinkSetting, blendableQueryManager, secondaryDatasetId)
{
	...
	blendableQueryManager.setUseEmptyAllMemberMeasureFilterByDatasetId(inaQueryModelLinkSetting.getBooleanByKey("UseAllMemberFilter"), secondaryDatasetId);
...
{
	var inaQueryModelLinkSetting = inaQueryModelLinkSettings.addNewStructure();
...
oFF.QInARepoBlendableQueryManager.exportQueryModelLinkSettings = function(blendableQueryManager, primaryDatasetId, secondaryDatasetId, inaQueryModelLinkSetting)
{
	...
	inaQueryModelLinkSetting.putBoolean("UseAllMemberFilter", blendableQueryManager.isUseEmptyAllMemberMeasureFilterByDatasetId(secondaryDatasetId));
	var inaActiveLinkDimensionNames = inaQueryModelLinkSetting.putNewList("ActiveDimensionLinks");
	var activeLinkDimensionNamesIter = blendableQueryManager.getActivePrimaryLinkDimensionNamesSettings(secondaryDatasetId).getIterator();
...
oFF.QInARepoBlendableQueryManager.prototype.importComponentWithStructure = function(importer, inaStructure, modelComponent, parentComponent, context)
{
	...
	var inaQueryModelLinkSettingsList = inaStructure.getListByKey("QueryModelLinkSettingsList");
	var inaQueryManagerItem;
	var inaQueryModelLinkSettings;
	var inaQueryManagersIter;
	var secondaryDatasetId;
	if (oFF.notNull(inaQueryModelLinkSettingsList))
	{
		var inaQueryModelLinkSettingsIter = inaQueryModelLinkSettingsList.getIterator();
		while (inaQueryModelLinkSettingsIter.hasNext())
		{
			...
			while (inaQueryManagersIter.hasNext())
			{
				...
				{
					this.importSecondaryModel(importer, blendableQueryManager, inaQueryModelLinkSettings, inaQueryManagerItem, inaQueryModelLinkSettings.getStringByKey("SecondModelName"));
					break;
				}
			}
		}
	...
	{
		var inaQueryModelLinkSettingsMap = inaStructure.getStructureByKey("QueryModelLinkSettings");
		if (oFF.notNull(inaSecondaryQueryManagers))
		{
			...
			while (inaQueryManagersIter.hasNext())
			{
				...
				this.importSecondaryModel(importer, blendableQueryManager, inaQueryModelLinkSettings, inaQueryManagerItem, secondaryDatasetId);
			}
		}
	}
...
oFF.QInARepoBlendableQueryManager.prototype.importSecondaryModel = function(importer, blendableQueryManager, inaQueryModelLinkSettings, inaQueryManagerItem, secondaryDatasetId)
{
...
oFF.QInARepoBlendableQueryManager.prototype.exportComponentWithStructure = function(exporter, modelComponent, inaStructure, flags)
{
	...
	var inaQueryModelLinkSettings = inaStructure.putNewList("QueryModelLinkSettingsList");
	var queryModelLinkSettingsIter = blendableQueryManager.getAllQueryModelLinkSettings().getIterator();
	while (queryModelLinkSettingsIter.hasNext())
	{
		var queryModelLinkSettings = queryModelLinkSettingsIter.next();
		var secondaryDatasetId = queryModelLinkSettings.getSecondDatasetId();
		var secondaryQueryManager = blendableQueryManager.getQueryManagerByDatasetId(secondaryDatasetId);
...
{
	if (exporter.mode !== oFF.QModelFormat.INA_REPOSITORY_DELTA || oFF.QInARepoDimension.exportResultSetAttributeDetails(dimension))
	{
...
oFF.QInARepoDimension.prototype.importComponentWithStructure = function(importer, inaStructure, modelComponent, parentComponent, context)
{
	...
	if (oFF.notNull(dimension))
	{
		...
		var displayGroup = inaStructure.getStringByKey("DisplayGroup");
		if (oFF.XStringUtils.isNotNullAndNotEmpty(displayGroup))
		{
			dimension.setDisplayGroup(displayGroup);
		}
	}
	return dimension;
};
...
oFF.QInARepoDimension.prototype.exportComponentWithStructure = function(exporter, modelComponent, inaStructure, flags)
{
	...
	if (exporter.getOriginalMode() === oFF.QModelFormat.INA_CLONE)
	{
		inaStructure.putStringNotNull("DisplayGroup", dimension.getDisplayGroup());
	}
	return oFF.QInARepository.prototype.exportComponentWithStructure.call( this , exporter, modelComponent, inaStructure, flags);
};
...
oFF.QInARepoFormulaCalculatedDimension.importCalculatedDimensionProperties = function(importer, inaStructure, context, dimension)
{
	...
	var clientDefaultField = dimension.getFieldByName(inaStructure.getStringByKey("ClientDefaultTextAttribute"));
	dimension.setClientDefaultTextField(clientDefaultField);
	...
	if (importer.mode !== oFF.QModelFormat.INA_REPOSITORY_DELTA || inaStructure.containsKey("FieldLayoutType"))
	{
		var fieldLayoutType = oFF.QInAConverter.lookupFieldLayoutType(inaStructure.getStringByKey("FieldLayoutType"));
		dimension.setFieldLayoutType(fieldLayoutType);
	}
...
oFF.QInARepoFormulaException.prototype.importComponentWithStructure = function(importer, inaStructure, modelComponent, parentComponent, context)
{
	...
	if (importer.mode !== oFF.QModelFormat.INA_REPOSITORY_DELTA || inaStructure.containsKey("HandleExcludedFilters"))
	{
		formulaException.setHandleExcludedFilters(inaStructure.getBooleanByKey("HandleExcludedFilters"));
	}
	if (importer.mode !== oFF.QModelFormat.INA_REPOSITORY_DELTA || inaStructure.containsKey("CalculateRatioVariance"))
	{
	...
	if (importer.mode !== oFF.QModelFormat.INA_REPOSITORY_DELTA || inaStructure.containsKey("EvaluateDefault"))
	{
		var evalType = oFF.QExceptionEvalType.lookupExceptionEvalType(inaStructure.getStringByKey("EvaluateDefault"));
	...
	{
		var structureStructure = structureContext.getStructureAt(3);
		var structureDimName = this.getDimensionName(queryModel, structureStructure);
		var structureMemberName = structureStructure.getStringByKey("MemberName");
		if (oFF.notNull(structureDimName) && oFF.notNull(structureMemberName))
		{
			var structureDim = queryModel.getDimensionByNameFromExistingMetadata(structureDimName);
			var structureMember = structureDim.getStructureMemberByAliasOrMember(structureMemberName);
...
oFF.QInARepoFormulaException.prototype.exportComponentWithStructure = function(exporter, modelComponent, inaStructure, flags)
{
	...
	if ((exporter.mode !== oFF.QModelFormat.INA_REPOSITORY_DELTA || formulaException.hasPropertyChanged(oFF.QFormulaExceptionProperties.QY_HANDLE_EXCLUDED_FILTERS)) && formulaException.isHandleExcludedFilters())
	{
		inaStructure.putBoolean("HandleExcludedFilters", formulaException.isHandleExcludedFilters());
	}
	if ((exporter.mode !== oFF.QModelFormat.INA_REPOSITORY_DELTA || formulaException.hasPropertyChanged(oFF.QFormulaExceptionProperties.QY_CALCULATE_RATIO_VARIANCE)) && formulaException.isRatioVarianceCalculated())
	{
	...
	if (exporter.mode !== oFF.QModelFormat.INA_REPOSITORY_DELTA || formulaException.hasPropertyChanged(oFF.QExceptionProperties.QY_ACTIVE))
	{
	...
	var structureMember = formulaException.getStructure();
	if (structureMember.isPresent())
	{
		var structureMemberStruct = structureContext.addNewStructure();
		var structureDim = structureMember.get().getDimension();
...
oFF.QInARepoMembersAll.prototype.exportComponentWithStructure = function(exporter, modelComponent, inaStructure, flags)
{
	...
	if (oFF.notNull(allStructureMembers))
	{
		...
		for (var i = 0; i < len; i++)
		{
			...
			var isKeyRefSerializationEnabled = dimension.getQueryManager() !== null && dimension.getQueryManager().isKeyRefSerializationEnabled();
			var isSharedObject = oFF.notNull(olapEnv) && olapEnv.getClientQueryObjectManager().getClientQueryObjectIncludeReleasedByKeyRef(structureMember, dimension.getQueryManager().getClientQueryObjectStorageName()) !== null;
			...
			if (shouldContinueExporting)
			{
				...
				if (isSharedObject)
				{
					var copiedMeasure = oFF.QStructureMember.createMeasure(structureMember.getMemberType(), dimension.getQueryModel(), dimension);
...
oFF.QInARepoQuery.prototype.importComponentWithStructure = function(importer, inaStructure, modelComponent, parentComponent, context)
{
	...
	if (oFF.notNull(inaDefinition))
	{
		...
		if (importer.mode.isTypeOf(oFF.QModelFormat.INA_REPOSITORY) && queryModel.isBlendingModel())
		{
			this.importBlendingSources(importer, inaStructure, queryModel);
		}
...
oFF.QInARepoTableCellReferenceScope.prototype.importComponentWithStructure = function(importer, inaStructure, modelComponent, parentComponent, context)
{
	if (oFF.notNull(modelComponent) && oFF.notNull(inaStructure) && modelComponent.getComponentType() === oFF.OlapComponentType.VISUALIZATION_CELL_REFERENCE_SCOPE)
	{
		...
		cellReferenceScope.setMatchOrdinal(inaStructure.getIntegerByKeyExt("MatchOrdinal", 0));
		cellReferenceScope.setMatchModulo(inaStructure.getIntegerByKeyExt("MatchModulo", 0));
		cellReferenceScope.setMatchSkipFirst(inaStructure.getIntegerByKeyExt("MatchSkipFirst", 0));
		cellReferenceScope.setMatchSkipLast(inaStructure.getIntegerByKeyExt("MatchSkipLast", 0));
		cellReferenceScope.setMatchFullHeaderSection(inaStructure.getBooleanByKeyExt("MatchHeaderSectionFull", false));
		cellReferenceScope.setMatchHeaderSectionStart(inaStructure.getBooleanByKeyExt("MatchHeaderSectionStart", false));
		cellReferenceScope.setMatchHeaderSectionEnd(inaStructure.getBooleanByKeyExt("MatchHeaderSectionEnd", false));
		cellReferenceScope.setMatchHeaderFieldsSectionEnd(inaStructure.getBooleanByKeyExt("MatchHeaderFieldsSectionEnd", false));
		cellReferenceScope.setMatchFullDataSection(inaStructure.getBooleanByKeyExt("MatchDataSectionFull", false));
		cellReferenceScope.setMatchDataSectionStart(inaStructure.getBooleanByKeyExt("MatchDataSectionStart", false));
		cellReferenceScope.setMatchDataSectionEnd(inaStructure.getBooleanByKeyExt("MatchDataSectionEnd", false));
		...
		for (var i = 0; i < axisPathList.size(); i++)
		{
			...
			if (oFF.XCollectionUtils.hasElements(axisPathSubList))
			{
				for (var j = 0; j < axisPathSubList.size(); j++)
				{
					...
					if (axisPathElementStruct.hasStringByKey("DimensionType"))
					{
						var componentType = oFF.XComponentType.lookupComponentType(axisPathElementStruct.getStringByKey("DimensionType"));
						if (oFF.notNull(componentType) && componentType !== oFF.OlapComponentType.ABSTRACT_DIMENSION && componentType.isTypeOf(oFF.OlapComponentType.ABSTRACT_DIMENSION))
						{
							newAxisPathElement.setDimensionType(componentType);
						}
					}
					if (axisPathElementStruct.hasStringByKey("FieldName"))
					{
						newAxisPathElement.setFieldName(axisPathElementStruct.getStringByKey("FieldName"));
					}
					if (axisPathElementStruct.hasStringByKey("AttributeName"))
					{
						newAxisPathElement.setAttributeName(axisPathElementStruct.getStringByKey("AttributeName"));
					}
					if (axisPathElementStruct.hasStringByKey("SemanticType"))
					{
						newAxisPathElement.setSemanticType(axisPathElementStruct.getStringByKey("SemanticType"));
					}
					if (axisPathElementStruct.hasStringByKey("PresentationType"))
					{
						newAxisPathElement.setPresentationType(oFF.QInAConverter.lookupPresentationType(axisPathElementStruct.getStringByKey("PresentationType")));
					}
				}
			}
		}
...
oFF.QInARepoTableCellReferenceScope.prototype.exportComponentWithStructure = function(exporter, modelComponent, inaStructure, flags)
{
	if (oFF.notNull(inaStructure) && oFF.notNull(modelComponent))
	{
		...
		inaStructure.putInteger("MatchSkipFirst", cellsReferenceScope.getMatchSkipFirst());
		inaStructure.putInteger("MatchSkipLast", cellsReferenceScope.getMatchSkipLast());
		...
		inaStructure.putBoolean("MatchHeaderFieldsSectionEnd", cellsReferenceScope.isMatchHeaderFieldsSectionEnd());
	...
	if (oFF.XCollectionUtils.hasElements(axisPaths))
	{
		...
		for (var i = 0; i < axisPaths.size(); i++)
		{
			...
			for (var j = 0; j < axisPath.size(); j++)
			{
				...
				axisPathElementStruct.putStringNotNullAndNotEmpty("DimensionName", axisPathElement.getDimensionName());
				axisPathElementStruct.putStringNotNullAndNotEmpty("AttributeName", axisPathElement.getAttributeName());
				axisPathElementStruct.putStringNotNullAndNotEmpty("FieldName", axisPathElement.getFieldName());
				axisPathElementStruct.putStringNotNullAndNotEmpty("SemanticType", axisPathElement.getSemanticType());
				axisPathElementStruct.putStringNotNullAndNotEmpty("PresentationType", oFF.QInAConverter.lookupPresentationTypeInA(axisPathElement.getPresentationType()));
				if (axisPathElement.getDimensionType() !== null)
				{
					axisPathElementStruct.putString("DimensionType", axisPathElement.getDimensionType().getName());
				}
			}
		}
	}
...
oFF.QInARepoTableDefinition.prototype.importComponentWithStructure = function(importer, inaStructure, modelComponent, parentComponent, context)
{
	if (oFF.notNull(modelComponent) && oFF.notNull(inaStructure) && modelComponent.getComponentType() === oFF.OlapComponentType.VISUALIZATION_TABLE_DEFINITION)
	{
		...
		this.importLayeredRectangularStyles(importer, tableDefinition, inaStructure);
		...
		{
			tableDefinition.setStripeDataColumns(this.getTriStateBool(inaStructure, "StripeDataColumns"));
		}
		if (inaStructure.containsKey("StripeDataRows"))
		{
			tableDefinition.setStripeDataRows(this.getTriStateBool(inaStructure, "StripeDataRows"));
		}
		if (inaStructure.containsKey("FreezeHeaderRows"))
		{
			tableDefinition.setFreezeHeaderRows(this.getTriStateBool(inaStructure, "FreezeHeaderRows"));
		}
		if (inaStructure.containsKey("FreezeHeaderColumns"))
		{
			tableDefinition.setFreezeHeaderColumns(this.getTriStateBool(inaStructure, "FreezeHeaderColumns"));
		}
		...
		{
			tableDefinition.setShowFreezeLines(this.getTriStateBool(inaStructure, "ShowFreezeLines"));
		}
		if (inaStructure.containsKey("ShowGrid"))
		{
			tableDefinition.setShowGrid(this.getTriStateBool(inaStructure, "ShowGrid"));
		}
		if (inaStructure.containsKey("ShowDimensionTitles"))
		{
			tableDefinition.setShowDimensionTitles(this.getTriStateBool(inaStructure, "ShowDimensionTitles"));
		}
		...
		{
			tableDefinition.setShowCoordinateHeader(this.getTriStateBool(inaStructure, "ShowCoordinateHeader"));
		}
		...
		tableDefinition.setSerializeLinkedDefinitions(inaStructure.getBooleanByKeyExt("LinkedDefinitionsSerialize", false));
		if (inaStructure.containsKey("LinkedDefinitionPriorities"))
		{
			...
			oFF.XCollectionUtils.forEach(linkPrioList,  function(linkPrioStruct){
				...
				if (linkPrioStructure.containsKey("Priority"))
				{
		...
		if (inaStructure.containsKey("LinkedDefinitionNames"))
		{
			...
			oFF.XCollectionUtils.forEach(linkNameList,  function(linkNameStruct){
				...
				tableDefinition.putLinkedDefinitionName(linkNameStructure.getStringByKey("Key"), linkNameStructure.getStringByKey("Name"));
			}.bind(this));
		}
...
oFF.QInARepoTableDefinition.prototype.importFormattingStyles = function(tableDefinition, inaStructure)
{
	...
	if (oFF.XCollectionUtils.hasElements(formattingStyleList))
	{
		for (var i = 0; i < formattingStyleList.size(); i++)
		{
			...
			var cellTypeRestrictions = formattingStyleStruct.getListByKey("CellTypeRestrictions");
			if (oFF.XCollectionUtils.hasElements(cellTypeRestrictions))
			{
				oFF.XCollectionUtils.forEach(cellTypeRestrictions,  function(ctrInA){
					this.importCellTypeRestriction(formattingStyle.addNewCellTypeRestriction(), ctrInA.asStructure());
				}.bind(this));
			}
			...
			formattingStyle.setFontSize(formattingStyleStruct.getDoubleByKeyExt("FontSize", 0));
...
oFF.QInARepoTableDefinition.prototype.importCellTypeRestriction = function(newCellTypeRestriction, structure)
{
	if (oFF.notNull(structure))
	{
		newCellTypeRestriction.setExpanded(this.getTriStateBool(structure, "Expanded"));
		newCellTypeRestriction.setInHierarchy(this.getTriStateBool(structure, "InHierarchy"));
		newCellTypeRestriction.setInTotalsContext(this.getTriStateBool(structure, "Totals"));
		newCellTypeRestriction.setMerged(this.getTriStateBool(structure, "Merged"));
		newCellTypeRestriction.setRepeatedMemberName(this.getTriStateBool(structure, "RepeatedMemberName"));
		var inaCellTypes = structure.getListByKey("CellTypes");
		if (oFF.XCollectionUtils.hasElements(inaCellTypes))
		{
			oFF.XStream.of(inaCellTypes).map( function(ict){
				return ict.asString();
			}.bind(this)).filterNullValues().map( function(icts){
				return oFF.TableCellType.lookup(icts.getString());
			}.bind(this)).filterNullValues().forEach( function(ictv){
...
oFF.QInARepoTableDefinition.prototype.importStyledLine = function(styledLine, importStructure)
{
	if (oFF.notNull(importStructure))
	{
		...
		styledLine.setPatternWidth(importStructure.getDoubleByKeyExt("PatternWidth", 0));
...
oFF.QInARepoTableDefinition.prototype.importLayeredRectangularStyles = function(importer, tableDefinition, inaStructure)
{
	var layeredStyleList = inaStructure.getListByKey("LayeredRectangularStyle");
	...
	{
		for (var i = 0; i < layeredStyleList.size(); i++)
		{
			var layeredStyleStructure = layeredStyleList.getStructureAt(i);
			var format = tableDefinition.addNewLayeredRectangularStyle(layeredStyleStructure.getStringByKey("Name"), layeredStyleStructure.getStringByKey("Text"));
...
oFF.QInARepoTableDefinition.prototype.importTableMarkups = function(importer, tableDefinition, inaStructure)
{
	...
	if (oFF.XCollectionUtils.hasElements(tableMarkupList))
	{
		for (var i = 0; i < tableMarkupList.size(); i++)
		{
			...
			markup.setHide(tableMarkupStructure.getBooleanByKeyExt("Hidden", false));
			...
			markup.setCellHeightAddition(tableMarkupStructure.getIntegerByKeyExt("HeightAddition", 0));
			markup.setCellWidthAddition(tableMarkupStructure.getIntegerByKeyExt("WidthAddition", 0));
...
oFF.QInARepoTableDefinition.prototype.importInsertedTuples = function(tupleList, deleter, inserter)
{
	if (oFF.notNull(tupleList))
	{
		...
		for (var i = 0; i < tupleList.size(); i++)
		{
			...
			insertedTuple.setMerged(structure.getBooleanByKeyExt("MergeCells", false));
...
oFF.QInARepoTableDefinition.prototype.exportComponentWithStructure = function(exporter, modelComponent, inaStructure, flags)
{
	if (oFF.notNull(inaStructure) && oFF.notNull(modelComponent))
	{
		...
		this.exportLayeredRectangularStyles(exporter, tableDefinition, inaStructure);
		...
		this.setTriStateBool(inaStructure, "StripeDataRows", tableDefinition.isStripeDataRows());
		this.setTriStateBool(inaStructure, "StripeDataColumns", tableDefinition.isStripeDataColumns());
		this.setTriStateBool(inaStructure, "FreezeHeaderRows", tableDefinition.isFreezeHeaderRows());
		this.setTriStateBool(inaStructure, "FreezeHeaderColumns", tableDefinition.isFreezeHeaderColumns());
		...
		this.setTriStateBool(inaStructure, "ShowFreezeLines", tableDefinition.isShowFreezeLines());
		this.setTriStateBool(inaStructure, "ShowGrid", tableDefinition.isShowGrid());
		this.setTriStateBool(inaStructure, "ShowDimensionTitles", tableDefinition.isShowDimensionTitles());
		...
		this.setTriStateBool(inaStructure, "ShowCoordinateHeader", tableDefinition.isShowCoordinateHeader());
...
oFF.QInARepoTableDefinition.prototype.exportFormattingStyles = function(tableDefinition, inaStructure)
{
	...
	if (oFF.XCollectionUtils.hasElements(formattingStyles))
	{
		...
		for (var i = 0; i < formattingStyles.size(); i++)
		{
			...
			var cellTypeRestrictions = formattingStyle.getCellTypeRestrictions();
			if (oFF.XCollectionUtils.hasElements(cellTypeRestrictions))
			{
				var cellTypeRestrictionIna = formattingStyleStruct.putNewList("CellTypeRestrictions");
				oFF.XCollectionUtils.forEach(cellTypeRestrictions,  function(ctrObj){
					this.exportCellTypeRestriction(ctrObj, cellTypeRestrictionIna.addNewStructure());
				}.bind(this));
			}
			...
			this.setTriStateBool(formattingStyleStruct, "FontBold", formattingStyle.isFontBoldExt());
			this.setTriStateBool(formattingStyleStruct, "FontItalic", formattingStyle.isFontItalicExt());
			this.setTriStateBool(formattingStyleStruct, "FontUnderline", formattingStyle.isFontUnderlineExt());
			this.setTriStateBool(formattingStyleStruct, "FontStrikeThrough", formattingStyle.isFontStrikeThroughExt());
...
oFF.QInARepoTableDefinition.prototype.exportCellTypeRestriction = function(cellTypeRestriction, cellTypeRestrictionIna)
{
	this.setTriStateBool(cellTypeRestrictionIna, "Expanded", cellTypeRestriction.isExpanded());
	this.setTriStateBool(cellTypeRestrictionIna, "InHierarchy", cellTypeRestriction.isInHierarchy());
	this.setTriStateBool(cellTypeRestrictionIna, "Totals", cellTypeRestriction.isInTotalsContext());
	this.setTriStateBool(cellTypeRestrictionIna, "Merged", cellTypeRestriction.isMerged());
	this.setTriStateBool(cellTypeRestrictionIna, "RepeatedMemberName", cellTypeRestriction.isRepeatedMemberName());
	var matchingCellTypes = cellTypeRestriction.getMatchingCellTypes();
	if (oFF.XCollectionUtils.hasElements(matchingCellTypes))
	{
		var matchingCellTypesInA = cellTypeRestrictionIna.putNewList("CellTypes");
		oFF.XCollectionUtils.forEach(matchingCellTypes,  function(mct){
...
oFF.QInARepoTableDefinition.prototype.exportLayeredRectangularStyles = function(exporter, tableDefinition, inaStructure)
{
	var layeredStyles = tableDefinition.getLayeredRectangularStyles();
	if (oFF.XCollectionUtils.hasElements(layeredStyles))
	{
		var inaList = inaStructure.putNewList("LayeredRectangularStyle");
		for (var i = 0; i < layeredStyles.size(); i++)
		{
			var layeredStyle = layeredStyles.get(i);
			var layeredStyleStruct = inaList.addNewStructure();
...
oFF.QInARepoTableDefinition.prototype.exportTableMarkups = function(exporter, tableDefinition, inaStructure)
{
	...
	if (oFF.XCollectionUtils.hasElements(tableMarkups))
	{
		...
		for (var i = 0; i < tableMarkups.size(); i++)
		{
			...
			tableMarkupStructure.putInteger("HeightAddition", tableMarkup.getCellHeightAddition());
			tableMarkupStructure.putInteger("WidthAddition", tableMarkup.getCellWidthAddition());
...
oFF.QInARepoTableDefinition.prototype.getTriStateBool = function(struct, key)
{
	var result = oFF.TriStateBool._DEFAULT;
	if (struct.containsKey(key))
	{
...
oFF.QInARepoTableDefinition.prototype.setTriStateBool = function(struct, key, tsb)
{
	if (tsb === oFF.TriStateBool._TRUE)
	{
		struct.putBoolean(key, true);
...
oFF.QInARepoVisualizationManager.prototype.importComponentWithStructure = function(importer, inaStructure, modelComponent, parentComponent, context)
{
	if (oFF.notNull(inaStructure) && oFF.notNull(modelComponent) && modelComponent.getOlapComponentType() === oFF.OlapComponentType.VISUALIZATION_MANAGER)
	{
		...
		var activeVisualizationDefintion = inaStructure.getStringByKey("ActiveVisualizationDefinition");
		var visualizationManager = modelComponent;
		var activeVisualizationDefinition = visualizationManager.getVisualizationDefinitionByName(activeVisualizationDefintion);
		if (oFF.isNull(activeVisualizationDefinition))
		{
			activeVisualizationDefinition = oFF.XCollectionUtils.findFirst(visualizationManager.getVisualizationDefinitions(),  function(visDef){
...
oFF.QInARepoVisualizationManager.prototype.exportComponentWithStructure = function(exporter, modelComponent, inaStructure, flags)
{
	if (oFF.notNull(modelComponent) && modelComponent.getOlapComponentType() === oFF.OlapComponentType.VISUALIZATION_MANAGER)
	{
		...
		if (oFF.XCollectionUtils.hasElements(inaList))
		{
			inaStructure.put("Elements", inaList);
			var visualizationManager = modelComponent;
			var currentActiveVisualizationDefinition = visualizationManager.getCurrentActiveVisualizationDefinition();
...
oFF.QInARepoChartDefinition.prototype.importComponentWithStructure = function(importer, inaStructure, modelComponent, parentComponent, context)
{
	if (oFF.notNull(inaStructure) && oFF.notNull(modelComponent) && modelComponent.getOlapComponentType() === oFF.OlapComponentType.VISUALIZATION_CHART_DEFINITION)
	{
		...
		var keyFigureDimensionName = inaStructure.getStringByKey("KeyFigureDimension");
		var kfDimension = chartDefinitionBase.getQueryModel().getDimensionByName(keyFigureDimensionName);
		chartDefinitionBase.setKeyFigureDimension(kfDimension);
		var inAChartSettings = inaStructure.getStructureByKey("ChartSettings");
		var chartSettings = chartDefinitionBase.getChartSetting();
		var chartOrientation = inAChartSettings.getStringByKey("ChartOrientation");
		var chartStackingType = inAChartSettings.getStringByKey("ChartStacking");
		var chartType = inAChartSettings.getStringByKey("ChartType");
		var chartType2 = inAChartSettings.getStringByKey("ChartType2");
		var chartTitle = inAChartSettings.getStringByKey("Title");
		var chartSubTitle = inAChartSettings.getStringByKey("SubTitle");
		var chartHasTitleExpanded = inAChartSettings.getBooleanByKey("HasTitleExpanded");
		...
		this.importChartLegend(chartSettings, inAChartSettings);
		this.importCategoryAxis(chartSettings, inAChartSettings);
		this.importValueAxis(chartSettings, inAChartSettings);
		this.importColorScheme(chartSettings, inAChartSettings);
		this.importPlotArea(chartSettings, inAChartSettings);
		var inAKeyFigureSelections = inAChartSettings.getListByKey("KeyFigureSelections");
		oFF.XStream.of(inAKeyFigureSelections).forEach( function(inAKeyFigureSelection){
			var inaKeyFigureSel = inAKeyFigureSelection;
			var visualizationValueType = oFF.VisualizationValueType.lookup(inaKeyFigureSel.getStringByKey("VisualizationValueType"));
			var inAMembers = inaKeyFigureSel.getListByKey("Members");
			if (oFF.XCollectionUtils.hasElements(inAMembers))
			{
				oFF.XStream.of(inAMembers).forEach( function(inAMember){
					var figureSelection;
					var dimensionName = inAMember.asStructure().getStringByKey("DimensionName");
					var id = inAMember.asStructure().getStringByKey("Id");
					var visDimension = chartDefinitionBase.getQueryModel().getDimensionByName(dimensionName);
					figureSelection = oFF.QKeyFigureSelection.create(context, visDimension.getDimensionMember(id), chartDefinitionBase, visualizationValueType);
					var color = inAMember.asStructure().getStringByKey("Color");
					if (oFF.notNull(color))
					{
						figureSelection.setColor(color);
					}
					chartSettings.addTypedValueSelection(visualizationValueType, figureSelection);
				}.bind(this));
			}
		}.bind(this));
	}
...
oFF.QInARepoChartDefinition.prototype.importPlotArea = function(chartSetting, inaChartSetting)
{
	var plotAreaStructure = inaChartSetting.getStructureByKey("PlotArea");
	if (oFF.notNull(plotAreaStructure))
	{
		var plotArea = oFF.QChartPlotArea.create();
		var chartDataLabel = this.importChartLabel(plotAreaStructure, "DataLabel");
		plotArea.setDataLabel(chartDataLabel);
		var chartStackColumnLabel = this.importChartLabel(plotAreaStructure, "StackColumnLabel");
		plotArea.setStackColumnLabel(chartStackColumnLabel);
		var dimensionLabel = this.importChartLabel(plotAreaStructure, "DimensionLabel");
		plotArea.setDimensionLabel(dimensionLabel);
		chartSetting.setPlotArea(plotArea);
	}
};
oFF.QInARepoChartDefinition.prototype.importColorScheme = function(chartSetting, inaChartSetting)
{
	var colorSchemeStructure = inaChartSetting.getStructureByKey("ColorScheme");
	if (oFF.notNull(colorSchemeStructure))
	{
		var colorScheme = oFF.QChartColorScheme.create();
		var colorsList = oFF.PrUtils.asListOfString(colorSchemeStructure.getListByKey("Colors"));
		if (oFF.XCollectionUtils.hasElements(colorsList))
		{
			var colors = oFF.XListOfString.create();
			oFF.XStream.ofString(colorsList).forEach( function(color){
...
oFF.QInARepoChartDefinition.prototype.importCategoryAxis = function(chartSetting, inaChartSetting)
{
	var categoryAxisStructure = inaChartSetting.getStructureByKey("CategoryAxis");
	if (oFF.notNull(categoryAxisStructure))
	{
		var categoryAxis = oFF.QChartAxis.create();
		this.importAxis(categoryAxis, categoryAxisStructure);
		chartSetting.setCategoryAxis(categoryAxis);
	}
};
oFF.QInARepoChartDefinition.prototype.importValueAxis = function(chartSetting, inaChartSetting)
{
	var valueAxisStructure = inaChartSetting.getStructureByKey("ValueAxis");
	if (oFF.notNull(valueAxisStructure))
	{
		var valueAxis = oFF.QChartAxis.create();
		this.importAxis(valueAxis, valueAxisStructure);
		chartSetting.setValueAxis(valueAxis);
	}
};
oFF.QInARepoChartDefinition.prototype.importAxis = function(axis, axisStructure)
{
	var color = axisStructure.getStringByKey("Color");
	if (oFF.notNull(color))
	{
		axis.setColor(color);
	}
	var showLabelGrids = axisStructure.getBooleanByKey("ShowLabelGrids");
	axis.setShowLabelGrids(showLabelGrids);
	var chartLabel = this.importChartLabel(axisStructure, "Label");
	if (oFF.notNull(chartLabel))
	{
		axis.setLabel(chartLabel);
	}
	var axisLineStructure = axisStructure.getStructureByKey("CategoryAxis");
	if (oFF.notNull(axisLineStructure))
	{
		var axisLine = this.importAxisComponent(axisLineStructure);
		axis.setLine(axisLine);
	}
	var axisTickStructure = axisStructure.getStructureByKey("CategoryAxis");
	if (oFF.notNull(axisTickStructure))
	{
		var axisTick = this.importAxisComponent(axisTickStructure);
		axis.setTick(axisTick);
	}
};
oFF.QInARepoChartDefinition.prototype.importAxisComponent = function(componentStructure)
{
	var component = oFF.QChartAxisComponent.create();
	var size = componentStructure.getDoubleByKey("Size");
	component.setSize(size);
	var visible = componentStructure.getBooleanByKey("Visible");
	component.setVisible(visible);
	return component;
};
oFF.QInARepoChartDefinition.prototype.importChartLegend = function(chartSetting, inaStructure)
{
	var chartLegendStructure = inaStructure.getStructureByKey("Legend");
	if (oFF.notNull(chartLegendStructure))
	{
		var chartLegend = oFF.QChartLegend.create();
		var showFullLabel = chartLegendStructure.getBooleanByKey("ShowFullLabel");
		chartLegend.setShowFullLabel(showFullLabel);
		var chartLabel = this.importChartLabel(chartLegendStructure, "Label");
		if (oFF.notNull(chartLabel))
		{
			chartLegend.setLabel(chartLabel);
		}
		chartSetting.setLegend(chartLegend);
	}
};
oFF.QInARepoChartDefinition.prototype.importChartLabel = function(parentStructure, labelKey)
{
	var chartLegendLabelStructure = parentStructure.getStructureByKey(labelKey);
	if (oFF.notNull(chartLegendLabelStructure))
	{
		var chartLabel = oFF.QChartLabel.create();
		var alignmentString = chartLegendLabelStructure.getStringByKey("Alignment");
		if (oFF.notNull(alignmentString))
		{
			var alignment = oFF.ChartAlignment.lookup(alignmentString);
			chartLabel.setChartAlignment(alignment);
		}
		var verticalAlignmentString = chartLegendLabelStructure.getStringByKey("VerticalAlignment");
		if (oFF.notNull(verticalAlignmentString))
		{
			var verticalAlignment = oFF.ChartVerticalAlignment.lookup(verticalAlignmentString);
			chartLabel.setChartVerticalAlignment(verticalAlignment);
		}
		var autoResize = chartLegendLabelStructure.getBooleanByKey("AutoResize");
		var hiddenWhenOverlap = chartLegendLabelStructure.getBooleanByKey("HiddenWhenOverlap");
		var visible = chartLegendLabelStructure.getBooleanByKey("Visible");
		chartLabel.setAutoResize(autoResize);
		chartLabel.setHiddenWhenOverlap(hiddenWhenOverlap);
		chartLabel.setVisible(visible);
		var formatString = chartLegendLabelStructure.getStringByKey("FormatString");
		if (oFF.notNull(formatString))
		{
			chartLabel.setFormatString(formatString);
		}
		this.importChartLabelStyle(chartLabel, chartLegendLabelStructure);
		return chartLabel;
	}
	return null;
};
oFF.QInARepoChartDefinition.prototype.importChartLabelStyle = function(chartLabel, chartLegendLabelStructure)
{
	var chartLabelStyleStructure = chartLegendLabelStructure.getStructureByKey("Style");
	if (oFF.notNull(chartLabelStyleStructure))
	{
		var labelStyle = oFF.QChartLabelStyle.create();
		var chartLabelColor = chartLabelStyleStructure.getStringByKey("Color");
		labelStyle.setColor(chartLabelColor);
		var fontFamily = chartLabelStyleStructure.getStringByKey("FontFamily");
		labelStyle.setFontFamily(fontFamily);
		var fontSize = chartLabelStyleStructure.getStringByKey("FontSize");
		labelStyle.setFontSize(fontSize);
		var fontWeight = chartLabelStyleStructure.getStringByKey("FontWeight");
		labelStyle.setFontWeight(fontWeight);
...
oFF.QInARepoChartDefinition.prototype.exportComponentWithStructure = function(exporter, modelComponent, inaStructure, flags)
{
	if (oFF.notNull(modelComponent) && oFF.notNull(inaStructure))
	{
		...
		var inAChartSettings = inaStructure.putNewStructure("ChartSettings");
		var chartSetting = chartDefinition.getChartSetting();
		...
		this.exportChartLegend(chartSetting, inAChartSettings);
		var categoryAxis = chartSetting.getCategoryAxis();
		if (oFF.notNull(categoryAxis))
		{
			var inaCategoryAxis = inAChartSettings.putNewStructure("CategoryAxis");
			this.exportAxis(categoryAxis, inaCategoryAxis);
		}
		var valueAxis = chartSetting.getValueAxis();
		if (oFF.notNull(valueAxis))
		{
			var inaValueAxis = inAChartSettings.putNewStructure("ValueAxis");
			this.exportAxis(valueAxis, inaValueAxis);
		}
		var colorScheme = chartSetting.getColorScheme();
		if (oFF.notNull(colorScheme))
		{
			this.exportColorScheme(chartSetting, inAChartSettings);
		}
		var plotArea = chartSetting.getPlotArea();
		if (oFF.notNull(plotArea))
		{
			this.exportPlotArea(chartSetting, inAChartSettings);
		}
		var inAKFSelections = inAChartSettings.putNewList("KeyFigureSelections");
		oFF.XStream.of(oFF.VisualizationValueType.getAll()).forEach( function(visValueType){
			var typedValueSelectionsForType = chartSetting.getTypedValueSelectionsForType(visValueType);
			var valueStructure = inAKFSelections.addNewStructure();
			valueStructure.putString("VisualizationValueType", visValueType.getName());
			var membersList = oFF.PrList.create();
			var keyFigures = typedValueSelectionsForType.getValuesAsReadOnlyList();
			oFF.XStream.of(keyFigures).forEach( function(kf){
				var memberStructure = membersList.addNewStructure();
				memberStructure.putString("Id", kf.getName());
				memberStructure.putString("DimensionName", kf.getDimensionMember().getDimension().getName());
				var color = kf.getColor();
				if (oFF.notNull(color))
				{
					memberStructure.putString("Color", kf.getColor());
				}
			}.bind(this));
...
oFF.QInARepoChartDefinition.prototype.exportColorScheme = function(chartSetting, inaChartSetting)
{
	var colorScheme = chartSetting.getColorScheme();
	if (oFF.notNull(colorScheme))
	{
		var inaColorScheme = inaChartSetting.putNewStructure("ColorScheme");
		var colors = colorScheme.getColors();
		if (oFF.XCollectionUtils.hasElements(colors))
		{
			var inaColorsList = inaColorScheme.putNewList("Colors");
			oFF.XStream.ofString(colors).forEach( function(color){
				inaColorsList.addString(color.getString());
			}.bind(this));
		}
	}
};
oFF.QInARepoChartDefinition.prototype.exportPlotArea = function(chartSetting, inaChartSetting)
{
	var plotArea = chartSetting.getPlotArea();
	if (oFF.notNull(plotArea))
	{
		var inaPlotArea = inaChartSetting.putNewStructure("PlotArea");
		var dataLabel = plotArea.getDataLabel();
		if (oFF.notNull(dataLabel))
		{
			this.exportLabel(dataLabel, inaPlotArea, "DataLabel");
		}
		var dimensionLabel = plotArea.getDimensionLabel();
		if (oFF.notNull(dimensionLabel))
		{
			this.exportLabel(dimensionLabel, inaPlotArea, "DimensionLabel");
		}
		var stackColumnLabel = plotArea.getStackColumnLabel();
		if (oFF.notNull(dimensionLabel))
		{
			this.exportLabel(stackColumnLabel, inaPlotArea, "StackColumnLabel");
		}
	}
};
oFF.QInARepoChartDefinition.prototype.exportAxis = function(axis, inaAxis)
{
	var color = axis.getColor();
	if (oFF.notNull(color))
	{
		inaAxis.putString("Color", color);
	}
	var showLabelGrids = axis.isShowLabelGrids();
	inaAxis.putBoolean("ShowLabelGrids", showLabelGrids);
	var label = axis.getLabel();
	if (oFF.notNull(label))
	{
		this.exportLabel(label, inaAxis, "Label");
	}
	var line = axis.getLine();
	if (oFF.notNull(line))
	{
		var inaLine = inaAxis.putNewStructure("Line");
		this.exportAxisComponent(line, inaLine);
	}
	var tick = axis.getTick();
	if (oFF.notNull(tick))
	{
		var inaTick = inaAxis.putNewStructure("Tick");
		this.exportAxisComponent(tick, inaTick);
	}
};
oFF.QInARepoChartDefinition.prototype.exportAxisComponent = function(axisComponent, inaComponent)
{
	var size = axisComponent.getSize();
	inaComponent.putDouble("Size", size);
	var visible = axisComponent.isVisible();
	inaComponent.putBoolean("Visible", visible);
};
oFF.QInARepoChartDefinition.prototype.exportChartLegend = function(chartSetting, inaChartSetting)
{
	var chartLegend = chartSetting.getChartLegend();
	if (oFF.notNull(chartLegend))
	{
		var inaChartLegend = inaChartSetting.putNewStructure("Legend");
		var showFullLabel = chartLegend.isShowFullLabel();
		inaChartLegend.putBoolean("ShowFullLabel", showFullLabel);
		var label = chartLegend.getChartLabel();
		if (oFF.notNull(label))
		{
			this.exportLabel(label, inaChartLegend, "Label");
		}
	}
};
oFF.QInARepoChartDefinition.prototype.exportLabel = function(label, inaParent, labelKey)
{
	var inaLabel = inaParent.putNewStructure(labelKey);
	var chartAlignment = label.getChartAlignment();
	if (oFF.notNull(chartAlignment))
	{
		inaLabel.putString("Alignment", chartAlignment.getName());
	}
	var chartVerticalAlignment = label.getChartVerticalAlignment();
	if (oFF.notNull(chartVerticalAlignment))
	{
		inaLabel.putString("VerticalAlignment", chartVerticalAlignment.getName());
	}
	inaLabel.putBoolean("AutoResize", label.isAutoResize());
	inaLabel.putBoolean("HiddenWhenOverlap", label.isHiddenWhenOverlap());
	inaLabel.putBoolean("Visible", label.isVisible());
	var formatString = label.getFormatString();
	if (oFF.notNull(formatString))
	{
		inaLabel.putString("FormatString", formatString);
	}
	this.exportChartLabelStyle(label, inaLabel);
};
oFF.QInARepoChartDefinition.prototype.exportChartLabelStyle = function(chartLabel, inaChartLabel)
{
	var labelStyle = chartLabel.getStyle();
	if (oFF.notNull(labelStyle))
	{
		var inaLabelStyle = inaChartLabel.putNewStructure("Style");
		var color = labelStyle.getColor();
		if (oFF.notNull(color))
		{
			inaLabelStyle.putString("Color", color);
		}
		var fontFamily = labelStyle.getFontFamily();
		if (oFF.notNull(fontFamily))
		{
			inaLabelStyle.putString("FontFamily", fontFamily);
		}
		var fontSize = labelStyle.getFontSize();
		if (oFF.notNull(fontSize))
		{
			inaLabelStyle.putString("FontSize", fontSize);
		}
		var fontWeight = labelStyle.getFontWeight();
		if (oFF.notNull(fontWeight))
		{
			inaLabelStyle.putString("FontWeight", fontWeight);
		}
	}
};
...
{
	return this.isSystemType(context, oFF.SystemType.ABAP);
};
oFF.QInA.prototype.isAbapMds = function(context)
{
	return this.isSystemType(context, oFF.SystemType.ABAP_MDS);
};
oFF.QInA.prototype.isSystemType = function(context, systemType)
{
	var modelCapabilities = context.getModelCapabilities();
	if (oFF.notNull(modelCapabilities))
	{
		var st = modelCapabilities.getSystemType();
		if (oFF.notNull(st))
		{
			return st.isTypeOf(systemType);
		}
	}
	return false;
};
