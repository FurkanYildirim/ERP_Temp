...
oFF.BlendingMappingUtils = {
			...
			while (transientMembersIter.hasNext())
			{
				...
				if (oFF.notNull(measureFilter))
				{
					...
					if (!memberNamesFromFilter.contains(memberName))
					{
						...
						var hierarchyName = calculationDimension.supportsHierarchy() && calculationDimension.isHierarchyActive() ? calculationDimension.getHierarchyName() : null;
			...
			if (calculationDimension.getDimensionType() === oFF.DimensionType.ACCOUNT && calculationDimension.isHierarchyActive() && calculationDimension.getInitialDrillLevel() > -1)
		{
			...
			else
			{
				...
				if (!drillPathExist)
				{
					...
					{
						var cartesianProduct = visibilityFilter.getCartesianProductWithDefault();
						cartesianProduct.getTagging().put(oFF.QTransientConstants.TAG_IS_TRANSIENT, "");
					}
					var calculationDimensionList = visibilityFilter.getCartesianList(calculationDimension);
					...
					var membersFromExistingList = oFF.QFilterUtil.getMemberNamesFromFilter(calculationDimensionList, calculationDimension.getName());
					if (!membersFromExistingList.contains(memberName))
					{
						var newElement = calculationDimensionList.addNewCartesianElement();
	...
	processCalculationDimensionMappings:function(blendingDefinition, primarySource, secondarySource, blendedDrillOperations)
	{
		...
		if (oFF.notNull(primaryAccountDimension))
		{
			...
			if (primaryAccountDimension.getAxisType() !== oFF.AxisType.ROWS && primaryAccountDimension.getAxisType() !== oFF.AxisType.COLUMNS)
			{
				primaryQueryModel.getColumnsAxis().add(primaryAccountDimension);
			}
		...
		{
			if (primaryMeasureDimension.getAxisType() !== oFF.AxisType.ROWS && primaryMeasureDimension.getAxisType() !== oFF.AxisType.COLUMNS)
			{
				primaryQueryModel.getColumnsAxis().add(primaryMeasureDimension);
			}
			oFF.BlendingMappingUtils.addSignedDataToFilter(primaryQueryModel);
		}
		...
		if (oFF.notNull(secondaryMeasureDimension))
		{
			...
			oFF.BlendingMappingUtils.addSignedDataToFilter(secondaryQueryModel);
		}
	...
	addSignedDataToFilter:function(queryModel)
	{
			if (queryModel.getAccountDimension() !== null && !queryModel.hasUserDefinedMeasures())
		{
			...
			var measureDimensionFilter = measureDimension.getFilter();
			if (oFF.isNull(measureDimensionFilter) || measureDimensionFilter.isEmpty())
			{
				var filterContainer = queryModel.getFilter().getDynamicFilter();
				if (filterContainer.getFilterRootElement() === null || filterContainer.getFilterRootElement().getOlapComponentType() !== oFF.FilterComponentType.FILTER_ACROSS_MODELS)
				{
					...
					var newMeasureFilter = measureDimensionFilter.addNewCartesianElement();
	...
	processTimeDimensionAttributeMapping:function(primarySource, secondarySource, blendingProcess, primaryLinkDimension, secondaryLinkDimension, linkMapping)
	{
		...
		if (primaryLinkDimension.isHierarchyActive())
		{
			...
			var primaryLevelMaxDepth = primaryLeveledHierarchy.getMostGranularLevel().getLevelNumber();
			var secondaryLevelMaxDepth = secondaryLeveledHierarchy.getMostGranularLevel().getLevelNumber();
			...
			if (oFF.notNull(primaryLeveledHierarchy) && oFF.notNull(secondaryLeveledHierarchy))
		{
			...
			while (primaryHierarchyLevelsIter.hasNext())
			{
				...
				if (oFF.notNull(secondaryHierarchyLevel))
				{
					...
					if (!primaryLevelHierarchyField.isAlwaysRequested())
					{
						primaryLevelHierarchyField.setAlwaysRequested(true);
						primaryLevelHierarchyField.getTagging().put("transientAlwaysRequested", "true");
					}
					secondaryLinkDimension.getResultSetFields().add(secondaryLevelHierarchyField);
					if (!secondaryLevelHierarchyField.isAlwaysRequested())
					{
						secondaryLevelHierarchyField.setAlwaysRequested(true);
						secondaryLevelHierarchyField.getTagging().put("transientAlwaysRequested", "true");
					}
					var attributeMapping = oFF.BlendingMappingUtils.getAttributeMapping(linkMapping, primaryLevelHierarchyField.getName());
					if (oFF.isNull(attributeMapping))
					{
	...
	{
			return oFF.XCollectionUtils.findFirst(linkMapping.getAttributeMappings(),  function(attributeMapping){
	...
	getAttributeJoinField:function(dimensionLinkPart, dimension, process)
	{
		...
		while (iterator.hasNext())
		{
			...
			{
				oRequestedField.getTagging().put("transientAlwaysRequested", "false");
...
oFF.BlendingSortUtils = {
	...
	addSortByDimension:function(blendableQueryManager, queryModel, sortingManager, sort)
	{
		...
		var dimension = oFF.BlendingUtils.getBlendedDimensionFromLeafDimension(keyField.getDimension().getName(), sort.getDatasetId(), queryModel);
...
oFF.BlendableQMPersistedInAExportAction.prototype.addSecondaryInaToDependentQueries = function(remoteQueryMangager, localSystemDescription, remoteSystemDescription, dependentQueries)
{
	...
	{
		dependentQueries.add(oFF.DependentPersistedQueryInfo.create(remoteQueryMangager.getSystemName(), this.generateRemoteQuery(secondaryInaQuery, null, null)));
	}
	else
	{
		dependentQueries.add(oFF.DependentPersistedQueryInfo.create(remoteQueryMangager.getSystemName(), this.generateRemoteQuery(secondaryInaQuery, remoteSystemDescription.getSystemMapping(localSystemDescription.getSystemName()), identifier)));
	}
...
oFF.BlendableQMPersistedInAExportAction.prototype.generateRemoteQuery = function(originalQuery, systemMapping, persistenceIdentifier)
{
	...
	if (oFF.notNull(batchQuery))
	{
		...
		while (queryIter.hasNext())
		{
			...
			var definitionStructure = oFF.PrUtils.getStructureProperty(inaRemoteContext, "Definition");
...
oFF.BlendingProcess.PRIMARY_MEASURE_FILTER_EXPRESSION = "primaryMeasureFilterExpression";
oFF.BlendingProcess.SECONDARY_MEASURE_FILTER_EXPRESSION = "secondaryMeasureFilterExpression";
...
oFF.BlendingProcess.prototype.validateCachedRuntimeBlendedQueryManager = function(blendableQueryManager, runtimeBlendedQueryManager, previousMeasureFilterModCounters)
{
	...
	while (iterator.hasNext())
	{
		...
		if (oFF.notNull(primaryCalculation) && primaryCalculation.getFilter() !== null)
		{
			...
			var primaryFilterExpression = primaryCalculation.getFilter().getFilterExpression();
			if (oFF.notNull(primaryFilterExpression))
			{
				primaryModCounter = oFF.XIntegerValue.create(primaryFilterExpression.getModCounter());
				if (oFF.isNull(previousModCounter) || previousModCounter.getByKey(oFF.BlendingProcess.PRIMARY_MEASURE_FILTER_EXPRESSION) !== null && !primaryModCounter.isEqualTo(previousModCounter.getByKey(oFF.BlendingProcess.PRIMARY_MEASURE_FILTER_EXPRESSION)))
				{
					this.m_cachedRuntimeBlendedQueryManagerResult = null;
				}
				modCounters.put(oFF.BlendingProcess.PRIMARY_MEASURE_FILTER_EXPRESSION, primaryModCounter);
			}
		}
		if (oFF.notNull(secondaryCalculation) && secondaryCalculation.getFilter() !== null)
		{
			...
			var secondaryFilterExpression = secondaryCalculation.getFilter().getFilterExpression();
			if (oFF.notNull(secondaryFilterExpression))
			{
				secondaryModCounter = oFF.XIntegerValue.create(secondaryFilterExpression.getModCounter());
				if (oFF.isNull(previousModCounter) || previousModCounter.getByKey(oFF.BlendingProcess.SECONDARY_MEASURE_FILTER_EXPRESSION) !== null && !secondaryModCounter.isEqualTo(previousModCounter.getByKey(oFF.BlendingProcess.SECONDARY_MEASURE_FILTER_EXPRESSION)))
				{
					this.m_cachedRuntimeBlendedQueryManagerResult = null;
				}
				modCounters.put(oFF.BlendingProcess.SECONDARY_MEASURE_FILTER_EXPRESSION, secondaryModCounter);
			}
		}
		var dimensions = oFF.XList.create();
		dimensions.addAll(queryModel.getRowsAxis());
		...
		while (dimensionsIter.hasNext())
		{
			...
			var isDimensionHierarchyActive = oFF.XBooleanValue.create(dimension.isHierarchyActive());
			if (oFF.isNull(previousModCounter) || previousModCounter.getByKey(dimension.getName()) !== null && !isDimensionHierarchyActive.isEqualTo(previousModCounter.getByKey(dimension.getName())))
			{
				this.m_cachedRuntimeBlendedQueryManagerResult = null;
			}
			modCounters.put(dimension.getName(), isDimensionHierarchyActive);
		}
	...
	while (blendedMeasuresIter.hasNext())
	{
		...
		if (blendedMeasure.getMemberType() === oFF.MemberType.FORMULA)
		{
			...
			while (formulaItemMembersIter.hasNext())
			{
				...
				var formulaItemQueryModel = oFF.notNull(formulaDimensionContext) ? formulaDimensionContext.getQueryModel() : formulaItemMember.getQueryModel();
				...
				if (oFF.notNull(formulaDimensionContext))
				{
					...
					if (oFF.notNull(blendDimensionContext))
					{
						...
						if (oFF.isNull(structureMember))
						{
							...
							isSharedCalculation = true;
						}
						if (!isSharedCalculation && (blendDimensionContext.getFilter() === null || blendDimensionContext.getFilter().isEmpty()) && this.m_blendableQueryManager.isUseEmptyAllMemberMeasureFilterByDatasetId(blendQueryModelContext.getQueryModel().getDatasetId()))
						{
...
oFF.BlendingProcess.prototype.processSynchronization = function(syncType)
{
	...
	else if (oFF.BlendingUtils.isQueryModelInUseForBlending(primaryQueryModel, this.m_blendableQueryManager, false))
	{
...
oFF.BlendingProcess.prototype.processBlendingQueryManagerCreation = function()
{
	...
	oFF.BlendingMappingUtils.processTransientCalculations(primaryQueryModel, this.m_transientMembers.getByKey(primaryQueryModel.getDatasetId()), this.m_blendableQueryManager.isUseEmptyAllMemberMeasureFilterByDatasetId(primaryQueryModel.getDatasetId()));
	oFF.BlendingMappingUtils.processTransientCalculations(secondaryQueryModel, this.m_transientMembers.getByKey(secondaryQueryModel.getDatasetId()), this.m_blendableQueryManager.isUseEmptyAllMemberMeasureFilterByDatasetId(secondaryQueryModel.getDatasetId()));
	this.collectLeafQueryAccountDrills(primaryQueryManager);
	this.collectLeafQueryAccountDrills(secondaryQueryManager);
	if (!this.m_blendableQueryManager.isUseEmptyAllMemberMeasureFilterByDatasetId(primaryQueryModel.getDatasetId()))
	{
		this.addDummyCalculationMeasure(primaryQueryManager);
	}
	if (!this.m_blendableQueryManager.isUseEmptyAllMemberMeasureFilterByDatasetId(secondaryQueryModel.getDatasetId()))
	{
...
oFF.BlendingProcess.prototype.collectLeafQueryAccountDrills = function(leafQueryManager)
{
	var queryModel = leafQueryManager.getQueryModel();
	if (leafQueryManager.getInitSettings().getMode() !== oFF.QueryManagerMode.DEFAULT || queryModel.getAccountDimension() === null || oFF.XCollectionUtils.contains(this.m_blendedDrillOperations,  function(blendedDrillOperation){
	...
	for (var i = 0; i < drillOperations.size(); i++)
	{
		...
		var drillOperation = drillOperations.get(i);
...
{
	oFF.XCollectionUtils.forEach(this.m_blendableQueryManager.getAllQueryManagers(),  function(queryManager){
		...
		while (dimensionsInDrillIter.hasNext())
		{
			...
			if (oFF.notNull(originalDimensionResultSetFields))
			{
				...
				while (iterator.hasNext())
				{
					...
					if (oRequestedField.getTagging().containsKey("transientAlwaysRequested"))
					{
						oRequestedField.setAlwaysRequested(oFF.XBoolean.convertFromString(oRequestedField.getTagging().getByKey("transientAlwaysRequested")));
		...
		if (oFF.notNull(transientMembers))
		{
			...
			{
				var transientMember = transientMembersIter.next();
				var structureMember = this.getOlapEnv().getClientQueryObjectManager().getStructuredMember(transientMember.getAliasOrMemberName(), this.m_blendableQueryManager.getClientQueryObjectStorageName(), true);
				if (oFF.notNull(structureMember) || oFF.XString.isEqual(transientMember.getAliasOrMemberName(), "DummyCalculation"))
				{
		...
		if (oFF.notNull(visibilityFilter) && visibilityFilter.getFilterRootElement() !== null)
		{
			...
			else
			{
				...
				if (oFF.notNull(visiblityFilterList))
				{
					...
					{
						var filterParent = visiblityFilterList.getParent();
...
oFF.BlendingProcess.prototype.applyRanks = function()
{
	...
	while (rankSettingsIter.hasNext())
	{
		...
		while (appliedDimensionsIter.hasNext())
		{
			...
			var blendedDimension = oFF.BlendingUtils.getBlendedDimensionFromLeafDimension(appliedDimension.getName(), appliedDimension.getQueryModel().getDatasetId(), blendedQueryModel);
...
oFF.BlendingProcess.prototype.processBlendedDimensionsOnAxis = function(queryManager, axisType)
{
	...
	if (oFF.notNull(axisBlendedDimension))
	{
		...
		{
			var axis = queryManager.getQueryModel().getAxis(axisType);
			axis.clear();
			for (var i = 0; i < axisBlendedDimension.size(); i++)
			{
				var blendedDimensionInfo = axisBlendedDimension.get(i);
				var dimension = oFF.BlendingUtils.getBlendedDimensionFromLeafDimension(blendedDimensionInfo.getDimensionName(), blendedDimensionInfo.getDatasetId(), queryManager.getQueryModel());
				if (oFF.notNull(dimension))
				{
					axis.add(dimension);
				}
...
oFF.BlendingProcess.prototype.prepareQueryExecution = function()
{
	if (oFF.notNull(this.m_blendedQueryManager))
	{
		...
		var signedDataDimension = this.m_blendedQueryManager.getQueryModel().getDimensionByName("SignedData");
		if (oFF.notNull(signedDataDimension))
		{
		...
		this.processBlendedDrills(this.m_blendedQueryManager, this.m_blendedDrillOperations, true);
...
oFF.BlendingProcess.prototype.processThresholds = function()
{
	...
	this.m_blendedQueryManager.getQueryModel().getFormulaExceptionManager().setFormulaResultVisible(true);
...
oFF.BlendingProcess.prototype.getConvertedMeasure = function(formulaException, measure, measureName)
{
	...
	var effectiveMeasure = this.m_blendedQueryManager.getQueryModel().getPrimaryCalculationDimension().getStructureMemberByAliasOrMember(effectiveMeasureName);
...
oFF.BlendableResultSetContainer.prototype.processBlendedQueryManagerCreation = function(syncType, customIdentifier)
{
	this.clearMessages();
	var blendableQueryManager = this.getActionContext();
	var allQueryManagers = blendableQueryManager.getAllQueryManagers();
	oFF.XCollectionUtils.forEach(allQueryManagers,  function(queryManager){
...
{
	var blendedMeasureContainer = new oFF.QBlendedMeasureContainer();
...
oFF.QBlendedMeasureContainer.prototype.copyFromInternal = function(other, flags)
{
	...
	var iterator = origin.m_primaryBlendedMeasures.getIterator();
	while (iterator.hasNext())
	{
...
oFF.QGeoManager.prototype.initialize = function()
{
	...
	this.m_joinFieldNames = oFF.XHashMapOfStringByString.create();
...
oFF.QGeoManager.prototype.getChoroplethExtendedDimension = function(systemName, hierarchyId, levelId, geoDimensionName, parentQueryManager)
{
	...
	var theSystemName = this._getSystemName(systemName);
	var joinFieldKey = this.getJoinFieldKeyByDimensionNameLevelId(theSystemName, parentQueryManager, geoDimensionName, levelId);
	var joinFieldName = this.m_joinFieldNames.getByKey(joinFieldKey);
	var useAreaJoin = this.m_useAreaJoins.getByKey(joinFieldKey);
	if (oFF.isNull(joinFieldName))
	{
		joinFieldName = this.getAreaJoinFieldName(theSystemName, geoDimensionName, parentQueryManager, levelId);
		if (oFF.isNull(joinFieldName))
		{
			joinFieldName = this.getAgileAreaFieldName(theSystemName, geoDimensionName, parentQueryManager, levelId);
			if (oFF.isNull(joinFieldName))
			{
				joinFieldName = this.getUniversalAreaFieldName(theSystemName, geoDimensionName, parentQueryManager, levelId);
			}
		}
	...
	var extendedDimension = oFF.QExtendedDimension.create(extendDimensionName, joinFieldName, useAreaJoin.getBoolean() ? oFF.QGeoConstants.CHOROPLETH_METADATA_AREA_ID : level.getLocation());
	extendedDimension.setDataSource(externalDataSource);
	var joinType = useAreaJoin.getBoolean() ? oFF.JoinType.INNER : oFF.JoinType.CONTAINS;
...
oFF.QGeoManager.prototype.getJoinFieldName = function(geoDimensionName, parentQueryManager, getPointOnly)
{
	...
	if (oFF.notNull(dimension))
	{
		...
		for (var fieldIndex = 0; fieldIndex < dimensionFields.size(); fieldIndex++)
		{
			var field = dimensionFields.getFieldAt(fieldIndex);
...
oFF.QGeoManager.prototype.getAreaJoinFieldName = function(systemName, geoDimensionName, parentQueryManager, levelId)
{
	var joinFieldName = null;
	var geoDimensionPrefix = this.getAreaEnrichedExtendedDimensionNamePrefix(geoDimensionName);
	if (oFF.notNull(geoDimensionPrefix))
	{
		var joinFieldKey = this.getJoinFieldKeyByDimensionNameLevelId(systemName, parentQueryManager, geoDimensionPrefix, levelId);
		...
		if (oFF.isNull(joinFieldName))
		{
			...
			{
				var useAreaJoins = oFF.XBooleanValue.create(true);
				for (var index = 0; index < extendedDimensions.size(); index++)
				{
					var extendedDimension = extendedDimensions.get(index);
					var extendedDimensionName = extendedDimension.getName();
					var prefix = this.getAreaEnrichedExtendedDimensionNamePrefix(extendedDimensionName);
					var dimension = queryModel.getDimensionByName(extendedDimensionName);
					if (oFF.notNull(dimension) && oFF.notNull(prefix) && extendedDimension.getLocationType() === oFF.LocationType.AREA)
					{
						var dimensionFields = dimension.getFields();
						var areaLevelIndex = -1;
						var areaIdFieldName = null;
						for (var fieldIndex = 0; fieldIndex < dimensionFields.size(); fieldIndex++)
						{
							var field = dimensionFields.getFieldAt(fieldIndex);
							var levelIndex = this.getAreaEnrichedPrefixLevelIndex(field);
							if (levelIndex >= 0)
							{
							...
							{
								joinFieldKey = this.getJoinFieldKeyByDimensionNameLevelId(systemName, parentQueryManager, prefix, oFF.XInteger.convertToString(areaLevelIndex + 1));
								if (this.m_joinFieldNames.getByKey(joinFieldKey) === null)
								{
									this.m_joinFieldNames.put(joinFieldKey, areaIdFieldName);
									this.m_useAreaJoins.put(joinFieldKey, useAreaJoins);
								}
								if (areaLevelIndex + 1 === levelNumber && oFF.XString.isEqual(prefix, geoDimensionPrefix))
								{
...
oFF.QGeoManager.prototype.getAgileAreaFieldName = function(systemName, geoDimensionName, parentQueryManager, levelId)
{
	var joinFieldName = null;
	var queryModel = parentQueryManager.getQueryModel();
	var geoDimension = queryModel.getDimensionByName(geoDimensionName);
	var levelNumber = oFF.XInteger.convertFromString(levelId);
	if (oFF.notNull(geoDimension) && levelNumber >= 1 && levelNumber <= oFF.QGeoConstants.AGILE_AREA_SEMANTIC_TYPES.size())
	{
		var joinField = this.getAgileAreaFieldWithSemanticType(geoDimension);
		if (oFF.notNull(joinField))
		{
			var fieldName = joinField.getName();
			var agileAreaSemanticType = oFF.QGeoConstants.AGILE_AREA_SEMANTIC_TYPES.get(levelNumber - 1);
			var semanticType = joinField.getSemanticType();
			...
			{
				var dimension = queryModel.getDimensionByName(fieldName);
				if (oFF.notNull(dimension))
				{
					var groupingDimensionNames = dimension.getGroupingDimensionNames();
					if (oFF.notNull(groupingDimensionNames))
					{
						var joinFieldKey = null;
						var useAreaJoins = oFF.XBooleanValue.create(true);
						for (var index = 0; index < groupingDimensionNames.size(); index++)
						{
							var groupingDimensionName = groupingDimensionNames.get(index);
							var groupingDimension = queryModel.getDimensionByName(groupingDimensionName);
							...
							{
								joinFieldKey = this.getJoinFieldKeyByDimensionNameLevelId(systemName, parentQueryManager, groupingDimensionName, levelId);
								joinFieldName = this.m_joinFieldNames.getByKey(joinFieldKey);
								...
								{
									var numberOfJoinFieldsFound = 0;
									var dimensionFields = groupingDimension.getFields();
									for (var fieldIndex = 0; fieldIndex < dimensionFields.size(); fieldIndex++)
									{
										var field = dimensionFields.getFieldAt(fieldIndex);
										...
										{
											var levelIndex = oFF.QGeoConstants.AGILE_AREA_SEMANTIC_TYPES.getIndex(semanticType);
											if (levelIndex >= 0)
											{
												joinFieldKey = this.getJoinFieldKeyByDimensionNameLevelId(systemName, parentQueryManager, groupingDimensionName, oFF.XInteger.convertToString(levelIndex + 1));
												if (this.m_joinFieldNames.getByKey(joinFieldKey) === null)
												{
...
oFF.QGeoManager.prototype.getUniversalAreaFieldName = function(systemName, geoDimensionName, parentQueryManager, levelId)
{
	var joinFieldName = null;
	var dimension = parentQueryManager.getQueryModel().getDimensionByName(geoDimensionName);
	var levelNumber = oFF.XInteger.convertFromString(levelId);
	if (oFF.notNull(dimension) && levelNumber >= 1 && levelNumber <= oFF.QGeoConstants.UNIVERSAL_AREA_SEMANTIC_TYPES.size())
	{
		...
		var useAreaJoins = oFF.XBooleanValue.create(true);
		var dimensionFields = dimension.getFields();
		for (var index = 0; index < dimensionFields.size(); index++)
		{
			var field = dimensionFields.getFieldAt(index);
			var semanticType = field.getSemanticType();
			if (oFF.notNull(semanticType))
			{
				var levelIndex = oFF.QGeoConstants.UNIVERSAL_AREA_SEMANTIC_TYPES.getIndex(semanticType);
				if (levelIndex >= 0)
				{
					var fieldName = field.getName();
					var joinFieldKey = this.getJoinFieldKeyByDimensionNameLevelId(systemName, parentQueryManager, geoDimensionName, oFF.XInteger.convertToString(levelIndex + 1));
					if (this.m_joinFieldNames.getByKey(joinFieldKey) === null)
					{
...
oFF.QGeoManager.prototype.getAreaEnrichedPrefixLevelIndex = function(field)
{
	...
	if (field.getValueType().isSpatial())
	{
		var upperCaseFieldName = oFF.XString.toUpperCase(field.getName());
		for (var index = 0; index < oFF.QGeoConstants.AREA_ENRICHED_PREFIXES.size(); index++)
		{
...
oFF.QGeoManager.prototype.getAreaEnrichedExtendedDimensionNamePrefix = function(dimensionName)
{
	var prefix = null;
	var index = oFF.XString.indexOf(dimensionName, "_");
	if (index >= 0)
	{
		index = oFF.XString.indexOfFrom(dimensionName, "_", index + 1);
		if (index >= 1)
		{
			prefix = oFF.XString.substring(dimensionName, 0, index);
		}
	}
	return prefix;
};
...
oFF.QGeoManager.prototype.getJoinFieldKeyByDimensionNameLevelId = function(systemName, parentQueryManager, dimensionName, levelId)
{
	var datasetId = parentQueryManager.getQueryModel().getDatasetId();
	return oFF.XStringUtils.concatenate4(systemName, datasetId, dimensionName, levelId);
};
	...
	else
	{
		...
		if (oFF.isNull(drillPathResult))
		{
			...
			for (var index = 0; index < sortedLevelIds.size(); index++)
			{
				...
				var level = this.getChoroplethHierarchyLevel(theSystemName, hierarchyId, levelId);
				if (oFF.isNull(level))
				{
...
oFF.QModelDimensionLinksManager.prototype.addDimensionLinkFromDimensionParts = function(storageName, firstDimensionLinkPart, secondDimensionLinkPart, linkOnMatchingHierarchies)
{
	...
	var modelDimensionLinks = this.getModelDimensionLinksStorage(storageName, firstDatasetId, secondDatasetId);
...
{
	var keyRefStoreContext = oFF.QFactory.createKeyRefStoreContext(this, oFF.KeyRefConstants.MAIN_STORAGE);
...
oFF.AbstractBlendableQueryManager.prototype.cloneQueryManagerExt = function(cloneMode)
{
	...
	var keyRefStoreContext = oFF.QFactory.createKeyRefStoreContext(this.getOlapEnv().getModelLinkManager(), this.getClientQueryObjectStorageName());
...
oFF.BlendableQueryManager.prototype.addQueryManager = function(queryManager)
{
	...
	var rowsIter = queryModel.getRowsAxis().getIterator();
	while (rowsIter.hasNext())
	{
		this.addBlendedDimensionInfoToAxis(oFF.AxisType.ROWS, datasetId, rowsIter.next().getName());
	}
	var columnsIter = queryModel.getColumnsAxis().getIterator();
...
oFF.BlendableQueryManager.prototype.removeSecondaryQueryManager = function(datasetId)
{
	...
	var blendedDimensionOrderIter = this.m_blendedDimensionOrder.getKeysAsIteratorOfString();
	while (blendedDimensionOrderIter.hasNext())
	{
		var dimensionInfos = this.m_blendedDimensionOrder.getByKey(blendedDimensionOrderIter.next());
		for (var i = dimensionInfos.size() - 1; i >= 0; i--)
		{
			if (oFF.XString.isEqual(dimensionInfos.get(i).getDatasetId(), datasetId))
			{
				dimensionInfos.removeAt(i);
			}
		}
	}
	for (var j = this.m_blendedDrillOperations.size() - 1; j >= 0; j--)
	{
	...
	var blendableDimensionSorts = this.m_sortContainer.getBlendableDimensionSorts();
	for (var k = blendableDimensionSorts.size() - 1; k >= 0; k--)
	{
	...
	for (var l = this.m_rankSettings.size() - 1; l >= 0; l--)
	{
	...
	var removedBlendedMeasure = oFF.XList.create();
	var primaryBlendedMeasures = this.m_primaryBlendedMeasureContainer.getPrimaryBlendedMeasures();
	for (var m = primaryBlendedMeasures.size() - 1; m >= 0; m--)
	{
		var blendedMeasure = primaryBlendedMeasures.get(m);
		if (oFF.XCollectionUtils.contains(oFF.BlendingUtils.getFormulaItemDimensionContexts(blendedMeasure),  function(dimensionContext){
	...
	for (var n = this.m_formulaExceptions.size() - 1; n >= 0; n--)
	{
		var formulaException = this.m_formulaExceptions.get(n);
		if (oFF.XString.isEqual(formulaException.getQueryModel().getDatasetId(), datasetId) || removedBlendedMeasure.contains(formulaException.getMeasure()))
		{
			...
			this.m_formulaExceptions.removeAt(n);
		}
	}
};
...
{
	var activePrimaryLinkedDimensionNames = oFF.XHashSetOfString.create();
	activePrimaryLinkedDimensionNames.addAll(this.getActivePrimaryLinkDimensionNamesSettings(secondaryDatasetId));
	var modelLinks = this.getModelLinks(this.getOlapEnv().getModelLinkManager(), this.m_primaryDatasetId, secondaryDatasetId);
	var primaryQueryModel = this.getPrimaryQueryManager().getQueryModel();
	oFF.XCollectionUtils.forEach(modelLinks,  function(modelLink){
		var dimension = primaryQueryModel.getDimensionByName(modelLink.getFirstPart().getDimensionName());
	...
	return activePrimaryLinkedDimensionNames.getValuesAsReadOnlyListOfString();
};
...
oFF.BlendableQueryManager.prototype.getBlendingProcessConfigs = function(validationResult)
{
	...
	if (this.m_blendingRestrictionsEnabled && !oFF.BlendingUtils.isQueryModelInUseForBlending(primaryQueryManager.getQueryModel(), this, false))
	{
		var primaryNotInUseArgs = oFF.XListOfString.create();
		primaryNotInUseArgs.add(primaryQueryManager.getQueryModel().getDatasetShortDescriptionOrName());
	...
	while (queryModelLinkSettingsIterator.hasNext())
	{
		...
		if (oFF.isNull(secondaryQueryManager) || !oFF.BlendingUtils.isQueryModelInUseForBlending(secondaryQueryManager.getQueryModel(), this, true))
		{
			var secondaryQMNotInUseArgs = oFF.XListOfString.create();
			secondaryQMNotInUseArgs.add(secondaryQueryManager.getQueryModel().getDatasetShortDescriptionOrName());
		...
		var modelLinks = this.getModelLinks(modelLinkManager, primaryDatasetId, secondDatasetId);
		if (modelLinks.size() > 0)
		{
			...
			var primaryLinkDimensionFound = !this.m_blendingRestrictionsEnabled;
			...
			{
				linkDimensionNotInDrillArgs = oFF.XListOfString.create();
				linkDimensionNotInDrillArgs.add(secondDatasetId);
...
{
	var modelLinks = oFF.XCollectionUtils.filter(modelLinkManager.getDimensionLinksByDatasetIds(this.getClientQueryObjectStorageName(), primaryDatasetId, secondDatasetId, true),  function(dimensionLink){
		var firstPart = dimensionLink.getFirstPart();
		var secondPart = dimensionLink.getSecondPart();
		var firstDimension = firstPart.getDimensionAccessor().getDimensionByName(firstPart.getDimensionName());
		var isFirstPartFlexibleTime = oFF.QTimeDimensionUtil.isFlexibleTimeDimension(firstDimension) || oFF.QTimeDimensionUtil.hasFlexibleTimeGroupingDimension(firstDimension);
		var secondDimension = secondPart.getDimensionAccessor().getDimensionByName(secondPart.getDimensionName());
		var isSecondPartFlexibleTime = oFF.QTimeDimensionUtil.isFlexibleTimeDimension(secondDimension) || oFF.QTimeDimensionUtil.hasFlexibleTimeGroupingDimension(secondDimension);
...
{
	var isEventingPaused = false;
	for (var i = 0; i < changedNodes.size(); i++)
	{
		...
		if (oFF.notNull(componentType))
		{
			...
			{
				var axis = changedNode;
				var dimensionNamesInAxis = oFF.XListOfString.create();
				dimensionNamesInAxis.addAll(axis.getDimensionNames());
				var datasetId = axis.getQueryModel().getDatasetId();
				for (var j = 0; j < dimensionNamesInAxis.size(); j++)
				{
					var dimensionName = dimensionNamesInAxis.get(j);
					...
					{
						var blendedDimensionInfos = oFF.XList.create();
						blendedDimensionInfos.addAll(this.m_blendedDimensionOrder.getByKey(axis.getType().getName()));
						var isDimensionFound = false;
						for (var k = blendedDimensionInfos.size() - 1; k >= 0; k--)
						{
							var blendedDimensionInfo = blendedDimensionInfos.get(k);
...
oFF.BlendableQueryManager.prototype.addBlendedDimensionInfoToAxis = function(axis, datasetId, dimensionName)
{
	...
	var newBlendedDimensionInfo = this.insertBlendedDimensionInfoToAxisInternal(axis, datasetId, dimensionName, oFF.notNull(blendedDimensionInfo) ? blendedDimensionInfo.size() : 0);
...
oFF.BlendableQueryManager.prototype.insertBlendedDimensionInfoToAxisInternal = function(axis, datasetId, dimensionName, index)
{
	...
	if (oFF.notNull(queryManager))
	{
		...
		if (oFF.notNull(dimension))
		{
			...
			var blendedDimensionInfoList = this.m_blendedDimensionOrder.getByKey(axis.getName());
...
{
	var blendedDimensionOrderLists = this.m_blendedDimensionOrder.getValuesAsReadOnlyList();
	var blendedDimensionOrderListsIter = blendedDimensionOrderLists.getIterator();
	while (blendedDimensionOrderListsIter.hasNext())
	{
		var blendedDimensionOrderList = blendedDimensionOrderListsIter.next();
		var matchBlendedDimensionInfo = oFF.XCollectionUtils.findFirst(blendedDimensionOrderList,  function(blendedDimensionInfo){
...
oFF.BlendableQueryManager.prototype.setUseEmptyAllMemberMeasureFilterByDatasetId = function(useEmptyAllMemberMeasureFilter, datasetId)
{
	if (oFF.XString.isEqual(datasetId, this.m_primaryDatasetId))
	{
		this.setUseEmptyAllMemberMeasureFilter(useEmptyAllMemberMeasureFilter);
	}
	var queryModelLinkSettings = this.getQueryModelLinkSettings(datasetId);
	if (oFF.notNull(queryModelLinkSettings))
	{
		queryModelLinkSettings.setUseEmptyAllMemberMeasureFilter(useEmptyAllMemberMeasureFilter);
	}
};
oFF.BlendableQueryManager.prototype.isUseEmptyAllMemberMeasureFilterByDatasetId = function(datasetId)
{
	if (oFF.XString.isEqual(datasetId, this.m_primaryDatasetId) || this.m_useEmptyAllMemberMeasureFilter)
	{
	...
	if (oFF.notNull(queryModelLinkSettings))
	{
		return queryModelLinkSettings.isUseEmptyAllMemberMeasureFilter();
	}
	return false;
};
...
{
	oFF.XCollectionUtils.forEach(this.m_formulaExceptions,  function(formulaException){
		formulaException.removeSecondaryParent(this);
	}.bind(this));
...
{
	var datasetIds = oFF.XListOfString.create();
	oFF.XCollectionUtils.forEach(this.m_queryModelLinkSettings,  function(queryModelLinks){
