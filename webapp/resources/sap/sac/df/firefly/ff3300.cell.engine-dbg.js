...
{
	var instance = new oFF.XCellEngineCellProvider();
...
oFF.XCellEngineCellProvider.prototype.setupCellProvider = function(spreadsheet)
{
	...
	for (var i = 0; i < 100; i++)
	{
		...
		for (var j = 0; j < 26; j++)
		{
			var cell = oFF.XCellEngineCell.createWithSpreadsheet(this.m_spreadsheet);
			rows.add(cell);
		}
	}
};
...
{
	var instance = new oFF.XSpreadsheetCellAccessor();
...
{
	var baseCells = this.m_baseProvider.getCells();
	var keyIterator = this.m_extraProviders.getKeysAsReadOnlyList().getIterator();
	while (keyIterator.hasNext())
	{
		var key = keyIterator.next();
		var row = key.getRow();
		var column = key.getColumn();
		for (var rowIndex = row; rowIndex - row < this.m_extraProviders.getByKey(key).getCells().size(); rowIndex++)
		{
			for (var columnIndex = column; columnIndex - column < this.m_extraProviders.getByKey(key).getCells().get(rowIndex - row).size(); columnIndex++)
			{
...
{
	var instance = new oFF.XCellEngineCell();
	instance.m_spreadsheet = spreadsheet;
...
oFF.XCellEngineCell.prototype.evaluate = function()
{
	...
	{
		var compiler = oFF.XSpreadsheetInterpreter.create(this.m_spreadsheet.getCellAccessor());
