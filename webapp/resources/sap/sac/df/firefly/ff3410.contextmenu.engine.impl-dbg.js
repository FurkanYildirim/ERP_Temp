...
{
	var result = oFF.CMEMenuTreePopulatorSelfRef.create(widgetsFactory, uiCache);
	result.setShowNames( function(){
...
{
	var result = oFF.CMEMenuTreePopulatorBranching.createWithSubMapper(widgetsFactory, subMapper, uiCache);
	result.setShowNames( function(){
		return this.m_showNamesInMenu;
	}.bind(this));
	return result;
...
oFF.CMEMenuTreeGenerator.prototype.loadConfiguration = function(baseConfiguration)
{
	...
	{
		var menuGenerator = oFF.CMECreatorJsonHelper.createMenuGenerator(menus, subMenuMap, oFF.CMEFactory.getRegistry(), this.m_actionController);
...
oFF.CMEMenuTreeGenerator.prototype.dynamicallyExtendMenu = function(groupingMenuItem, inputStructure)
{
	...
	{
		var menuExtender = oFF.CMECreatorJsonHelper.createMenuExtender(menuMenuExtensions, subMenuMap, oFF.CMEFactory.getRegistry(), this.m_actionController);
...
oFF.CMEMenuTreePopulatorAbstract.prototype.remapMenuTree = function(container, abstractMenuTree, defaultOverflowPriority)
{
	...
	var oldMenuTree = this.m_uiCache.getMenuItem(container, abstractMenuTree.getNameOrFallback());
	var subItems = abstractMenuTree.getEffectiveSubItems();
	var subAbstractItem;
	var subItem;
	var i;
	var compatible = !abstractMenuTree.isFlat() && this.m_factory.supportsSynchronization() && oFF.notNull(oldMenuTree) && oldMenuTree.getActionGroup() !== null && oldMenuTree.getActionGroup().areAllEffectiveSubItemsCompatibleTo(subItems);
	if (compatible)
	{
		var index = 0;
		for (i = 0; i < subItems.size(); i++)
		{
			index = this.synchronizeMenuItem(container, index, subItems.get(i), defaultOverflowPriority);
		}
	}
	else
	{
		...
		this.m_uiCache.registerUiItem(container, abstractMenuTree.getNameOrFallback(), abstractMenuTree,  function(){
			this.m_factory.clearGroup(container);
		}.bind(this));
			...
			else if (oFF.notNull(subItem) && subItem.isVisible())
			{
				...
				else
				{
					...
					if (oFF.notNull(group))
					{
						...
						var hasDefaultAction = group.isFirstSubOptionIsDefault();
						if (hasDefaultAction)
						{
							var groupItems = oFF.XStream.of(group.getSubItems()).filter( function(subI){
								return subI.getActionLeaf() !== null && subI.getActionLeaf().isVisible() && subI.getActionLeaf().isEnabled();
							}.bind(this)).map( function(subIt){
								return subIt.getActionLeaf();
							}.bind(this)).collect(oFF.XStreamCollector.toList());
							defaultAction = groupItems.size() === 0 ? null : groupItems.get(0).getCommand();
						}
						var menuItem = this.m_factory.createGroupItem(container, sectionStart, overflowPriority, subItem.getName(), this.getTextForItem(subItem), subItem.getIcon(), subItem.isEnabled(), subItem.getExplanation(), subItem.getHighlightProcedure(), subItem.getUnHighlightProcedure(), hasDefaultAction, defaultAction);
					...
					if (oFF.notNull(leaf))
					{
						if (leaf.isToggle())
						{
							var toggleItem = this.m_factory.createToggleItem(container, sectionStart, overflowPriority, subItem.getName(), this.getTextForItem(subItem), subItem.getIcon(), subItem.isEnabled(), subItem.getExplanation(), subItem.getHighlightProcedure(), subItem.getUnHighlightProcedure(), leaf.getCommand(), leaf.getActiveExtended(), leaf.getStateIcon());
							this.m_uiCache.registerSubUiItem(container, abstractMenuTree.getRootName(), toggleItem);
						}
						else
						{
							var triggerItem = this.m_factory.createTriggerItem(container, sectionStart, overflowPriority, subItem.getName(), this.getTextForItem(subItem), subItem.getIcon(), subItem.isEnabled(), subItem.getExplanation(), subItem.getHighlightProcedure(), subItem.getUnHighlightProcedure(), leaf.getCommand());
...
oFF.CMEMenuTreePopulatorAbstract.prototype.synchronizeMenuItem = function(container, index, subAbstractItem, defaultOverflowPriority)
{
	var currentIndex = index;
	var subItem = subAbstractItem.getMenuItem();
	if (oFF.notNull(subItem))
	{
		var uiItem = this.m_uiCache.getSubUiItem(container, subAbstractItem.getRootName(), index);
		var leaf = subItem.getActionLeaf();
		var group = subItem.getActionGroup();
		if (oFF.notNull(leaf))
		{
			if (leaf.isToggle())
			{
				this.m_factory.synchronizeToggleMenuItem(uiItem, this.getTextForItem(subItem), subItem.getIcon(), subItem.isEnabled(), subItem.getExplanation(), subItem.getHighlightProcedure(), subItem.getUnHighlightProcedure(), leaf.getCommand(), leaf.getActiveExtended(), leaf.getStateIcon());
			}
			else
			{
				this.m_factory.synchronizeTriggerMenuItem(uiItem, this.getTextForItem(subItem), subItem.getIcon(), subItem.isEnabled(), subItem.getExplanation(), subItem.getHighlightProcedure(), subItem.getUnHighlightProcedure(), leaf.getCommand());
			}
			currentIndex++;
		}
		if (oFF.notNull(group))
		{
			if (oFF.notNull(uiItem) && !group.isFlat())
			{
				...
				this.m_factory.synchronizeGroupMenuItem(uiItem, this.getTextForItem(subItem), subItem.getIcon(), subItem.isEnabled(), subItem.getExplanation(), subItem.getHighlightProcedure(), subItem.getUnHighlightProcedure(), hasDefaultAction, defaultAction);
			}
			if (group.isFlat())
			{
				var effectiveSubItems = group.getEffectiveSubItems();
				for (var i = 0; i < effectiveSubItems.size(); i++)
				{
					currentIndex = this.synchronizeMenuItem(container, currentIndex, effectiveSubItems.get(i), defaultOverflowPriority);
				}
			}
			else if (oFF.notNull(uiItem))
			{
				this.getSubMapper().remapMenuTree(uiItem, group, defaultOverflowPriority);
				currentIndex++;
			}
		}
	}
...
{
	var instance = new oFF.CMEUiCache();
	instance.setup();
	return instance;
};
...
{
	oFF.XCollectionUtils.forEach(this.m_cleaningFactory.getValuesAsReadOnlyList(),  function(clu){
		try
		{
			this.cleanupAll(clu);
		}
	...
	oFF.XCollectionUtils.forEach(this.m_cleaningFactory.getValuesAsReadOnlyList(),  function(pro){
		oFF.XCollectionUtils.releaseEntriesAndCollectionIfNotNull(pro);
	}.bind(this));
	oFF.XCollectionUtils.forEach(this.m_cleaningFactory.getKeysAsReadOnlyList(),  function(k0){
		oFF.XObjectExt.release(k0);
	}.bind(this));
	this.m_cleaningFactory = oFF.XObjectExt.release(this.m_cleaningFactory);
	oFF.XCollectionUtils.forEach(this.m_subItemsRegistry.getValuesAsReadOnlyList(),  function(val){
		oFF.XCollectionUtils.releaseEntriesAndCollectionIfNotNull(val);
	}.bind(this));
	oFF.XCollectionUtils.forEach(this.m_subItemsRegistry.getKeysAsReadOnlyList(),  function(k1){
		oFF.XObjectExt.release(k1);
	}.bind(this));
	this.m_subItemsRegistry = oFF.XObjectExt.release(this.m_subItemsRegistry);
	oFF.XCollectionUtils.forEach(this.m_uiItemRegistry.getValuesAsReadOnlyList(),  function(uit){
		oFF.XCollectionUtils.releaseEntriesAndCollectionIfNotNull(uit);
	}.bind(this));
	oFF.XCollectionUtils.forEach(this.m_uiItemRegistry.getKeysAsReadOnlyList(),  function(k2){
...
{
	oFF.XCollectionUtils.forEach(clu,  function(item){
		item.execute();
	}.bind(this));
};
...
oFF.CMEUiCache.prototype.prepareSubUiItemRegistry = function(uiObject, rootName)
{
	...
	{
		var subMap = this.m_subItemsRegistry.getByKey(uiObject);
...
oFF.CMESelectionOption.prototype.isSelectOption = function()
{
	return true;
};
...
oFF.CMEOperationCreator.prototype.getMatchingTopMenuName = function(parameters)
{
	return this.getName();
};
...
{
	return this.getName() === null ? oFF.XCollectionUtils.join(this.getTags().getValuesAsReadOnlyListOfString(), ":") : this.getName();
};
...
{
	var otherSeparator = other.getMenuSeparator();
...
{
	var compatible;
	var otherGroup = other.getActionGroup();
	if (oFF.isNull(otherGroup) || !oFF.XString.isEqual(this.getName(), other.getName()))
	{
		compatible = false;
	}
...
oFF.CMEGroupingMenuItem.prototype.areAllEffectiveSubItemsCompatibleTo = function(effectiveSubItems)
{
	...
	var thisSubItems = this.getEffectiveSubItems();
	if (thisSubItems.size() === effectiveSubItems.size())
	{
		compatible = true;
		for (var i = 0; i < thisSubItems.size(); i++)
		{
			var tsi = thisSubItems.get(i);
			var osi = effectiveSubItems.get(i);
...
{
	var otherLeaf = other.getActionLeaf();
...
oFF.CMEConditionalMenuCreator.prototype.getMatchingTopMenuName = function(parameters)
{
	...
	var conditionFulFilled = this.checkCondition(this.getSubContext(parameters), null);
