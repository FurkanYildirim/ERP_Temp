...
{
	var type = this.getOriginDataType();
...
oFF.ODataResult.prototype.parseResult = function(response)
{
	...
	{
		var propertyD = oFF.PrUtils.getProperty(response, oFF.ProcessorODataConstants.RS_D);
		if (propertyD.isList())
		{
...
{
	var navPropertyStructure = structure.getElementTypeByKey(nestedPropertyName) === oFF.PrElementType.STRUCTURE ? structure.getStructureByKey(nestedPropertyName) : null;
...
oFF.InARequestParser.prototype.parseStructureMembers = function(measureStructure)
{
	...
	{
		var includedMembers = oFF.XListOfString.create();
		var excludedMembers = oFF.XListOfString.create();
		this.getFilteredStructureMembers(includedMembers, excludedMembers, this.getFilterSelection());
		return oFF.XStream.of(members).map( function(m1){
			return m1.asStructure();
		}.bind(this)).filter( function(m2){
	...
	else
	{
		...
		if (oFF.XString.isEqual(fieldName, oFF.ProcessorConstants.CUSTOM_DIM_KEY) && !oFF.PrUtils.isListEmpty(elements))
		{
			for (var k = 0; k < elements.size(); k++)
			{
				...
				var filterValue = oFF.PrUtils.getStringValueProperty(element, "Low", "");
...
{
	var inaField = this.createField(oFF.ProcessorConstants.CUSTOM_DIM_KEY, oFF.ProcessorConstants.CUSTOM_DIM_KEY_DESCRIPTION, oFF.ProcessorConstants.CUSTOM_DIM, "String", -1, property, true);
...
{
	var inaField = this.createField(oFF.ProcessorConstants.CUSTOM_DIM_TEXT, oFF.ProcessorConstants.CUSTOM_DIM_TEXT_DESCRIPTION, oFF.ProcessorConstants.CUSTOM_DIM, "String", -1, property, true);
...
{
	var inaField = this.createField(property.getName(), property.getDescription(), dimensionName, property.getDataType(), property.getMaxLength(), property, false);
	inaField.putBoolean("IsKey", true);
	inaField.putString("PresentationType", "Key");
	return inaField;
};
oFF.InAMetadataBuilder.prototype.addDimensionTextField = function(property, inaDimension, isDefaultTextField)
{
	...
	var inaField = this.createField(name, property.getDescription(), inaDimension.getStringByKey("Name"), property.getDataType(), property.getMaxLength(), property, false);
	inaField.putString("PresentationType", "Text");
...
oFF.ODataMetadataConverter.prototype.createInADimensionsFromEntity = function(schemas, entityName)
{
	for (var i = 0; i < schemas.size(); i++)
	{
		...
		if (oFF.notNull(entitySet))
		{
			...
			var dimensions = this.createInADimensions(entityType, schemas);
...
{
	var complexType = this.getComplexType(schemas, complexTypeName);
	var properties = oFF.PrUtils.getListProperty(complexType, oFF.ProcessorODataConstants.MD_PROPERTY);
	if (oFF.notNull(properties))
	{
		oFF.XStream.of(properties).filter( function(property){
			return property.isStructure();
		}.bind(this)).map( function(structure){
			var struct = structure.asStructure();
		...
		}.bind(this)).forEach( function(prop){
...
oFF.ODataMetadataConverter.prototype.addDimensionAttribute = function(inaDimension, fieldPrefix, prefixAttributeWithDimName, property)
{
	...
	var attributeText = oFF.XStringUtils.concatenate2(fieldPrefix, property.getNameWithoutPrefix());
	var attributeName = oFF.XStringUtils.concatenate2(fieldPrefix, property.getName());
...
oFF.ODataMetadataConverter.prototype.getTypeStructure = function(schemas, structureName, tagName)
{
	...
	{
		var schema = schemas.getStructureAt(i);
		var nameWithoutNamespace = this.removeNamespace(schema, structureName);
		var entityTypes = oFF.PrUtils.getPropertyAsList(schema, tagName);
...
oFF.ODataResultSetUrl.prototype.addDimensionAttributesToSelection = function(selection, dimension)
{
	...
	if (!oFF.PrUtils.isListEmpty(attributes))
	{
		...
		for (var i = 0; i < attributes.size(); i++)
		{
			...
			var isKeyAttribute = oFF.XString.isEqual(dimName, attributeName);
	...
	if (oFF.XStringUtils.isNotNullAndNotEmpty(fieldName) && !isMeasureFilter && !oFF.PrUtils.isListEmpty(elements))
	{
		...
		for (var i = 0; i < elements.size(); i++)
		{
			...
			else if (oFF.XString.isEqual(comparisonValue, "MATCH") && oFF.notNull(low))
			{
				...
				var substringFilter = oFF.ProcessorODataConstants.URI_FILTER_SUBSTRING;
