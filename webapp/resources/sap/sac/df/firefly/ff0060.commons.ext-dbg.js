...
oFF.XDateRange.prototype.compute = function()
{
	if (oFF.isNull(this.m_from) || oFF.isNull(this.m_to))
	{
		...
		{
			var timezoneOffset = oFF.notNull(this.m_dateTimeProvider) ? this.m_dateTimeProvider.getLocalTimezoneOffset() : oFF.XSystemUtils.getSystemTimezoneOffsetInMilliseconds();
...
oFF.XEnvironmentConstants = {
	...
	FIREFLY_PROGRAM_CFG_FIRMWARE:"ff_prg_cfg_firmware",
	FIREFLY_PROGRAM_CFG_ADMIN:"ff_prg_cfg_admin",
	FIREFLY_PROGRAM_CFG_PERSONALIZATION:"ff_prg_cfg_personalization",
...
oFF.XNumberFormatter = {
	...
	formatHeaderStringUsingSettingsWithCurrency:function(numberFormatterSettings, currency, scaleFactor, scaleTexts)
	{
		...
		var formattedValue = "";
		var suffix = currency.getSecondString();
		var currencyFormatSettings = numberFormatterSettings.getCurrencyFormatSettings();
		var valuePosition = oFF.isNull(currencyFormatSettings) ? 0 : currencyFormatSettings.getValuePosition();
		var scaleTextPosition = oFF.isNull(currencyFormatSettings) ? 1 : currencyFormatSettings.getScaleTextPosition();
		var currencyPresentationPosition = oFF.isNull(currencyFormatSettings) ? 2 : currencyFormatSettings.getCurrencyPresentationPosition();
		var currencyPresentationHasSpace = oFF.isNull(currencyFormatSettings) || currencyFormatSettings.currencyPresentationHasSpace();
		var scaleTextHasSpace = oFF.isNull(currencyFormatSettings) || currencyFormatSettings.scaleTextHasSpace();
		var currencyPresentation = currency.getFirstString();
		if (oFF.XStringUtils.isNotNullAndNotEmpty(currencyPresentation) && currencyPresentationHasSpace)
		{
			var spaceIsPrefixed = currencyPresentationPosition > valuePosition;
			currencyPresentation = oFF.XNumberFormatter.concatSpace(currencyPresentation, spaceIsPrefixed);
		}
		var scaleFormatTuple = oFF.XNumberFormatter.getScaleText(scaleFactor, numberFormatterSettings.getScaleFormat(), scaleTexts);
		var scaleText = scaleFormatTuple.getName();
		if (oFF.XStringUtils.isNotNullAndNotEmpty(scaleText) && scaleTextHasSpace)
		{
			var spaceIsPrefixed2 = scaleTextPosition > valuePosition;
	...
	formatDoubleToStringUsingSettingsWithCurrency:function(value, numberFormatterSettings, currencyText, scaleTexts, defaultExcelFormat, suppressScaleText)
	{
		...
		if (oFF.notNull(currencyFormatSettings))
		{
			...
			var scaleText = suppressScaleText ? "" : scaleFormatTuple.getName();
	...
	moveDecimalRight:function(number, rightShiftCount)
	{
		...
		{
			var first = oFF.XString.substring(afterDecimal, 0, rightShiftCount);
			var second = oFF.XString.substring(afterDecimal, rightShiftCount, oFF.XString.size(afterDecimal));
	...
	standardizeFormatString:function(formatStr, isBW, numeric)
	{
			var result = "";
		if (oFF.XStringUtils.isNullOrEmpty(formatStr))
		{
			result = "";
		}
		else if (numeric)
		{
			...
			for (var i = 0; i < oFF.XString.size(formatStr); i++)
			{
				...
				if (isBW && c === oFF.XString.getCharAt(",", 0))
				{
					result = oFF.XStringUtils.concatenate3(result, oFF.XString.substring(formatStr, lastProcessedIdx, i), ".");
					lastProcessedIdx = i + 1;
				}
				else if (isBW && c === oFF.XString.getCharAt(".", 0))
				{
					result = oFF.XStringUtils.concatenate3(result, oFF.XString.substring(formatStr, lastProcessedIdx, i), ",");
					lastProcessedIdx = i + 1;
				}
				else if (c !== oFF.XString.getCharAt("#", 0) && c !== oFF.XString.getCharAt("0", 0) && c !== oFF.XString.getCharAt(",", 0) && c !== oFF.XString.getCharAt(".", 0))
				{
					result = oFF.XStringUtils.concatenate2(result, oFF.XString.substring(formatStr, lastProcessedIdx, i));
					lastProcessedIdx = i + 1;
				}
			}
			result = oFF.XStringUtils.concatenate2(result, oFF.XString.substring(formatStr, lastProcessedIdx, -1));
			while (oFF.XString.size(result) > 0 && oFF.XString.getCharAt(result, oFF.XString.size(result) - 1) === oFF.XString.getCharAt(",", 0))
			{
				result = oFF.XString.substring(result, 0, oFF.XString.size(result) - 1);
			}
		}
		else
		{
...
oFF.XTime.createTimeFromSAPFormat = function(time)
{
	...
	if (oFF.XStringUtils.isNotNullAndNotEmpty(time))
	{
		...
		{
			result = oFF.XTime.createEmptyTime();
		}
		else
		{
...
oFF.XDate.createDateFromSAPFormat = function(date)
{
	...
	{
		result = oFF.XDate.createEmptyDate();
	}
	return result;
};
...
oFF.XDateTime.createDateTimeFromSAPFormat = function(dateTime)
{
	...
	if (oFF.XStringUtils.isNotNullAndNotEmpty(dateTime))
	{
		if (oFF.XString.size(dateTime) >= 8)
		{
			...
			if (oFF.notNull(date))
			{
				var resultInternal = new oFF.XDateTime();
				resultInternal.m_date = date;
				if (oFF.XString.size(dateTime) <= oFF.XString.size(dateString))
				{
					resultInternal.m_time = oFF.XTime.createEmptyTime();
				}
				else
				{
		...
		{
			result = oFF.XDateTime.createEmptyDateTime();
		}
	}
	return result;
};
oFF.XDateTime.createDateTimeSafe = function(dateTime)
{
	...
	if (oFF.XStringUtils.isNotNullAndNotEmpty(dateTime))
	{
		...
		{
			date = oFF.XDate.createEmptyDate();
