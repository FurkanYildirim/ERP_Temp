...
{
	var lovRestoreSettings = new oFF.LovSortRestoreSettings();
...
oFF.LovSortRestoreSettings.prototype.setupRestoreSettings = function(dimension, useSelectorReadMode, useSelectorInitialDrillLevel)
{
	if (oFF.notNull(dimension))
	{
		...
		var conditionsManager = oFF.notNull(queryModel) ? queryModel.getConditionManager() : null;
		if (oFF.notNull(queryModel) && oFF.notNull(conditionsManager) && conditionsManager.hasConditions())
		{
			this.m_conditions = queryModel.getConditionManager().serializeToElement(oFF.QModelFormat.INA_REPOSITORY_DATA);
...
oFF.LovSortRestoreSettings.prototype.restore = function(dimension)
{
	if (oFF.notNull(dimension))
	{
		...
		{
			queryModel.getConditionManager().deserializeFromElementExt(oFF.QModelFormat.INA_REPOSITORY_DATA, this.m_conditions);
		}
	}
};
...
oFF.LovProcess.prototype.revertQueryChanges = function()
{
	...
	{
		this.m_lovSortRestoreSettings.restore(this.getDimension());
	}
	dimension.resumeEventing();
};
...
{
	var isBw = this.getQueryManager() !== null && this.getQueryManager().getConvenienceCommands().isBw();
	var isResultSetForcedForSort = this.m_lovProcessConfig.requiresResultSetRequest() && this.m_lovProcessConfig.isResultSetForcedForSort();
...
oFF.RsCursorAxis.prototype.addNextFieldMetadata = function(fieldName, fieldText, isVisible, presentationType, valueType, mimeType, isHierarchyNavigationField)
{
	...
	var rsField = this.m_currentRsDimension.addRsField2(fieldName, fieldText, presentationType === oFF.PresentationType.UNDEFINED ? field.getPresentationType() : presentationType, valueType, field.getMimeType(), field.isHierarchyNavigationField());
	if (isVisible)
	{
...
oFF.RsDefField.create = function(name, text, presentationType, valueType, rsDefDimension, mimeType, isHierarchyNavigationField)
{
...
oFF.RsVizVisualizationContainer.prototype.onQueryExecuted = function(extResult, resultSetContainer, customIdentifier)
{
	...
	else
	{
		...
		{
			var errorIt = extResult.getErrors().getIterator();
			while (errorIt.hasNext())
			{
				var errorMsg = errorIt.next();
...
oFF.RsDefDimension.prototype.addRsField2 = function(fieldName, fieldText, presentationType, valueType, mimeType, isHierarchyNavigationField)
{
	var rsField = oFF.RsDefField.create(fieldName, fieldText, presentationType, valueType, this, mimeType, isHierarchyNavigationField);
...
oFF.RsDefDimension.prototype.getDisplayGroup = oFF.noSupport;
oFF.RsDefDimension.prototype.setDisplayGroup = oFF.noSupport;
