...
sap.ui.define(
[
"sap/sac/df/firefly/ff2220.ui.tools","sap/sac/df/firefly/ff2670.visualization.ui","sap/sac/df/firefly/ff4310.olap.impl"
],
function(oFF)
{
"use strict";
...
oFF.RsHiChartRenderHelper.createWithVisualizationDefinition = function(visualizationDefinition, cursorResultSet)
{
	...
	if (oFF.notNull(cursorResultSet))
	{
		...
		var isTimeseries = chartType === oFF.ChartType.TIME_SERIES || chartSetting.isTimeSeries();
...
oFF.RsHiChartRenderHelper.prototype.setupBase = function(vizDef, globalDef, visualizationDefinition, rs, chartType, secondChart, stackingType, innerRadius, isPolar, isCombinationChart, inverted)
{
	...
	var leftMargin = 50;
	if (oFF.XString.isEqual(this.m_chartType, oFF.VizDefConstants.V_CHART_TYPE_BAR))
	{
		bottomMargin = 50;
		leftMargin = 150;
	}
	...
	var marginPrList = chartTypeInfo.putNewList(oFF.VizDefConstants.K_MARGIN);
	marginPrList.addInteger(topMargin);
	marginPrList.addInteger(rightMargin);
	marginPrList.addInteger(bottomMargin);
	marginPrList.addInteger(leftMargin);
	...
	if (oFF.notNull(vizDef))
	{
		var feedMembersFromVizDef = oFF.XHashMapByString.create();
		customFormatting = oFF.RsVizAxisExtractor.extractCustomFormatting(vizDef, globalDef);
		var feedMemberStructure = vizDef.getStructureByKey("feedMembers");
		oFF.XCollectionUtils.forEachString(feedMemberStructure.getKeysAsReadOnlyListOfString(),  function(feedKey){
			var feedValues = oFF.XStream.of(feedMemberStructure.getListByKey(feedKey)).filterNullValues().collect(oFF.XStreamCollector.toListOfString( function(fv){
...
oFF.RsVizAxisExtractor = {
	...
	getAxisMemberFeedTypes:function(cursorAxis, feedTypes)
	{
		...
		while (cursorAxis.hasNextFieldValue())
		{
			...
			for (var i = 0; i < keys.size(); i++)
			{
				...
				var feedMembers = feedTypes.getByKey(feedType);
				for (var j = 0; j < feedMembers.size(); j++)
				{
					var currentFeedName = feedMembers.get(j);
	...
	getFeedMembersFromVisualizationDefinition:function(rs, visualizationDefinition)
	{
		...
		var valueList2 = oFF.XListOfString.create();
		feedMembersMap.put(oFF.VizDefConstants.K_VALUE_AXIS2, valueList2);
		var valueListVariance = oFF.XListOfString.create();
	...
	vizProcessor:function(vizDef, chartTypeInfo, theChartData, chartType)
	{
		...
		if (oFF.notNull(vizDef))
		{
			...
			if (oFF.notNull(chartObject))
			{
				...
				var plotOptionsSeriesMarker = plotOptionsSeries.putNewStructure(oFF.VizDefConstants.K_MARKER);
...
oFF.RsSacTableRenderer.prototype.render = function(tableContainer)
{
	...
	else
	{
		if (table.isPartial())
		{
			...
			this.m_renderHelper.fillRowsFromList(table.getLocalRowList(), this.m_rowList, table.getTableRowOffsetForTupleIndex(table.getCurrentRowsOffset()), false, table.getFreezeUpToRow());
...
oFF.RsSacTableRenderer.prototype.reHighlight = function(rowMin, rowMax, colMin, colMax, tableContainer)
{
	...
	var result = this.reFormatInternal(tableContainer, false, false, false);
	tableContainer.resetRenderingScope();
	return result;
};
oFF.RsSacTableRenderer.prototype.reFormat = function(tableContainer)
{
	var tableContainerC = tableContainer;
	return oFF.isNull(tableContainerC) ? null : this.reFormatInternal(tableContainerC, tableContainerC.needsRecreateRows(), tableContainerC.needsReApplyFormatting(), true);
};
oFF.RsSacTableRenderer.prototype.reFormatInternal = function(tableContainer, recreateRows, reapplyStyles, markPristine)
{
	var table = oFF.isNull(tableContainer) ? null : tableContainer.getResultTable();
	var tableJson = null;
...
oFF.RsKpiRenderer.prototype.render = function(rs)
{
	...
	if (oFF.notNull(vizDef) && vizDef.containsKey(oFF.VizDefConstants.K_CHART) && rs.getAvailableDataCellCount() > 0)
	{
		...
		if (oFF.XString.isEqual(chart.getStringByKey(oFF.VizDefConstants.K_TYPE), oFF.SapKpiConstants.K_METRIC))
		{
			...
			var feedMemberStructure = vizDef.getStructureByKey("feedMembers");
			oFF.XCollectionUtils.forEachString(feedMemberStructure.getKeysAsReadOnlyListOfString(),  function(feedKey){
				var feedValues = oFF.XStream.of(feedMemberStructure.getListByKey(feedKey)).collect(oFF.XStreamCollector.toListOfString( function(fv){
					return fv.asString().getString();
				}.bind(this)));
				feedMembersFromVizDef.put(feedKey, feedValues);
			}.bind(this));
			var columnTuples = oFF.RsVizAxisExtractor.getTuplesFromColumnAxis(rs, feedMembersFromVizDef, customFormatting);
...
oFF.QStructureMemberFormatterSettings.create = function(structureMembers, queryModel)
{
	...
	var structureMember1 = oFF.XCollectionUtils.hasElements(structureMembers) ? structureMembers.get(0) : null;
	var structureMember2 = oFF.XCollectionUtils.hasElements(structureMembers) && structureMembers.size() > 1 ? structureMembers.get(1) : null;
	var queryDataCell = settings.getDataCell(structureMember1, structureMember2, queryModel);
...
oFF.SacTableAggregation.create = function()
{
...
oFF.SacTableAggregation.getMemberHeader = function(cursorAxis, dimHeader, previousHeader, overwriteTexts)
{
	...
	while (cursorAxis.hasNextTupleElement())
	{
		...
		while (cursorAxis.hasNextFieldValue())
		{
			...
			var newTableAggregation = oFF.SacTableAggregation.create();
			newTableAggregation.setText(formattedValue);
			newTableAggregation.setPlain(plain);
			newTableAggregation.setDimension(oFF.isNull(header) ? field.getDimension() : header.getDimension());
			newTableAggregation.setDimensionMemberType(memberType);
			newTableAggregation.setDrillState(tupleElement.getDrillState());
			newTableAggregation.setDisplayLevel(displayLevel);
			if (cursorAxis.hasUniqueScalingFactor())
			{
				newTableAggregation.setNumericShift(cursorAxis.getScalingFactor());
			}
			if (cursorAxis.hasUniqueComplexUnit())
			{
				newTableAggregation.setComplexUnit(cursorAxis.getComplexUnit());
			}
			if (cursorAxis.hasUniqueCurrencyUnit())
			{
				newTableAggregation.setCurrencyUnit(cursorAxis.getCurrencyUnit());
			}
			if (cursorAxis.hasUniqueCellValueType())
			{
				newTableAggregation.setCellValueType(cursorAxis.getCellValueType());
			}
			newTableAggregation.setField(oFF.isNull(header) ? field : header.getField());
			newTableAggregation.setIndex(oFF.isNull(header) ? -1 : header.getIndex());
			newTableAggregation.setMainAttribute(oFF.notNull(header) && header.isMainAttribute());
			newTableAggregation.setDimensionIndex(oFF.isNull(header) ? field.getDimension().getIndexOnAxis() : header.getDimensionIndex());
			newTableAggregation.setDimensionMemberName(dimensionMemberName);
			newTableAggregation.setParentDimensionMemberName(parentDimensionMemberName);
			newTableAggregation.setPathKey(pathKey);
			newTableAggregation.setRepeatedMemberName(repeatedMemberName);
			newTableAggregation.setValueException(valueException);
			newTableAggregation.setDimensionMemberValueException(dmValueException);
			newTableAggregation.setLowerLevelNodeAlignment(oFF.isNull(header) ? field.getDimension().getLowerLevelNodeAlignment() : header.getDimension().getHierarchyManager().getLowerLevelNodeAlignment());
	...
	for (i = 0; i < size; i++)
	{
		...
		if (dimension.isUniversalDisplayHierarchyDimension())
		{
			...
			for (j = 0; j < rsfSize; j++)
			{
				...
				aggregation = oFF.SacTableAggregation.create();
				aggregation.setText(text);
				aggregation.setPlain(field.getName());
				aggregation.setDimension(dimension);
				aggregation.setDisplayLevel(-1);
				aggregation.setField(field);
				aggregation.setIndex(index);
				aggregation.setMainAttribute(true);
				aggregation.setDimensionIndex(i);
				aggregation.setRepeatedMemberName(false);
		...
		else
		{
			...
			resultSetFields = baseDimension.getResultSetFields();
			var mainFields = oFF.isNull(mainAttribute) ? null : mainAttribute.getFields();
			var curAttribute;
			...
			for (j = 0; j < rsfSize; j++)
			{
				...
				else if (j === 0 && oFF.isNull(curAttribute) && oFF.isNull(mainFields) && oFF.SacTableAggregation.isImplicitMainField(dimension, field))
				{
					attributeText = oFF.SacTableAggregation.getDimText(overwriteTexts, dimension);
				}
				...
				var inMainFields = oFF.notNull(mainFields) && mainFields.contains(field) || oFF.isNull(mainFields) && field.getAttribute() === null && oFF.SacTableAggregation.isImplicitMainField(dimension, field);
				...
				aggregation.setField(field);
				aggregation.setIndex(index);
				aggregation.setMainAttribute(inMainFields);
				aggregation.setDimensionIndex(i);
				aggregation.setRepeatedMemberName(false);
				aggregation.setLowerLevelNodeAlignment(dimension.getLowerLevelNodeAlignment());
				result.put(field.getName(), aggregation);
...
oFF.SacTableAggregation.isImplicitMainField = function(dimension, field)
{
...
oFF.SacTableAggregation.prototype.m_numericShift = null;
oFF.SacTableAggregation.prototype.m_complexUnit = null;
oFF.SacTableAggregation.prototype.m_currencyUnit = null;
oFF.SacTableAggregation.prototype.m_cellValueType = null;
oFF.SacTableAggregation.prototype.setNumericShift = function(numericShift)
{
	this.m_numericShift = numericShift;
};
oFF.SacTableAggregation.prototype.setComplexUnit = function(unit)
{
	this.m_complexUnit = unit;
};
oFF.SacTableAggregation.prototype.setCurrencyUnit = function(unit)
{
	this.m_currencyUnit = unit;
};
oFF.SacTableAggregation.prototype.setCellValueType = function(valueType)
{
...
oFF.SacTableAggregation.prototype.getNumericShift = function()
{
	return this.m_numericShift;
};
oFF.SacTableAggregation.prototype.getReversedScalingFactor = function()
{
	return oFF.isNull(this.m_numericShift) ? null : oFF.XIntegerValue.create(-this.m_numericShift.getInteger());
};
oFF.SacTableAggregation.prototype.getCurrencyUnit = function()
{
	return this.m_currencyUnit;
};
oFF.SacTableAggregation.prototype.getComplexUnit = function()
{
	return this.m_complexUnit;
};
oFF.SacTableAggregation.prototype.getCellValueType = function()
{
...
oFF.SacTableAggregation.prototype.setText = function(text)
{
	this.m_text = text;
};
oFF.SacTableAggregation.prototype.setPlain = function(plain)
{
	this.m_plain = plain;
};
oFF.SacTableAggregation.prototype.setDimension = function(dimension)
{
	this.m_dimension = dimension;
};
oFF.SacTableAggregation.prototype.setDimensionMemberType = function(dimensionMemberType)
{
	this.m_dimensionMemberType = dimensionMemberType;
};
oFF.SacTableAggregation.prototype.setDrillState = function(drillState)
{
	this.m_drillState = drillState;
};
oFF.SacTableAggregation.prototype.setDisplayLevel = function(displayLevel)
{
	this.m_displayLevel = displayLevel;
};
oFF.SacTableAggregation.prototype.setField = function(field)
{
	this.m_field = field;
};
oFF.SacTableAggregation.prototype.setIndex = function(index)
{
	this.m_index = index;
};
oFF.SacTableAggregation.prototype.setMainAttribute = function(isMainAttribute)
{
	this.m_isMainAttribute = isMainAttribute;
};
oFF.SacTableAggregation.prototype.setDimensionMemberName = function(dimensionMemberName)
{
	this.m_dimensionMemberName = dimensionMemberName;
};
oFF.SacTableAggregation.prototype.setDimensionIndex = function(dimensionIndex)
{
	this.m_dimensionIndex = dimensionIndex;
};
oFF.SacTableAggregation.prototype.setParentDimensionMemberName = function(parentDimensionMemberName)
{
	this.m_parentDimensionMemberName = parentDimensionMemberName;
};
oFF.SacTableAggregation.prototype.setPathKey = function(pathKey)
{
	this.m_pathKey = pathKey;
};
oFF.SacTableAggregation.prototype.setRepeatedMemberName = function(repeatedMemberName)
{
	this.m_repeatedMemberName = repeatedMemberName;
};
oFF.SacTableAggregation.prototype.setValueException = function(valueException)
{
	this.m_valueException = valueException;
};
oFF.SacTableAggregation.prototype.setDimensionMemberValueException = function(dimensionMemberValueException)
{
	this.m_dimensionMemberValueException = dimensionMemberValueException;
};
...
oFF.OlapVizToPivotConverterUtil = {
	...
	{
			var dimensionHeader = tableAxis.addPartOfHeaderSectionInfo(false, false);
	...
	applyHeaderSectionInfoFromField:function(field, tableAxis)
	{
		...
		var axisLevel = dimension.getIndexOnAxis();
		var reversedAxisLevel = axisLevel - dimension.getAxis().size() - 1;
	...
	{
			var groupName = null;
		var field = axisPathElement.getField();
		if (oFF.isNull(field))
		{
			var attribute = axisPathElement.getAttribute();
			var dimension = axisPathElement.getDimension();
			var dimensionType = axisPathElement.getDimensionType();
			var presentationType = axisPathElement.getPresentationType();
			var semanticType = axisPathElement.getSemanticType();
	...
	applyCellTypeRestrictionFromViz:function(vizCellTypeRestriction, sacCellTypeRestriction)
	{
		...
		oFF.XCollectionUtils.forEach(vizCellTypeRestriction.getMatchingCellTypes(),  function(mct){
	...
	applyDataSectionInfoFromTuple:function(tableAggregations, tableAxis)
	{
			if (oFF.XCollectionUtils.hasElements(tableAggregations))
		{
			var keysIterator = tableAggregations.getKeysAsIteratorOfString();
	...
	{
			oFF.OlapVizToPivotConverterUtil.createDataPathElementFromTableAggregationDimension(dataPath, tableAggregation, true);
		oFF.OlapVizToPivotConverterUtil.createDataPathElementFromTableAggregationAttribute(dataPath, tableAggregation, true);
		oFF.OlapVizToPivotConverterUtil.createDataPathElementFromTableAggregationField(dataPath, tableAggregation, true);
		...
		{
			oFF.OlapVizToPivotConverterUtil.createDataPathElementFromTableAggregationDimension(dataPath, parentAggregation, false);
			oFF.OlapVizToPivotConverterUtil.createDataPathElementFromTableAggregationAttribute(dataPath, parentAggregation, false);
			oFF.OlapVizToPivotConverterUtil.createDataPathElementFromTableAggregationField(dataPath, parentAggregation, false);
			parentAggregation = parentAggregation.getParent();
		}
		return dataPath;
	},
	createDataPathElementFromTableAggregationDimension:function(dataPath, tableAggregation, exactSectionLevel)
	{
			var dataPathElement = oFF.OlapVizToPivotConverterUtil.createDataPathElementFromTableAggregation(dataPath, tableAggregation);
		...
		dataPathElement.setExactSectionLevel(exactSectionLevel);
		return dataPathElement;
	},
	createDataPathElementFromTableAggregationAttribute:function(dataPath, tableAggregation, exactSectionLevel)
	{
			var attribute = tableAggregation.getField().getAttribute();
		var dataPathElement = null;
		if (oFF.notNull(attribute))
		{
			dataPathElement = oFF.OlapVizToPivotConverterUtil.createDataPathElementFromTableAggregation(dataPath, tableAggregation);
			dataPathElement.setGroupName(attribute.getName());
			dataPathElement.setSectionNodeName(tableAggregation.getPlain());
			dataPathElement.setExactSectionLevel(exactSectionLevel);
		}
		return dataPathElement;
	},
	createDataPathElementFromTableAggregationField:function(dataPath, tableAggregation, exactSectionLevel)
	{
			var dataPathElement = oFF.OlapVizToPivotConverterUtil.createDataPathElementFromTableAggregation(dataPath, tableAggregation);
		dataPathElement.setGroupName(tableAggregation.getField().getName());
		dataPathElement.setSectionNodeName(tableAggregation.getPlain());
		dataPathElement.setExactSectionLevel(exactSectionLevel);
		return dataPathElement;
	},
	createDataPathElementFromTableAggregation:function(dataPath, tableAggregation)
	{
		...
		dataPathElement.setTotal(isTotal);
		dataPathElement.setGroupLevel(tableAggregation.getDimensionIndex());
		dataPathElement.setReversedGroupLevel(tableAggregation.getDimensionIndex() - tableAggregation.getDimension().getAxis().size() - 1);
	...
	applyTableStyleFromViz:function(formattingStyle, sacTableStyle)
	{
		...
		oFF.XCollectionUtils.forEach(formattingStyle.getCellTypeRestrictions(),  function(ctr){
...
oFF.SacAnalyticTableWidgetRenderHelper.prototype.postFormatCellChart = function(structure, parentRow, parentColumn)
{
	...
	if (oFF.XStringUtils.isNotNullAndNotEmpty(anaTab.getDimensionToColorate()) && oFF.XCollectionUtils.hasElements(anaTab.getDimensionColorPalette()))
	{
		...
		if (anaTab.getRowDimensionMemberIndex().containsKey(anaTab.getDimensionToColorate()))
		{
			...
			{
				color = oFF.RenderThemingHelper.remapColor(anaTab.getDimensionColorPalette().get(oFF.XMath.mod(index, paletteSize)));
...
oFF.RsHiChartCategorialSeriesHelper.prototype.readResultSetData = function(columnAxis, rowAxis, rs)
{
	...
	if (rs.getAvailableDataCellCount() > 0)
	{
		...
		var tuppleStrings = oFF.XStream.ofString(rowCategories).map( function(str){
			return oFF.XStringTokenizer.splitString(str.getString(), "/");
		...
		if (rowCategoryCount > 0)
		{
			...
			var categoriesList = xAxisStruct.putNewList(oFF.VizDefConstants.K_CATEGORIES);
			this.fillCategories(tuppleStrings, categoriesList);
		}
		else
		{
...
oFF.RsHiChartCategorialSeriesHelper.prototype.fillCategories = function(tuppleStrings, categories)
{
	for (var i = 0; i < tuppleStrings.size(); i++)
	{
		var tuppleString = tuppleStrings.get(i);
		this.createCategoryStructureRec(tuppleString, categories);
	}
};
oFF.RsHiChartCategorialSeriesHelper.prototype.createCategoryStructureRec = function(categoryNames, categoryPrList)
{
	if (oFF.XCollectionUtils.hasElements(categoryNames))
	{
		var categoryName = categoryNames.get(0);
		categoryNames.removeAt(0);
		var categoryStructure = this.getCategoryStructureWithName(categoryPrList, categoryName);
		if (oFF.isNull(categoryStructure))
		{
		...
		{
			categoryStructure.getListByKey(oFF.VizDefConstants.K_CATEGORIES).addString(categoryNames.get(0));
		}
		if (categoryNames.size() > 1)
		{
			this.createCategoryStructureRec(categoryNames, categoryStructure.getListByKey(oFF.VizDefConstants.K_CATEGORIES));
		}
	}
};
oFF.RsHiChartCategorialSeriesHelper.prototype.getCategoryStructureWithName = function(categoryPrList, categoryName)
{
	for (var i = 0; i < categoryPrList.size(); i++)
	{
		var categoryStructure = categoryPrList.get(i);
		var name = categoryStructure.getStringByKey(oFF.VizDefConstants.K_NAME);
		if (oFF.XString.isEqual(categoryName, name))
		{
...
oFF.SacAnalyticsTable.getValueException = function(dataCell)
{
	var valueException = dataCell.getValueException();
...
oFF.SacAnalyticsTable.getValueSign = function(dataCell)
{
	var valueException = dataCell.getValueException();
	var resultSign = null;
	...
	else if (oFF.SacAnalyticsTable.isCellUnbooked(dataCell))
	{
...
oFF.SacAnalyticsTable.isCellUnbooked = function(dataCell)
{
	var valueException = dataCell.getValueException();
...
oFF.SacAnalyticsTable.prototype.m_headerEndColumn = 0;
...
oFF.SacAnalyticsTable.prototype.m_supportsComments = false;
oFF.SacAnalyticsTable.prototype.m_derivePercentageFromShift = false;
...
oFF.SacAnalyticsTable.prototype.m_emptyCurrencyPair = null;
...
oFF.SacAnalyticsTable.prototype.applyTableMarkups = function()
{
	...
	{
		var vizLIterator = this.getAvailableLayeredStyles().getIterator();
		while (vizLIterator.hasNext())
		{
			oFF.OlapVizToPivotConverterUtil.applyLayeredRectangularStyleFromViz(vizLIterator.next(), this.addNewLayeredRectangularStyle());
		}
	}
};
oFF.SacAnalyticsTable.prototype.reApplyTableStyling = function()
{
	this.clearDataPointStyles();
	this.applyDataPointStyles();
	this.clearTableMarkups();
	this.clearLayeredRectangularStyles();
	this.applyTableMarkups();
};
...
oFF.SacAnalyticsTable.prototype.formatTable = function(rowFrom, rowTo, colFrom, colTo)
{
	...
	for (i = colTo === -1 || colTo >= this.m_tupleColumns.size() ? this.m_tupleColumns.size() - 1 : colTo; i > -1; i--)
	{
		if (this.m_tupleColumns.get(i) !== null)
		{
	...
	for (i = rowTo === -1 || rowTo >= this.m_tupleRows.size() ? this.m_tupleRows.size() - 1 : rowTo; i > -1; i--)
	{
		if (this.m_tupleRows.get(i) !== null)
		{
...
oFF.SacAnalyticsTable.prototype.createHeaderColumns = function()
{
	...
	this.m_headerEndColumn = this.m_rowKeysSize - 1;
};
oFF.SacAnalyticsTable.prototype.fillHeaderRows = function(offsetOrig, maxTupleId)
{
	...
	for (i = 0; i < this.m_columnKeysSize; i++)
	{
		...
		for (h = this.getHeaderColumnList().size() + offset; h < cells.size(); h++)
		{
			...
			columnTupleIndex = columnObject.getTupleIndex();
			...
			if (columnTupleIndex > -1)
			{
				...
				if (oFF.notNull(dimensionMemberName))
				{
	...
	if (this.getPreRowsAmount() > this.m_columnKeysSize)
	{
		...
		for (h = this.getHeaderColumnList().size() + offset; h < cells.size(); h++)
		{
			...
			if (columnTupleIndex > -1 && columnTupleIndex < this.m_columnValueAggregations.size())
			{
				cell.setType(oFF.SacTableConstants.CT_HEADER);
				oFF.OlapVizToPivotConverterUtil.applyHeaderSectionInfoFromUniqueHeader(row);
				valueAggregation = this.m_columnValueAggregations.get(columnTupleIndex);
				valueAggregationElement = oFF.XCollectionUtils.hasElements(this.m_columnKeys) && oFF.XCollectionUtils.hasElements(valueAggregation) ? valueAggregation.getByKey(this.m_columnKeys.get(0)) : null;
				var unitScalingHeader = this.getFormattedUnitScalingHeader(valueAggregationElement, -1, columnTupleIndex);
				if (oFF.XStringUtils.isNotNullAndNotEmpty(unitScalingHeader))
				{
					cell.getTableStyle().setHorizontalAlignment(oFF.SacTableCellHorizontalAlignment.RIGHT);
					cell.getTableStyle().setVerticalAlignment(oFF.SacTableCellVerticalAlignment.BOTTOM);
					cell.setFormatted(unitScalingHeader);
				}
			}
		}
	}
...
oFF.SacAnalyticsTable.prototype.createDataColumns = function(rs, offsetOrig, upperBound)
{
	...
	if (this.m_columnsTotalCount === -1 || this.m_columnsTotalCount > offsetOrig)
	{
		...
		if (oFF.XCollectionUtils.hasElements(this.m_columnValueAggregations))
		{
			var firstValueAggregation = this.m_columnValueAggregations.get(0);
			oFF.XCollectionUtils.forEachString(firstValueAggregation.getKeysAsReadOnlyListOfString(),  function(firstColKey){
				var vagg = firstValueAggregation.getByKey(firstColKey);
				if (vagg.getField() !== null && vagg.getDimension() !== null)
				{
					this.putColumnHeaderGroupMapping(vagg.getField().getName(), vagg.getDimension().getName());
				}
			}.bind(this));
		}
		...
		for (; tupleIndex < this.m_lastColumnToFetch; tupleIndex++, insertPosition++)
		{
			...
			oFF.OlapVizToPivotConverterUtil.applyDataSectionInfoFromTuple(valueAggregation, columnObject);
...
{
	var newOffset = this.createDataRows(rs, offsetOrig, upperBound);
	this.fillHeaderColumns(newOffset, this.m_lastRowToFetch);
	this.setReversedHierarchy(this.checkReversedHierarchy());
	return newOffset;
};
oFF.SacAnalyticsTable.prototype.checkReversedHierarchy = function()
{
	var reversedHierarchy = false;
	var rowsAxis = oFF.isNull(this.m_queryModel) ? null : this.m_queryModel.getRowsAxis();
...
oFF.SacAnalyticsTable.prototype.isAxisReversed = function(axis)
{
	var reversedHierarchy = false;
	if (axis.getModelCapabilities().supportsUniversalDisplayHierarchies() && axis.getQueryModel().getUniversalDisplayHierarchies().getByAxisType(axis.getType()).isActive())
	{
...
oFF.SacAnalyticsTable.prototype.createDataRows = function(rs, offsetOrig, upperBound)
{
	var offset = offsetOrig;
	...
	if (this.m_rowsTotalCount === -1 || this.m_rowsTotalCount > offsetOrig)
	{
		...
		if (oFF.XCollectionUtils.hasElements(this.m_rowValueAggregations))
		{
			var firstValueAggregation = this.m_rowValueAggregations.get(0);
			oFF.XCollectionUtils.forEachString(firstValueAggregation.getKeysAsReadOnlyListOfString(),  function(firstColKey){
				var vagg = firstValueAggregation.getByKey(firstColKey);
				if (vagg.getField() !== null && vagg.getDimension() !== null)
				{
					this.putRowHeaderGroupMapping(vagg.getField().getName(), vagg.getDimension().getName());
				}
			}.bind(this));
		}
		if (this.m_hasDataForRows)
		{
			...
			for (; h < this.m_lastRowToFetch; h++, insertPosition++)
			{
				...
				oFF.OlapVizToPivotConverterUtil.applyDataSectionInfoFromTuple(rowValueAggregation, rowObject);
...
oFF.SacAnalyticsTable.prototype.fillHeaderColumns = function(offset, upperBound)
{
	var rowObject;
	var cellObject;
	if (!this.m_emptyRowDimensions)
	{
		...
		for (var h = offset; h < this.getRowList().size(); h++)
		{
			rowObject = this.getRowList().get(h);
			if (oFF.isNull(rowObject))
			{
				continue;
			}
			var tupleIndex = rowObject.getTupleIndex();
			if (tupleIndex > -1)
			{
				if (upperBound > -1 && tupleIndex > upperBound)
				{
					break;
				}
				var rowValueAggregation = this.m_rowValueAggregations.get(tupleIndex);
				var valueAggregationElement;
				for (var i = 0; i < this.m_rowKeysSize; i++)
				{
				...
				if (this.m_rowKeysSize < this.getPreColumnsAmount())
				{
					cellObject = rowObject.getCells().get(this.getPreColumnsAmount() - 1);
					cellObject.setType(oFF.SacTableConstants.CT_HEADER);
					oFF.OlapVizToPivotConverterUtil.applyHeaderSectionInfoFromUniqueHeader(cellObject.getParentColumn());
					valueAggregationElement = oFF.XCollectionUtils.hasElements(this.m_rowKeys) && oFF.XCollectionUtils.hasElements(rowValueAggregation) ? rowValueAggregation.getByKey(this.m_rowKeys.get(0)) : null;
					var unitScalingHeader = this.getFormattedUnitScalingHeader(valueAggregationElement, tupleIndex, -1);
					if (oFF.XStringUtils.isNotNullAndNotEmpty(unitScalingHeader))
					{
						cellObject.getTableStyle().setHorizontalAlignment(oFF.SacTableCellHorizontalAlignment.LEFT);
						cellObject.getTableStyle().setVerticalAlignment(oFF.SacTableCellVerticalAlignment.BOTTOM);
						cellObject.setFormatted(unitScalingHeader);
					}
				}
			}
		}
	}
	if (this.getRowList().size() > 0 && this.m_emptyRowDimensions && this.m_showDimensionTitles && this.m_columnKeysSize > 0 && this.m_hasData)
	{
		rowObject = this.getRowList().get(0);
...
oFF.SacAnalyticsTable.prototype.retrieveColumnsTotalCount = function(rs)
{
	...
	this.m_columnsTotalCount = oFF.XMath.max(this.m_columnsTotalCount, oFF.XMath.max(rs.getDataColumns(), rs.getCursorColumnsAxis().getTuplesCount()));
};
oFF.SacAnalyticsTable.prototype.retrieveRowsTotalCount = function(rs)
{
	...
	this.m_rowsTotalCount = oFF.XMath.max(this.m_rowsTotalCount, oFF.XMath.max(rs.getDataRows(), rs.getCursorRowsAxis().getTuplesCount()));
};
				...
				if (rowTupleIndex > -1)
				{
					...
					for (var columnTupleIndex = offsetColumns; columnTupleIndex < this.m_lastColumnToFetch && columnTupleIndex < this.m_tupleColumns.size(); columnTupleIndex++)
					{
						...
						if (hasDataCells && columnTupleIndex > -1)
						{
							...
							{
								cellObject.setPlainString("");
							}
							if (dataCell.getValueException() !== null && dataCell.getValueException() !== oFF.ValueException.NORMAL)
							{
								cellObject.setValueException(oFF.SacAnalyticsTable.getValueException(dataCell));
							}
							if (this.m_supportsComments)
							{
							...
							exceptionInformation.setValueSign(oFF.SacAnalyticsTable.getValueSign(dataCell));
						}
					}
				}
			}
...
oFF.SacAnalyticsTable.prototype.getFormattedUnitScalingHeader = function(aggregation, rowTupleIndex, columnTupleIndex)
{
	var formattedValue = "";
	if (oFF.notNull(aggregation))
	{
		var structureMembers = this.extractStructureMembers(columnTupleIndex, rowTupleIndex);
		var settingsFactory = oFF.XNumberFormatterSettingsFactory.getInstance();
		var qStructureMemberFormatterSettings = oFF.QStructureMemberFormatterSettings.create(structureMembers, this.getQueryModel());
		var formatterSettings = settingsFactory.merge(this.m_userProfile, qStructureMemberFormatterSettings);
		var currencyUnitPair = this.getCurrencyUnitPair(aggregation.getComplexUnit(), false);
		if (aggregation.getCellValueType() === oFF.XValueType.PERCENT)
		{
			formattedValue = oFF.SacAnalyticsTable.PERCENT;
		}
		else if (oFF.notNull(currencyUnitPair))
		{
			formattedValue = oFF.XNumberFormatter.formatHeaderStringUsingSettingsWithCurrency(formatterSettings, currencyUnitPair, aggregation.getReversedScalingFactor(), this.m_scaleTextsMap);
		}
		else if (aggregation.getCurrencyUnit() !== null)
		{
			var currencyUnit = aggregation.getCurrencyUnit();
			...
			formattedValue = oFF.XNumberFormatter.formatHeaderStringUsingSettingsWithCurrency(formatterSettings, currency, aggregation.getReversedScalingFactor(), this.m_scaleTextsMap);
		}
		else
		{
			formattedValue = oFF.XNumberFormatter.formatHeaderStringUsingSettingsWithCurrency(formatterSettings, this.m_emptyCurrencyPair, aggregation.getReversedScalingFactor(), this.m_scaleTextsMap);
		}
	}
	return formattedValue;
};
...
{
	var rowValueAggregation = this.getFirstValueAggregation(this.m_rowValueAggregations, rowTupleIndex);
	var columnValueAggregation = this.getFirstValueAggregation(this.m_columnValueAggregations, columnTupleIndex);
	var skipUnit = this.shouldSkipUnit(rowValueAggregation) || this.shouldSkipUnit(columnValueAggregation);
	var skipScaling = this.shouldSkipScaling(rowValueAggregation) || this.shouldSkipScaling(columnValueAggregation);
	var skipPercent = this.shouldSkipPercent(rowValueAggregation) || this.shouldSkipPercent(columnValueAggregation);
	var formattedValue = dataCell.getFormattedValue();
	var excelFormatString = oFF.XNumberFormatter.standardizeFormatString(dataCell.getFormatString(), dataCell.getQueryModel().getSystemType().isTypeOf(oFF.SystemType.BW), dataCell.getValueType().isNumber() || dataCell.getValueType().isTypeOf(oFF.XValueType.NUMBER));
	var valueException = dataCell.getValueException();
	var currencyUnit = dataCell.getCurrencyUnit();
	var currencyUnitPrefix = "";
	var currencyUnitSuffix = "";
	...
	if (oFF.notNull(formattedValue) && (valueException === oFF.ValueException.NORMAL || valueException === oFF.ValueException.ZERO))
	{
		var emptyCurrency = oFF.XPairOfString.create(currencyUnitPrefix, currencyUnitSuffix);
		var structureMembers = this.extractStructureMembers(columnTupleIndex, rowTupleIndex);
		var valueType = dataCell.getValueType();
		if (this.shouldDerivePercentageFromShift(dataCell, structureMembers))
		{
			var formatterSettingsPercent = this.createFormatterSettings(dataCell, structureMembers);
			var scaleFormatTuplePercent = oFF.XNumberFormatter.formatDoubleToStringUsingSettingsWithCurrency(dataCell.getDouble(), formatterSettingsPercent, emptyCurrency, this.m_scaleTextsMap, excelFormatString, true);
			if (skipScaling)
			{
				formattedValue = scaleFormatTuplePercent.getFirstString();
			}
			else
			{
				formattedValue = oFF.XStringUtils.concatenate3(scaleFormatTuplePercent.getFirstString(), " ", oFF.SacAnalyticsTable.PERCENT);
			}
			excelFormatString = oFF.XStringUtils.concatenate3(scaleFormatTuplePercent.getSecondString(), " ", oFF.SacAnalyticsTable.PERCENT);
			unitFormattingApplied = true;
		}
		else if (valueType.isTypeOf(oFF.XValueType.DOUBLE))
		{
			var formatterSettings = this.createFormatterSettings(dataCell, structureMembers);
			var currencyUnitPair = this.getCurrencyUnitPair(dataCell.getComplexUnit(), skipUnit);
			unitFormattingApplied = true;
			if (oFF.notNull(currencyUnitPair))
			{
				var scaleFormatTuple = oFF.XNumberFormatter.formatDoubleToStringUsingSettingsWithCurrency(dataCell.getDouble(), formatterSettings, currencyUnitPair, this.m_scaleTextsMap, excelFormatString, skipScaling);
			...
			{
				var currency = oFF.XPairOfString.create(currencyUnitPrefix, currencyUnitSuffix);
				var scaleFormatTuple2 = oFF.XNumberFormatter.formatDoubleToStringUsingSettingsWithCurrency(dataCell.getDouble(), formatterSettings, currency, this.m_scaleTextsMap, excelFormatString, skipScaling);
		...
		{
			var formatterSettingsPercentBw = this.createFormatterSettings(dataCell, structureMembers);
			formatterSettingsPercentBw.setScaleFactor(oFF.XIntegerValue.create(-2));
			var scaleFormatTuplePercentBW = oFF.XNumberFormatter.formatDoubleToStringUsingSettingsWithCurrency(dataCell.getDouble(), formatterSettingsPercentBw, emptyCurrency, this.m_scaleTextsMap, excelFormatString, true);
			if (skipPercent)
			{
				formattedValue = scaleFormatTuplePercentBW.getFirstString();
			}
			else
			{
				formattedValue = oFF.XStringUtils.concatenate3(scaleFormatTuplePercentBW.getFirstString(), " ", oFF.SacAnalyticsTable.PERCENT);
			}
			excelFormatString = oFF.XStringUtils.concatenate3(scaleFormatTuplePercentBW.getSecondString(), " ", oFF.SacAnalyticsTable.PERCENT);
			unitFormattingApplied = true;
		}
	...
	{
		formattedValue = "*";
		excelFormatString = oFF.XNumberFormatter.wrapXlsString(formattedValue, null, null);
	}
...
oFF.SacAnalyticsTable.prototype.createFormatterSettings = function(dataCell, structureMembers)
{
	...
	var qStructureMemberFormatterSettings = oFF.QStructureMemberFormatterSettings.create(structureMembers, dataCell.getQueryModel());
	return settingsFactory.merge(formatterSettings, qStructureMemberFormatterSettings);
};
oFF.SacAnalyticsTable.prototype.shouldDerivePercentageFromShift = function(dataCell, structureMembers)
{
	var numericShift = dataCell.getScalingFactor();
	var structureMember1 = oFF.XCollectionUtils.hasElements(structureMembers) ? structureMembers.get(0) : null;
	var structureMember2 = oFF.XCollectionUtils.hasElements(structureMembers) && structureMembers.size() > 1 ? structureMembers.get(1) : null;
	if (!dataCell.getQueryModel().getModelCapabilities().supportsUnifiedDataCells())
	{
		var first = structureMember1;
		var second = structureMember2;
		if (oFF.notNull(structureMember1) && oFF.notNull(structureMember2) && structureMember1.getSolveOrder() < structureMember2.getSolveOrder())
		{
			first = structureMember2;
			second = structureMember1;
		}
		if (oFF.notNull(first) && first.getNumericShift() !== null)
		{
			numericShift = first.getNumericShift().getInteger();
		}
		else if (oFF.notNull(second) && second.getNumericShift() !== null)
		{
			numericShift = second.getNumericShift().getInteger();
		}
	}
	return this.m_derivePercentageFromShift && dataCell.getValueType().isTypeOf(oFF.XValueType.DOUBLE) && numericShift === 2 && (dataCell.getCurrencyUnit() === null || dataCell.getCurrencyUnit().isEmpty()) && this.getCurrencyUnitPair(dataCell.getComplexUnit(), false) === null;
};
oFF.SacAnalyticsTable.prototype.getFirstValueAggregation = function(valueAggregations, tupleIndex)
{
	var valueAggregation = null;
	if (tupleIndex > -1 && tupleIndex < valueAggregations.size())
	{
		var valueAggregationMap = valueAggregations.get(tupleIndex);
		if (oFF.XCollectionUtils.hasElements(valueAggregationMap))
		{
			valueAggregation = valueAggregationMap.getValuesAsReadOnlyList().get(0);
		}
	}
	return valueAggregation;
};
oFF.SacAnalyticsTable.prototype.shouldSkipUnit = function(valueAggregation)
{
	return oFF.notNull(valueAggregation) && (valueAggregation.getCurrencyUnit() !== null || valueAggregation.getComplexUnit() !== null);
};
oFF.SacAnalyticsTable.prototype.shouldSkipScaling = function(valueAggregation)
{
	return oFF.notNull(valueAggregation) && valueAggregation.getNumericShift() !== null;
};
oFF.SacAnalyticsTable.prototype.shouldSkipPercent = function(valueAggregation)
{
	return oFF.notNull(valueAggregation) && valueAggregation.getCellValueType() === oFF.XValueType.PERCENT;
};
oFF.SacAnalyticsTable.prototype.getCurrencyUnitPair = function(complexUnit, skipUnit)
{
	var pair = null;
	if (skipUnit)
	{
	...
	else if (oFF.notNull(complexUnit))
	{
...
oFF.SacAnalyticsTable.prototype.extractStructureMembers = function(columnIndex, rowIndex)
{
	var structureMembers = oFF.XList.create();
	if (oFF.XCollectionUtils.hasElements(this.m_columnValueAggregations) && columnIndex < this.m_columnValueAggregations.size() && columnIndex > -1)
	{
		this.extractStructureMembersInternal(structureMembers, this.m_columnValueAggregations.get(columnIndex));
	}
	if (oFF.XCollectionUtils.hasElements(this.m_rowValueAggregations) && rowIndex < this.m_rowValueAggregations.size() && rowIndex > -1)
	{
		this.extractStructureMembersInternal(structureMembers, this.m_rowValueAggregations.get(rowIndex));
	}
	return structureMembers;
};
oFF.SacAnalyticsTable.prototype.extractStructureMembersInternal = function(listToFill, sacTableAggregationIXMapByString)
{
	if (oFF.XCollectionUtils.hasElements(sacTableAggregationIXMapByString))
	{
		...
		for (var i = 0; i < values.size(); i++)
		{
			...
			{
				var member = dimension.getStructureMemberByAliasOrMember(value.getDimensionMemberName());
...
oFF.SacAnalyticsTable.prototype.getAvailableLayeredStyles = function()
{
...
oFF.SacAnalyticsTable.prototype.getTableRowOffsetForTupleIndex = function(tupleIndex)
{
	var index = tupleIndex;
	if (tupleIndex > -1 && tupleIndex < this.m_tupleRows.size())
	{
		var rowList = this.getRowList();
		index = rowList.getIndex(this.m_tupleRows.get(tupleIndex));
		while (index > 0 && rowList.get(index - 1) !== null && rowList.get(index - 1).getRootTupleIndex() === tupleIndex)
		{
			index--;
		}
		if (index === -1)
	...
	return index + this.getHeaderRowList().size();
};
oFF.SacAnalyticsTable.prototype.getTableColumnOffsetForTupleIndex = function(tupleIndex)
{
	var index = tupleIndex;
	if (tupleIndex > -1 && tupleIndex < this.m_tupleColumns.size())
	{
		var columnList = this.getColumnList();
		index = columnList.getIndex(this.m_tupleColumns.get(tupleIndex));
		while (index > 0 && columnList.get(index - 1) !== null && columnList.get(index - 1).getRootTupleIndex() === tupleIndex)
		{
			index--;
		}
		if (index === -1)
	...
	return index + this.getHeaderColumnList().size();
};
...
oFF.SacAnalyticsTable.prototype.getHeaderEndColumn = function()
{
	return this.m_headerEndColumn;
};
...
oFF.SacAnalyticsTable.prototype.setSupportsComments = function(supportsComments)
{
	this.m_supportsComments = supportsComments;
};
oFF.SacAnalyticsTable.prototype.putOverwriteText = function(key, value)
{
	this.m_overwriteTexts.put(key, value);
};
oFF.SacAnalyticsTable.prototype.setDerivePercentFromShift = function(derivePercentageFromShift)
{
...
oFF.SacAnalyticsTable.prototype.getColumnKeysSize = function()
{
	return this.m_columnKeysSize;
};
oFF.SacAnalyticsTable.prototype.getRowKeysSize = function()
{
	return this.m_rowKeysSize;
};
...
oFF.SacAnalyticsColumn.prototype.getRootDimensionIndex = function()
{
	if (this.getParentColumn() !== null && this.getParentColumn() !== this)
	{
		return this.getParentColumn().getRootTupleIndex();
...
oFF.SacAnalyticsColumn.prototype.getHeaderFieldSize = function()
{
	var columnList = this.getParentTable().getHeaderColumnList();
	var size = columnList.size();
	while (size > 0 && columnList.get(size - 1).getRootDimensionIndex() === -1)
	{
		size--;
	}
	return size;
...
oFF.SacAnalyticsRow.prototype.handleTotalsBand = function(maxLevel)
{
	var thisCells = this.getCells();
	for (var i = 0; i < thisCells.size(); i++)
	{
		var cell = thisCells.get(i);
		if (cell.isTotalsContext())
		{
			cell.setMergedColumns(maxLevel - i);
			break;
		}
	}
};
oFF.SacAnalyticsRow.prototype.moveOutBandInformation = function(newTuple, index)
{
	...
	for (var i = 0; i < headerColumnListSize; i++)
	{
		...
		otherCell.setType(thisCell.getType());
		if (headerColumn.getDimensionIndex() < index)
		{
			otherCell.setType(oFF.SacTableConstants.CT_HEADER);
		}
		else if (headerColumn.getDimensionIndex() === index)
		{
...
oFF.SacAnalyticsRow.prototype.getRootDimensionIndex = function()
{
	if (this.getParentRow() !== null && this.getParentRow() !== this)
	{
		return this.getParentRow().getRootDimensionIndex();
	}
	return this.m_dimensionIndex;
};
oFF.SacAnalyticsRow.prototype.getHeaderFieldSize = function()
{
	var rowList = this.getParentTable().getHeaderRowList();
	var size = rowList.size();
	while (size > 0 && rowList.get(size - 1).getRootDimensionIndex() === -1)
	{
		size--;
	}
	return size;
...
oFF.RsVizTableContainer.prototype.createVisualizationData = function()
{
	...
	if (oFF.isNull(tableData) || offsetRows === 0 && offsetColumns === 0 || emptyResultToRender)
	{
		...
		tableData.setSupportsComments(applicationSettings.isSupportCommentDocuments());
		tableData.setDerivePercentFromShift(applicationSettings.isDerivePercentageFromShift());
...
oFF.RsVizTableContainer.prototype.getHeaderEndColumn = function()
{
	var headerEndColumn = -1;
	var resultTable = this.getResultTable();
	if (oFF.notNull(resultTable))
	{
...
{
	return this.getResultTable() !== null && this.hasRowDimensions() && column < this.getHeaderColumnAmount() && column <= this.getHeaderEndColumn();
};
...
{
	var hct = this.getVisualizationDefinition().getEffectiveTableHeaderCompactionType();
	var fieldName;
