...
{
	this.setupAction(syncType, listener, customIdentifier, this.getActionContext());
	var filePath = this.getActionContext().getSystemDescription().getProperties().getStringByKey("METADATA");
	var file = oFF.XFile.create(this.getActionContext().getProcess(), filePath);
	file.processLoad(syncType, this, customIdentifier, oFF.CompressionType.NONE);
	return this;
};
oFF.OlapRpcVirtualFunction.prototype.onFileLoaded = function(extResult, file, fileContent, customIdentifier)
{
	this.m_metadata = !extResult.hasErrors() && oFF.notNull(fileContent) ? fileContent.getJsonContent() : null;
