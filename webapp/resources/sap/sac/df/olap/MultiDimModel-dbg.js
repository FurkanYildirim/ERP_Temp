...
sap.ui.define("sap/sac/df/olap/MultiDimModel", ["sap/base/Log", "sap/ui/model/Model",//
  ...
  "sap/sac/df/utils/SyncActionHelper",  "sap/sac/df/utils/ListHelper", //
  ...
  "sap/sac/df/thirdparty/lodash", "sap/sac/df/firefly/library"], /* eslint-disable max-params */

function (Log, Model, Context, ValueType, SystemLandscape, SyncActionHelper, ListHelper, //
  DFKernel, OlapListBinding, OlapListGridBinding, OlapPropertyBinding, DataProvider, ResourceBundle, StylingProvider, _, FF) {
  ...
  var MultiDimModel = DFKernel.extend("sap.sac.df.olap.MultiDimModel", {
    constructor: function (mSettings) {
      ...
      /**
         ...
         */
      that.addDataProvider = function (sDataProviderName, sQueryName, sSystem, sPackage, sSchema, sType) {
        ...
        return ensurePlanningService(systemName).then(function () {
          ...
          oModelData.DataProvider[sDataProviderName].SystemName = systemName;
          updateVariableGroup();
          that.fireEvent("queryAdded", {
            dataProviderName: sDataProviderName
          });
          that.fireEvent("dataProviderAdded", {
            dataProviderName: sDataProviderName
          });
      ...
      that.fireDataProviderUpdated = function(oParameters) {
        that.fireEvent("dataProviderUpdated", oParameters);
        return that;
      };
       ...
       * @param {string} sDataProviderName  the name of the new <code>DataProvider</code>.
       * @public
       * @returns {Promise<voidr>} Promise which resolves when the removing has been finished
       */
      that.removeDataProvider = function (sDataProviderName) {
        var dataProvider = oModelData.DataProvider[sDataProviderName];
        oModelData.DataProvider[sDataProviderName] = null;
        if(dataProvider){
          return dataProvider.logoff().then(function () {
            updateVariableGroup();
            that.fireEvent("dataProviderRemoved", {
              dataProviderName: sDataProviderName
            });
        ...
        _.forEach(oModelData.VariableGroups, function (group) {
          ...
          _.forEach(aVariables, function (oVariable) {
            if (oVariable.DataProviderName === sDataProviderName && !_.isEqual(group.MergedVariable.MemberFilter, oVariable.MemberFilter)) {
              oDataProvider.setVariableValueDirect(oVariable.Name, group.MergedVariable.MemberFilter, true);
            }
          });
        });
      ...
      var oProm = initCompletedPromise.then(function () {
        ...
        _.forEach(oFinalSettings.DataProvider || [], function (oDataProviderDef, sName) {
          var oRes = that.addDataProvider(sName, oDataProviderDef.dataSourceName, oDataProviderDef.systemName, oDataProviderDef.packageName, oDataProviderDef.schemaName, oDataProviderDef.dataSourceType);
      ...
      function updateVariableGroup(sGroupName) {
        ...
        function updateVariableInGroup(group, oVariable, sDataProviderName) {
          if (group.Rule(oVariable, sDataProviderName)) {
            ...
            oModelData.DataProvider[sDataProviderName].invalidate();
