          ...
          value: function formatAsync(resultSet) {
            ...
            try {
              for (_iterator.s(); !(_step = _iterator.n()).done;) {
                ...
                // default navigation: Add it, if there is no (Wikipedia) title link yet
                if (typeof item.defaultNavigationTarget === "undefined" && typeof item.titleAttributes[0] !== "undefined") {
                  var titleNavigationTarget = resultSet.sina._createNavigationTarget({
                    label: item.titleAttributes[0].valueFormatted,
                    // targetUrl: "...",
                    targetFunction: function targetFunction() {
                      MessageBox.information("Default nav. target via 'targetFunction'");
                    }
                  });
                  item.defaultNavigationTarget = titleNavigationTarget;
                }
              }
            } catch (err) {
              _iterator.e(err);
