...
sap.ui.define([
	...
	AxisTime.prototype.clone = function(){
		var oClonedAxisTime = new AxisTime([new Date(this.timeRange[0].valueOf()), new Date(this.timeRange[1].valueOf())],
			this.viewRange.slice(0), this.zoomRate, this.zoomOrigin, this.viewOffset, this.locale, this._oZoomStrategy);

		oClonedAxisTime._setEnableDateTimezoneFormatter(
			this._getEnableDateTimezoneFormatter()
		);

		return oClonedAxisTime;
	};
	...
	AxisTime.prototype.getNowLabel = function(bTimeInUTC){
		...
		} else if (this._getEnableDateTimezoneFormatter()) {
	...
	/**
	 * Returns the whether FLP Timezone formatting is enabled.
	 * @return {boolean} Index of first visible small interval label.
	 *
	 * @private
	 */
	AxisTime.prototype._getEnableDateTimezoneFormatter = function() {
		return this._enableDateTimezoneFormatter;
	};

	/**
	 * Set the value to enable or disable FLP Timezone formatting.
	 * @param {boolean} bValue The value to be set
	 *
	 * @private
	 */
	AxisTime.prototype._setEnableDateTimezoneFormatter = function(bValue) {
		this._enableDateTimezoneFormatter = bValue;
	};
