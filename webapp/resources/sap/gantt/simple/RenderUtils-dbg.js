	...
	var RenderUtils = {
		...
		renderMainRowAllShapes: function(oRm, oRowSetting, oGantt){
			...
			var sRowUid = oRowSetting.getRowUid(),
				...
				var aExpandScheme, aShapeSchemes = [], oMain;
				var aExpandedData = oGantt._oExpandModel.mExpanded[sRowUid];
				if (aExpandedData && aExpandedData[(aExpandedData.length) - 1]){
					for (var index = 1; index < (aExpandedData.length); index++){
						aShapeSchemes.push(aExpandedData[index].scheme);
					}
				}
				aExpandScheme = aShapeSchemes && oGantt.getShapeSchemes().filter(function(oScheme){
		...
		renderExpandShapesIfNecessary : function(oRm, oMainShape, mOption, bShowParentRow) {
			...
			var rowIndex = oMainShape.getParent().getParent().getIndex();
			if (oGantt.oExpandedTableRowShapesMap && oGantt.oExpandedTableRowShapesMap[rowIndex]){
				oGantt.oExpandedShapesMap[rowIndex] = oGantt.oExpandedTableRowShapesMap[rowIndex];
			} else {
			...
			if (!(oGantt.oExpandedTableRowShapesMap && oGantt.oExpandedTableRowShapesMap[rowIndex] && oGantt.oExpandedTableRowShapesMap[rowIndex].length)){
				if (oGantt.getUseParentShapeOnExpand()) {
					//experimental code
					// render subshapes only after all visible shapes are rendered
					if (!oGantt.oOverlapShapeIds || oGantt.oOverlapShapeIds && oGantt.oOverlapShapeIds[ oMainShape.getParentRowSettings().getParent().getIndex()]
