...
sap.ui.define([
    ...
    "sap/ui/core/Core",
    "sap/gantt/library"
], function (MultiActivityGroup, BaseRectangle, GanttUtils, Parameters, BaseImage, LinearGradient, Format, Stop, Core, library) {
	"use strict";
	/**
	 ...
	 * and to the FLP timezone based on EnableDateTimezoneFormatter value.
	 * @private
	 */
	BasePseudoShape.prototype._timeFormatter = function (oTaskBindingInfo, oBasePseudo, oShape, oGantt) {
		...
		var oFormatedTime = oGantt.getEnableDateTimezoneFormatter() ? GanttUtils.getFormatedDateByTimeZone(oTime) : oTime,
		oFormatedEndTime = oGantt.getEnableDateTimezoneFormatter() ? GanttUtils.getFormatedDateByTimeZone(oEndTime) : oEndTime;
		return {
			time: oFormatedTime,
			endTime: oFormatedEndTime
		};
	};
	/**
