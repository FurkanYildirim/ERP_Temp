...
sap.ui.define([
		...
		metadata: {
			...
			properties: {
				...
				*/
				...
				/**
				 * Flag to check whether the DateFormat should be used for Fiori launchpad timezone support. This flag is experimental, and it might be removed in a future release.
				 * When the flag is enabled, the start and end time for the shapes that are created outside of the <code>GanttChartWithTable</code> scope are not formatted to the Fiori launchpad timezone, unless those property values are binded again.
				 * @since 1.115
				 * @experimental
				 */
				enableDateTimezoneFormatter: {type: "boolean", defaultValue: false}
			},
			aggregations: {
				/**
	...
	/**
	* Sets EnableDateTimezoneFormatter property of the Gantt. This method is experimental, and it might be removed in a future release.
	* @param {boolean} bValue A value to be set for EnableDateTimezoneFormatter property of Gantt.
	* @param {boolean} bSuppressInvalidate Flag to suppress re-rendering of the control.
	* @since 1.115
	* @experimental
	* @public
	*/
	GanttChartWithTable.prototype.setEnableDateTimezoneFormatter = function (bValue, bSuppressInvalidate) {
		var oAxisTime = this.getAxisTime();

		if (oAxisTime) {
			oAxisTime._setEnableDateTimezoneFormatter(bValue);
		}

		this.setProperty("enableDateTimezoneFormatter", bValue, bSuppressInvalidate);
	};
	...
	GanttChartWithTable.prototype._updateVisibleHorizon = function (oTimeHorizon, sReasonCode, nVisibleWidth) {
		...
		var bChangeInInterval = (oPrevTimelineOptionInterval.unit != oCurrentTimelineOptionInterval.unit ||
			...
			oPrevTimelineOptionInterval.span != oCurrentTimelineOptionInterval.span) && sReasonCode != "horizontalScroll";
