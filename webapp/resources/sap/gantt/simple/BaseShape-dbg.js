...
sap.ui.define([
	...
	BaseShape.prototype.setTime = function (oDate, bSuppressInvalidate) {
		if (this._getEnableDateTimezoneFormatter()) {
			if (oDate) {
				this.setProperty("time", GanttUtils.getFormatedDateByTimeZone(oDate), bSuppressInvalidate);
			}
		} else {
			this.setProperty("time", oDate, bSuppressInvalidate);
		}
	};
	...
	BaseShape.prototype.setEndTime = function (oDate, bSuppressInvalidate) {
		if (this._getEnableDateTimezoneFormatter()) {
			...
			this.setProperty("endTime", oDate, bSuppressInvalidate);
		}
	};

	/**
	 * Function to ensure whether the time zone formatter should be enabled for date.
	 * @private
	 * @returns {bool} true: format the date, false: do not format
	 */
	BaseShape.prototype._getEnableDateTimezoneFormatter = function () {
		var oGantt = this.getGanttChartBase();
		return oGantt ? oGantt.getEnableDateTimezoneFormatter() : false;
	};
