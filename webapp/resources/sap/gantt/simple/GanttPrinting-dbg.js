...
sap.ui.define([
	...
	function (
		...
		/* When paper width in mm is converted to px, the dimensions are multiplied by 1.3 (scale factor of px) in jsPDF.
		To negate this and convert px to point -> paperWidth * 0.75 / 1.3 = paperWidth * 0.57 */
		var SCALEFACTORCONSTANT = 0.57;
		 ...
		 * @param {function} fnUpdateTable Function which takes cloned gantt chart's table for modification.
			...
			if (bPseudoShapeDisplay) {
				for (var key in oOverlapShapeIds){
					if (oOverlapShapeIds[key].length > 0) {
						aPseudoExpandIndices.push(parseInt(key, 10));
					}
				}
		...
		GanttPrinting.prototype._getCanvasOfPageN = function (iPageNumber, bForExport) {
			...
			if (bRTL) {
				...
				fXTextPosition = fPaperWidth - mmToPx(4);
				/* text padding */
			} else {
				oCanvasContext.textAlign = "left";
				fXTextPosition = mmToPx(4);
				/* text padding */
			}

			var fEllipseWidth = oCanvasContext.measureText("...").width;
			var fTargetWidth = fPaperWidth - mmToPx(4) - fEllipseWidth;
			...
			if (this._oModel.getProperty("/showHeaderText")) {
				...
				if (fHeaderTextWidth > (fPaperWidth - mmToPx(4))){
			...
			if (this._oModel.getProperty("/showFooterText")) {
				...
				if (fFooterTextWidth > (fPaperWidth - mmToPx(4))){
