	...
	var GanttUtils = {
		...
		findSpaceInLevel: function(level, shape, startTime, endTime) {
			...
			for (var j = 0; j < shapesWithRowLevels[level].length; j++) {
				...
				if (shape instanceof sap.gantt.simple.AdhocLine) {
					currEndTime = startTime;
				}
				if (shapesWithRowLevels[level][j] instanceof sap.gantt.simple.AdhocLine) {
					prevEndTime = startTime;
				}
				if (shapesWithRowLevels[level][j + 1] instanceof sap.gantt.simple.AdhocLine) {
		 ...
		 */
		calculateLevelForShapes: function (allShapes, startTimeProperty, endTimeProperty, bLines) {
