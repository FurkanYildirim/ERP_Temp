 ...
 */
sap.ui.define(
	["sap/ushell/utils/UrlParsing", "sap/ui/core/Core", "sap/base/Log"],
	function (UrlParsing, Core, Log) {
		"use strict";

		var sS4DestinationNameAlias = "(runtimeURL)";
		var sSAPLanguage = Core.getConfiguration().getSAPLogonLanguage();
		 ...
		 * @returns {string} URL with additional query parameter
		 */
		var addQueryParameter = function(sURL) {
			var sResultUrl = sURL;
		 ...
		 */
		var handleAction = function (oEvent) {
			...
			var oPromise = Promise.resolve(); //Wait for nothing or saveXAppState further down

			if (sIntent) {
				oEvent.preventDefault(true);

				if (bHasXAppStateData) {
					oPromise = saveXAppState(
						oCard,
						sXappStateId,
						oActionParams.ibnParams["sap-xapp-state-data"]
					);
				...
				oPromise
					.then(
						function () {
							return getRuntimeDestinationUrl(this._oCard); //_oCard is the card control instance
						}.bind(this)
					)
					.then(function (sRuntimeDestinationUrl) {
						if (sRuntimeDestinationUrl) {
							// We only navigate if we have the RuntimeDestination
							var sURL = addQueryParameter(sRuntimeDestinationUrl);
							sURL += sIntent;
