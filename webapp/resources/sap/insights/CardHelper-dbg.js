...
sap.ui.define([
    ...
    var PREVIEW_XML = "sap.insights.preview.Preview",
        ...
        HOUSE_OF_CARDS_DIALOG = "sap.insights.houseOfCards.HouseOfCardsDialog",
        CONFIGURE_DIALOG_XML = "sap.insights.configure.ColumnListDialog";
    ...
    var CardHelperService = {
         ...
         * @static
         */
        createCard: function (oCardManifest, bSkipValidation) {
            return this._mergeCard(oCardManifest, POST, bSkipValidation);
        },
         ...
         * @static
         */
        updateCard: function (oCardManifest, bSkipValidation) {
            return this._mergeCard(oCardManifest, PUT, bSkipValidation);
        },
         ...
         * @static
         */
        getUserCards: function (bForceUpdate) {
            if (bForceUpdate) {
                this.localCardCache = {};
            }
         ...
         * @static
         */
        getUserCardModel: function (bForceUpdate) {
            return this.getUserCards(bForceUpdate).then(function (aCards) {
        ...
        getSuggestedCards: function () {
            ...
            var url = CARD_READ_URL + "?$filter=visibility eq true&$select=descriptorContent,visibility,rank&$orderby=rank&$skip=0&$top=10";
            return this._readCard(url).then(function (aCards) {
    ...
    var UIService = {
        ...
        _getXMLView: function (sViewName, sId) {
        ...
        showCardPreview: function (oCard, bTransform, calledInternally) {
            if (!this.oCardLoadPromise) {
                this.oCardLoadPromise = this._getXMLView(PREVIEW_XML).then(function (oXMLView) {
                    return oXMLView.getController().showPreview(oCard, bTransform, calledInternally);
                })
                .finally(function() {
                    this.oCardLoadPromise = null;
                }.bind(this));
            }
            return this.oCardLoadPromise;
        },
        _getPreviewModelProperty: function(sProperty) {
            return this._getXMLView(PREVIEW_XML).then(function(oXMLView) {
                var oPreviewModel = oXMLView.getController().getView().getModel("cardPreviewModel");
                return oPreviewModel.getProperty(sProperty);
            });
          },

        _setPreviewModelProperty: function (sProperty, sValue) {
            return this._getXMLView(PREVIEW_XML).then(function(oXMLView) {
                var oPreviewModel = oXMLView.getController().getView().getModel("cardPreviewModel");
                return oPreviewModel.setProperty(sProperty, sValue);
            });
        },
        ...
        _showCardConfigureDialog: function (oCard) {
            return this._getXMLView(CONFIGURE_DIALOG_XML).then(function(oXMLView) {
              var oConfigView = oXMLView.getController().getView().getModel("configureView");
              if (!oConfigView.getProperty("/bDialogOpen")) {
                return oXMLView.getController().showColumnListDialog(oCard);
              }
        ...
        showHouseOfCardsDialog: function (oCard) {
            return this._getXMLView(HOUSE_OF_CARDS_DIALOG).then(function (oXMLView) {
                return oXMLView.getController().showHouseOfCardsDialog(oCard);
            });
        },
        _setHoCModelProperty: function (sProperty, sValue) {
            return this._getXMLView(HOUSE_OF_CARDS_DIALOG).then(function(oXMLView) {
                var oPreviewModel = oXMLView.getController().getView().getModel("hocModel");
                return oPreviewModel.setProperty(sProperty, sValue);
            });
          }
