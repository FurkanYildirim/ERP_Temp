...
sap.ui.define([
    ...
    var batchHelper = BaseObject.extend("ux.eng.s4producthomes1.utils.BatchHelper", {});
     ...
     */
    function MultiPartRequest(oPayload) {
        this.url = "INSIGHTS_CARDS('" + oPayload.id + "')";
        this.mOptions = {};
        this.batchRequests = [];
        this.payload = oPayload;
        this.boundary = "batch_id_" + Date.now() + "_01";
        this.mOptions.headers = {};
        this.mOptions.method = "PUT";
    ...
    batchHelper.createMultipartRequest = function (aPayloads) {
        return fetchCSRFToken().then(function(sCSRFToken){
            ...
            for (var iPayloadIndex = 0; iPayloadIndex < aPayloads.length; iPayloadIndex++) {
                ...
                var oMultRequest = new MultiPartRequest(oPayload);
                if (!oRequest) {
